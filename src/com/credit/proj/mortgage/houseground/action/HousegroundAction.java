package com.credit.proj.mortgage.houseground.action;
...
public class HousegroundAction extends CreditBaseAction {
	...
	private VProjMortHouseGround vProjMortHouseGround;
	 ...
	 * 添加信息
	 */
	public void addHouseground() {
		try {
			//final String projId = JbpmUtil.piKeyToProjectId(projectId);
			procreditMortgage.setMortgagenametypeid(pageconfig.HousegroundID);
			...
			procreditMortgage.setMortgagepersontypeforvalue(pageconfig.HousegroundValue);
	...
	public void seeHousegroundForUpdate() throws Exception{
		...
		List list=housegroundService.seeHouseground(rid);
		if(null!=list && list.size()!=0){
			vProjMortHouseGround=(VProjMortHouseGround)list.get(0);
			...
			mapObject.put("vProjMortHouseGround", vProjMortHouseGround);
	...
	public void updateHouseground(){
		...
		try {
			housegroundService.updateHouseground(mortgageid,procreditMortgageHouseground,procreditMortgage);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	
	public HousegroundService getHousegroundService() {
		return housegroundService;
	}

	public void setHousegroundService(HousegroundService housegroundService) {
		this.housegroundService = housegroundService;
	}
	...
	public VProjMortHouseGround getvProjMortHouseGround() {
		return vProjMortHouseGround;
	}

	public void setvProjMortHouseGround(VProjMortHouseGround vProjMortHouseGround) {
		this.vProjMortHouseGround = vProjMortHouseGround;
	}
