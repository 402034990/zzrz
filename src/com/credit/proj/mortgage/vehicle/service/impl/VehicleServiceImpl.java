...
public class VehicleServiceImpl implements  VehicleService{
	...
	public void addVehicle(ProcreditMortgageCar procreditMortgageCar,ProcreditMortgage procreditMortgage,BpMortgageCarConfigure bpMortgageCarConfigure,BpMortgageCarConfigureStatus bpMortgageCarConfigureStatus,BpMortgageCarPlaque bpMortgageCarPlaque,BpMortgageCarSurface bpMortgageCarSurface) throws Exception{
		boolean isSubmitOk = true;
		boolean isSubmitOk1 = true;
		boolean isSubmitOk2 = true;
		boolean isSubmitOk3 = true;
		boolean isSubmitOk4= true;
		try {
			...
			if(bpMortgageCarConfigure != null){
				bpMortgageCarConfigure.setMortgageid(procreditMortgage.getId());
				isSubmitOk1=  creditBaseDao.saveDatas(bpMortgageCarConfigure);
			}if (bpMortgageCarConfigureStatus != null){
				bpMortgageCarConfigureStatus.setMortgageid(procreditMortgage.getId());
				isSubmitOk2= creditBaseDao.saveDatas(bpMortgageCarConfigureStatus);
			}if (bpMortgageCarPlaque != null){
				bpMortgageCarPlaque.setMortgageid(procreditMortgage.getId());
				isSubmitOk3= creditBaseDao.saveDatas(bpMortgageCarPlaque);
			}if(bpMortgageCarSurface != null){
				bpMortgageCarSurface.setMortgageid(procreditMortgage.getId());
				isSubmitOk4= creditBaseDao.saveDatas(bpMortgageCarSurface);
			}
			...
			if(isSubmitOk && isSubmitOk1 && isSubmitOk2 && isSubmitOk3 && isSubmitOk4){
				...
				map.put("msg", "预售价格为："+procreditMortgageCar.getMarketPrice());
				JsonUtil.jsonFromObject(map, isSubmitOk) ;
	...
	//更新记录（u车保）
	@Transactional(propagation = Propagation.REQUIRED)
	public void updateVehicle(int mortgageid,ProcreditMortgageCar procreditMortgageCar,ProcreditMortgage procreditMortgage,BpMortgageCarConfigure bpMortgageCarConfigure,BpMortgageCarConfigureStatus bpMortgageCarConfigureStatus,BpMortgageCarPlaque bpMortgageCarPlaque,BpMortgageCarSurface bpMortgageCarSurface) throws Exception{
		boolean isUpdateOk = true;
		boolean isUpdateOk1 = true;
		boolean isUpdateOk2 = true;
		boolean isUpdateOk3 = true;
		boolean isUpdateOk4= true;
		...
		if(procreditMortgageCar != null){
			...
			try {
				...
				if(bpMortgageCarConfigure != null){
					isUpdateOk1 =creditBaseDao.updateDatas(bpMortgageCarConfigure);
				}if(bpMortgageCarConfigureStatus !=null){
					isUpdateOk2 =creditBaseDao.updateDatas(bpMortgageCarConfigureStatus);
				}if(bpMortgageCarPlaque !=null){
					isUpdateOk3 =creditBaseDao.updateDatas(bpMortgageCarPlaque);
				}if(bpMortgageCarPlaque !=null){
					isUpdateOk4 = creditBaseDao.updateDatas(bpMortgageCarSurface);
				}
				
				if(isUpdateOk && isUpdateOk1 && isUpdateOk2 && isUpdateOk3 && isUpdateOk4 ){
					
					Map<String, Object> map = new HashMap<String, Object>();
					map.put("procreditMortgage", procreditMortgage);
					map.put("procreditMortgageCar", procreditMortgageCar);
					map.put("msg", "预售价格为："+procreditMortgageCar.getMarketPrice());
					JsonUtil.jsonFromObject(map, isUpdateOk);
					//log.info("更新反担保车辆抵押贷款信息成功!!!");
//					JsonUtil.jsonFromObject("更新车辆抵押贷款信息成功!!!", isUpdateOk) ;
