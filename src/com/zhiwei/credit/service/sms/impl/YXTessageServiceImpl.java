...
public class YXTessageServiceImpl implements MessageStrategyService {
	...
	public String sendMsg(String phone, String content) {
		...
		if(ret[0].equals("8888")){
		     ...
		     String  LoginName = configMap.get("smsAccountID").toString();//获得登陆名称
		     String  Password = configMap.get("smsPassword").toString();//获得登陆密码
		     String  SendUrl = configMap.get("smsUrl").toString();//获得登陆密码
...
//		      post.addParameter("SpCode","201451");
//		      post.addParameter("LoginName","sh_drjr");
//		      post.addParameter("Password","sh_drjrE8@L7#");
//		      post.addParameter("MessageContent", "您的验证码为123456请在页面中输入以完成验证,如有问题请致电15701332275");
		      ...
		      System.out.println("#################################");
		      System.out.println("一信通|| 企业编号=="+SpCode+
		    		  " ||登陆账号=="+LoginName+" ||登陆密码=="+Password+" ||接收短信的手机号=="+phone+" ||发送内容=="+content);
		      try {
