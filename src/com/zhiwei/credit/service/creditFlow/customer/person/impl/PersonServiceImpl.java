...
public class PersonServiceImpl extends BaseServiceImpl<Person> implements PersonService{
		...
		if(person.getId() != null){
			...
			if(creditBaseDao.saveDatas(person)){
				...
				//JsonUtil.responseJsonString("{success:true,exsit:false,msg :'业务系统企业信息添加成功!      与财务系统对接信息："+str+"',newId:'"+pId+"'}");
		   		...
		   		//NC调用推送
		   		ncPushRecordService.newCustomersPush("person_customer",person.getId().toString());
				//return ;
			}else{
				log.error("添加失败");
	...
	@Override
	public String findPersonNumberById(String personId) {
		return dao.findPersonNumberById(Long.valueOf(personId));
	}
}