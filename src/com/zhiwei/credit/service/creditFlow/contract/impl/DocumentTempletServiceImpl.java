...
public class DocumentTempletServiceImpl extends
	...
	public void deleteFile(int id,String url){
		try{
			...
			tem.setHandleFun("<img title='上传模板文件' src='__ctxPath/images/upload-start.png' onclick='uploadTemplate("+tem.getId()+")'/>");
			tem.setFileid(0);
	...
	public void getAllDocumentTempletByMarkAndprojectId(String projectId,
		...
		if(projectId!=null&&!"".equals(projectId)){
			SlSmallloanProject slSmallloanProject=slSmallloanProjectDao.get(Long.valueOf(projectId));
			if(slSmallloanProject!=null){
				Long productId=slSmallloanProject.getProductId();
				BpProductParameter bpProductParameter=bpProductParameterDao.get(productId);
				String contranId=bpProductParameter.getContractId();
				if(contranId!=null&&!"".equals(contranId)){
					try{
						String ids[]=contranId.split(",");
						List<Integer> li=new ArrayList<Integer>();
						for (int i = 0; i < ids.length; i++) {
							DocumentTemplet document=dao.getById(Integer.valueOf(ids[i]));
							li.add(document.getParentid());
						}
						List<DocumentTemplet> list =dao.getListByOnlymark(mark);
						if(list != null && list.size()>0){
							DocumentTemplet dtt = list.get(0);
							//List<DocumentTemplet> list2 =dao.listByParentId(dtt.getId());
							List<DocumentTemplet> list2 =dao.listByParentId(dtt.getId(),li);
							StringBuffer buff = new StringBuffer("[");
							if(null!=list2 && list2.size()>0){
			...
			this.getAllDocumentTempletByOnlymark(mark);
		}
		
	}
	@Override
	public List<DocumentTemplet> listByParentId(int parentid, String projectId) {
		List<DocumentTemplet> li=null;
		if(projectId!=null&&!"".equals(projectId)){
			SlSmallloanProject slSmallloanProject=slSmallloanProjectDao.get(Long.valueOf(projectId));
			if(slSmallloanProject!=null){
				Long productId=slSmallloanProject.getProductId();
				...
				String contranId=bpProductParameter.getContractId();
				if(contranId!=null&&!"".equals(contranId)){
					li=dao.listByParentId(parentid,contranId);
				}
			}
		}
		return li;
