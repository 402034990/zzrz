...
public class ProcessServiceImpl implements ProcessService{
		...
		if(result==-1 || result>=1){//正常
			...
			if (processRun != null && processRun.getPiId() != null) {
				...
				if(list!=null&&list.size()!=0){
					if(list.size()>1){
						String taskId = list.get(1).getId();
						String activityName = list.get(1).getActivityName();
	...
	public ProcessRun doNextFlow(HttpServletRequest request) throws Exception{
		...
		try{
			...
			if(result==-1 || result>=1){//正常
				...
				thread.start();
				//发送短信
				sendShortMessage( processRun,nextInfo);
			}
			//执行之后的动作
	 ...
	 */
	public void sendShortMessage(ProcessRun processRun,FlowRunInfo flowRunInfo){
	...
	//	List<Long> assginList = proUserAssignService.getNextAssgin();
	...
	//	String mess=processRun.getCustomerName()+"-"+processRun.getProjectNumber()+"-"+processRun.getCreator();
		String mess=processRun.getSubject();
		if(signalName.contains("完成")||signalName.contains("结束")||signalName.contains("终止")){
			...
			for(Long str :allCreatorList){
				if(str!=null&&!"null".equals(str)&&!"".equals(str)&&!senderId.equals(str)&& 1!=str){
					AppUser user=appUserDao.get(str);
					if(null!=user.getMobile() && !"".equals(user.getMobile())){
					//	 System.out.println("-->"+user.getFullname());
		        	if(signalName.contains("完成")){
						sms.sms_projectFinish(user.getMobile(), mess, flowName, "1");//发送短信
					}else if(signalName.contains("结束")){
						sms.sms_projectFinish(user.getMobile(), mess, flowName, "2");//发送短信
					}else if(signalName.contains("终止")){
						sms.sms_projectFinish(user.getMobile(), mess, flowName, "3");//发送短信
					} 
	        	...
	        	for(int i=0;i<strs.length;i++){
	        		...
	        		if(null!=appuser1 && appuser1.size()>0){
	        			...
	        			while (it1.hasNext()) {  
	        				...
	        				if(1!=user.getUserId() && null!=user.getMobile() && !"".equals(user.getMobile())){
		    					  sms.sms_nextNodeRemind(user.getMobile(), mess, strs [i]);
		    					//  System.out.println("-->"+user.getFullname()); 
		    				  }
	        				}  
	        		}
	    		...
	    		if(null!=appuser && appuser.size()>0){
	    			...
	    			while (it.hasNext()) {  
	    				AppUser user = (AppUser) it.next(); 
	    				  if(1!=user.getUserId() && null!=user.getMobile() && !"".equals(user.getMobile())){
	    					  sms.sms_nextNodeRemind(user.getMobile(), mess, signalName);
	    					//  System.out.println("-->"+user.getFullname()); 
	    				  }
	    				}  
	    		}
	...
	/*			if(flowRunInfo.isSendMsg()){
					...
					if(appUser.getMobile()!=null && !appUser.getMobile().equals("")){
						...
						if(appUser.getMobile()!=null && !appUser.getMobile().equals("")){
							...
							String SmsType="0";
							String PhoneList=appUser.getMobile();
