...
public class SlPunishInterestAction extends BaseAction{
	...
	public String editQlideCheck() {
		...
		if(slPunishInterest.getType()==0){//修改罚息已对账
			slFundIntent.setFaxiAfterMoney(slPunishInterest.getAfterMoney());
			//计算罚息税
			if(slFundIntent.getPenaltyTax()==null){
				slFundIntent.setPenaltyTax(new BigDecimal("0"));
			}
			slFundIntent.setPenaltyTax(slFundIntent.getPenaltyTax().add(inIntentMoney.multiply(Constants.TAX)));
		}else{//修改逾期已对账
			slFundIntent.setOverdueAfterMoney(slPunishInterest.getAfterMoney());
			//计算逾期利息税
			if(slFundIntent.getOverdureTax()==null){
				slFundIntent.setOverdureTax(new BigDecimal("0"));
			}
			slFundIntent.setOverdureTax(slFundIntent.getOverdureTax().add(inIntentMoney.multiply(Constants.TAX)));
		}
		slFundIntentService.merge(slFundIntent);
		
		//NC推送调用
		//ncPushRecordService.penaltyPush(slPunishDetail);
		return SUCCESS;
	}
		...
		if (ids != null) {
			for (String id : ids ) {
				if (!id.equals("") && count < 1) {
					...
					if(slPunishInterest.getType()==0){//修改款项罚息已对账金额
						slFundIntent.setFaxiAfterMoney(slPunishInterest.getAfterMoney());
						slFundIntent.setPenaltyTax(slFundIntent.getPenaltyTax().add(payinmoney.multiply(Constants.TAX)));
					}else{//修改款项逾期已对账金额
						slFundIntent.setOverdueAfterMoney(slPunishInterest.getAfterMoney());
						slFundIntent.setOverdureTax(slFundIntent.getOverdureTax().add(payinmoney.multiply(Constants.TAX)));
					}
					slFundIntentService.merge(slFundIntent);
					//NC推送调用
					//ncPushRecordService.penaltyPush(slPunishDetail);
		...
		if (ids != null) {
			for (String id : ids ) {
				if (!id.equals("")) {
					...
					if(slPunishInterest.getType()==0){//修改罚息已对账
						slFundIntent.setFaxiAfterMoney(slPunishInterest.getAfterMoney());
						slFundIntent.setPenaltyTax(slFundIntent.getPenaltyTax().add(detailAfterMoney.multiply(Constants.TAX)));
					}else{//修改逾期已对账
						slFundIntent.setOverdueAfterMoney(slPunishInterest.getAfterMoney());
						slFundIntent.setOverdureTax(slFundIntent.getOverdureTax().add(detailAfterMoney.multiply(Constants.TAX)));
					}
					slFundIntentService.merge(slFundIntent);
	...
	public String list(){
		...
		List<SlPunishInterest> list= slPunishInterestService.listbyisInitialorId(fundIntentId,type);
...
//			 Collections.sort(list,comp);
	 ...
	 public String editAfterMoney() {
		 SlPunishInterest s = slPunishInterestService.get(slPunishInterest.getPunishInterestId());
		 if(null==s.getFlatMoney()){
