...
import com.zhiwei.credit.service.creditFlow.creditmanagement.OptionsComparator;
...
public class CreditRatingAction extends BaseAction {
	...
	public String result() {
		...
		List<RatingIndicator> list_result = new ArrayList<RatingIndicator>();
		StringBuffer buff = new StringBuffer("{success:true,result:[");
		for (int i = 0; i < arr_ratingIndicatorId.length; i++) {
			...
			RatingIndicator ri = (RatingIndicator) creditRatingDao
					.findRatingIndicator(ratingIndicatorId);
			...
			List ratingOptionList = creditRatingDao.getRatingOptionListOrder(ri
					.getId());
			String optionResult = "";
			/*-------------------------*/
	...
	public void deleteCreditRating() {
		String msg = "";
		try {
			...
			if(creditTemplateId!=0 &&!"".equals(creditTemplateId)){
				...
				if(indicatorList!=null&&indicatorList.size()>0){
					for(TemplateIndicator  temp:indicatorList){
						...
						List<Options> options=indicatorDao.getOptionsListOrder(indicator.getId(), "desc");
						if(options!=null&&options.size()>0){
							if("".equals(valueTest)){
								...
								OptionsComparator comparator=new OptionsComparator();
								Collections.sort(options, comparator);
								for(int i=0;i<options.size();i++){
									String comparValue="";
									if(options.get(i).getOptionName().contains("<=")){
										comparValue=options.get(i).getOptionName().substring(2, options.get(i).getOptionName().length());
									}else{
										comparValue=options.get(i).getOptionName().substring(1, options.get(i).getOptionName().length());
									}
									if(Integer.parseInt(valueTest)<=Integer.parseInt(comparValue)){
										if(i==0){
											...
											Atr_store+=options.get(i-1).getScore()+",";
											Atr_ants+=valueTest+",";
											ratingScore+=options.get(i-1).getScore()+options.get(i-1).getScore()*quanzhong;
											break;
										}
									}else{
										if(i==(options.size()-1)){
