...
public String listbycheckbox(){
		...
		EnterpriseBank  enterprise = new EnterpriseBank();
		if(name!=null&&!name.equals("")){
			enterprise.setName(name);
		}
		if(isEnterprise!=null&&!isEnterprise.equals("")){
			
			enterprise.setIsEnterprise(Short.valueOf(isEnterprise).shortValue());
		}
		if(accountnum!=null&&!accountnum.equals("")){
			enterprise.setAccountnum(accountnum);
		}
		...
		int totalProperty = 0;
		List<EnterpriseBank> list = enterpriseBankService.querySomeList(enterprise);
		totalProperty = enterpriseBankService.querySomeList(enterprise)==null?0:enterpriseBankService.querySomeList(enterprise).size() ;
		for(EnterpriseBank bank:list){
			if(null!=bank.getBankid()&&!"".equals(bank.getBankid())){
				CsBank cb=csBankService.get(bank.getBankid());
				if(null!=cb){
		...
		StringBuffer buff = new StringBuffer("{success:true,'totalCounts':").append(totalProperty).append(",result:");
