...
//			String relationPersonType=this.getRequest().getParameter("relationPersonType");
	...
	//更新客户联系人信息
	public void renewalRelation(){
		String id=getRequest().getParameter("personRelation.id");
		String  isKonw=getRequest().getParameter("personRelation.isKnow");
		if(id!=null&&!"".equals(id)){
			PersonRelation  personRelation=personRelationService.getById(Integer.valueOf(id));
			personRelation.setIsKnow(isKonw);
			try {
				personService.updateRelationPerson(personRelation);
			} catch (Exception e) {
				e.printStackTrace();
	...
	public String createNumber(){
		...
		if(personType.equals("person")){
			String personNum=personService.findPersonNumber();
			if(personNum!=null){
	...
	// 添加数据
	public void addInfo() {
		String  personsexId = this.getRequest().getParameter("person.sexId");
		// 页面只选省 ，手动添值
//		String homelandProvince=getRequest().getParameter("homelandProvince");
//		person.setHomeland(Integer.valueOf(homelandProvince));
		try {
        	...
        	person.setSex(Integer.valueOf(personsexId));
			...
			String num=personService.findPersonNumber();
	...
	public String seeInfo() {
		try {
			if("Financing".equals(businessType)){
				...
				Person p=null;
				if(!"0".equals(id)){
					p = personService.getById(Integer.valueOf(id));
	...
	public String delete() {
		...
		try {
			...
			if(flag==true){
				...
				personService.deletePerson(strTable,listArrayId,turl);
				for(int i= 0;i<listArrayId.length;i++){
...
//		String [] tableHeader = {"序号","姓名","性别","年龄","职务","婚姻状况","证件类型/号码","手机号码","家庭电话","出生日期"};
		String [] tableHeader = {"序号","姓名","客户编号","性别","证件类型","证件号码","手机号码","家庭电话"};
		try {
			Object ids=this.getRequest().getSession().getAttribute("userIds");
			Map<String,String> map=GroupUtil.separateFactor(getRequest(),ids);
...
//						BpCustPersonNegativeSurvey negativeInfo = (BpCustPersonNegativeSurvey) JSONMapper.toJava(parser.nextValue(), BpCustPersonNegativeSurvey.class);
	 ...
	 * @return
	 */
	public String findCardNumberById(){
		String personId=this.getRequest().getParameter("personId");
		if(personId!=null&&!"".equals(personId)){
			String personNumber=personService.findPersonNumberById(personId);
			if(personNumber!=null){
//				StringBuffer sb = new StringBuffer("{'topics':true,'licensenum':'"+personNumber+"'}");
				StringBuffer sb = new StringBuffer("{'topics':[{'licensenum':'"+personNumber+"'}]}");
