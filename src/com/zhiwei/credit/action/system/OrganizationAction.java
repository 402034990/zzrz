...
public class OrganizationAction extends BaseAction{
	...
	public String getDepartList1(){
		...
		list = organizationService.getDepartmentList(pb,sb.toString());
	...
	public String findParentByOrgId(){
		String userId=this.getRequest().getParameter("userId");
		Organization o=new Organization();
		Department dep=null;
		if(userId!=null){
			AppUser user=appUserService.get(Long.valueOf(userId));
			if(user!=null){
				dep=user.getDepartment();
				if(dep!=null){
					Long depId=dep.getDepId();
					o=organizationService.get(depId);
				}
				...
				if(parentId!=null){
					...
					if(organization!=null&&organization.getOrgType()!=2){
						o=organization;
					}else if(organization!=null&&organization.getOrgType()==2){
						Organization organization2= organizationService.get(organization.getOrgSupId());
						o=organization2;
					}
				}*/
			}
	...
	public String get(){
		...
		organization.setUserOrgs(null);//此行代码是为了解决对象转JSON时报错
    ...
    public String getOrgUserName(){
    	...
    	list=this.organizationService.listAllBranch(this.getRequest());
    ...
    public String getOrgList(){
    	List<Organization> list=this.organizationService.listAllBranch(this.getRequest());
     ...
     * @return
     */
    public String getOrgUserNameArray(){
    	List<Organization> list=new ArrayList<Organization>();
...
//		setJsonString(buff.toString());
	...
	/*
		String chargeIds = getRequest().getParameter("chargeIds");
		String isAddCompany=getRequest().getParameter("addCompany");
		String legalpersonid=getRequest().getParameter("organization.legalpersonid");
		if(organization.getOrgId()==null){
			if(organization.getOrgName()==null){
				organization.setOrgName(organization.getAgentName());
			}
			if(legalpersonid!=null&&!"".equals(legalpersonid)){
				organization.setLegalpersonid(Integer.valueOf(legalpersonid));
			}
			if(null!=isAddCompany && !"".equals(isAddCompany))
			{
				if(Boolean.valueOf(isAddCompany)){
					   boolean orgNameIsHave=this.organizationService.isExist("orgName", organization.getOrgName().trim());
					   if(orgNameIsHave){
						   setJsonString("{success:false,msg:'该代理商名称已经存在!'}");
						   return SUCCESS;
					   }
					   else if(this.organizationService.isExist("branchNO", organization.getBranchNO().trim())){
						   setJsonString("{success:false,msg:'代理商编号已经存在!'}");
						   return SUCCESS;
					   }
				}
			}
			...
			organization.setVmName(organization.getOrgName());//代理商vm文件的访问路径。新添后不可修改。
			organizationService.save(organization);
			if(chargeIds!=null&&chargeIds.length()>0){
				String[] pids = chargeIds.split(",");
				for(int index=0;index<pids.length;index++){
			...
			try{
					...
					if(Boolean.valueOf(isAddCompany)){
						if(Boolean.valueOf(isAddCompany)){
							  ...
							  if(orgNameIsHave){
								   ...
								   setJsonString("{success:false,msg:'该代理商缩写已经存在!'}");
								   return SUCCESS;
							  }
						}
				...
				if(legalpersonid!=null&&!"".equals(legalpersonid)){
					orgOrganization.setLegalpersonid(Integer.valueOf(legalpersonid));
				}
				Long parentId=orgOrganization.getOrgSupId();
				if(parentId==0){
					orgOrganization.setPath("0."+organization.getOrgId()+".");
	...
	*/}
	...
	public String tree(){
		...
		if(flag){
			...
			if(null!=orga && null!=orga.getOrgId()){
				...
				buff.append("]}]");
				System.out.println("最终结果集===="+buff.toString());
				setJsonString(buff.toString());
			}
		/*	String pDemId=getRequest().getParameter("demId");
			Long demId=0l;
	...
	public String getChildList(Long parentId,Long demId){
		...
		if(orgList.size()==0){
			...
			buff.deleteCharAt(buff.length() - 1);
			buff.append("]},");
			System.out.println("结果集==="+buff.toString());
			return buff.toString();
		}
	}
	/**
...
//		List<AppUser> users=appUserService.getDepUsers(orgPath, null, null);
	...
	public String getByUserIdToStoreNameAndStoreNameId(){
		...
		if(/*!"admin@jzww".equals(appuser2.getUsername())*/!"admin@hrsd".equals(appuser2.getUsername())){
