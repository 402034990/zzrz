...
public class DocumentTempletDaoImpl extends BaseDaoImpl<DocumentTemplet> implements DocumentTempletDao{
	...
	@Override
	public List<DocumentTemplet> getListByMarkAndcontranId(String contranId,
			String mark) {
		...
		String hql="from DocumentTemplet AS t where t.onlymark=? AND t.id in(:ids) ";
		return this.getSession().createQuery(hql).setParameter(0,mark).setParameterList("ids",list).list();
	}
	...
	public List<DocumentTemplet> listByParentId(int parentid, String contranId) {
		...
		String hql="from DocumentTemplet AS dt where dt.parentid=? AND dt.id in(:ids)";
		return this.getSession().createQuery(hql).setParameter(0,parentid).setParameterList("ids",list).list();
	}

	@Override
	public List<DocumentTemplet> listByParentId(int parentid, List<Integer> li) {
		String hql="from DocumentTemplet AS dt where dt.parentid=? AND dt.id in(:ids)";
		return this.getSession().createQuery(hql).setParameter(0,parentid).setParameterList("ids",li).list();
	}
}