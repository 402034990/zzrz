...
public class BpFundIntentDaoImpl extends BaseDaoImpl<BpFundIntent> implements
	...
	public List<BpFundIntent> listbyOwe(String businessType,
		...
		String hql="from BpFundIntent q where  q.fundType in (:fundtypelist) and q.isValid = 0 and q.isCheck = 0 and q.businessType=:businessType and  q.intentDate < :intentDate and q.notMoney !=0 ";
		return this.getSession().createQuery(hql).setParameterList("fundtypelist", fundtypelist).setParameter("businessType", businessType).setParameter("intentDate", date).list();
	}

	
	@Override
	public List<BpFundIntent> getByRequestNo(String requestNo) {
		String sql = "select * from BpFundIntent as fund where fund.requestNo=?";
	...
	public List<SlFundIntentPeriod> listByBidPlanIdAndpayintentPeriod( PagingBean pb,
		...
		sql.append(" IFNULL(bcm.loginname,bcm0.loginname) AS ptpborrowName, ");
		...
		sql.append(" WHERE  intentperiod1.isValid = 0 and intentperiod1.isCheck=0 	AND intentperiod1.fundType != 'principalLending'	 ");
...
/*	"	FROM"+
	...
	public List bidFundListByOrderNo(Long bidPlanId, String orderNo) {
		StringBuffer buff=new StringBuffer("SELECT"
				...
				+"jlf.reward"
				+" FROM bp_fund_intent AS f"
				+" left join (select sum(bj.incomeMoney) as principal,bj.payintentPeriod from bp_fund_intent as bj where bj.fundType='principalRepayment' and bj.bidPlanId="+bidPlanId+" and bj.orderNo='"+orderNo+"' group by bj.payintentPeriod) as bjf on bjf.payintentPeriod=f.payintentPeriod"
				+" left join (select sum(lx.incomeMoney) as interest,lx.payintentPeriod from bp_fund_intent as lx where lx.fundType='loanInterest' and lx.bidPlanId="+bidPlanId+" and lx.orderNo='"+orderNo+"' group by lx.payintentPeriod) as lxf on lxf.payintentPeriod=f.payintentPeriod"
	...
	public List<SlFundIntentPeriod> listOverDueByBidPlanIdAndpayintentPeriod(
		...
		sql.append(" bfp.oppositeType AS oppositeType FROM ");
		sql.append(" (SELECT * FROM bp_fund_intent intentperiod1 ");
		sql.append(" WHERE intentperiod1.payintentPeriod !=0 AND intentperiod1.isValid = 0 and intentperiod1.isCheck=0 	AND intentperiod1.fundType != 'principalLending' and intentperiod1.intentDate < DATE_FORMAT(DATE_ADD(NOW(),INTERVAL 1 DAY),'%Y-%m-%d') and intentperiod1.notMoney>0   ");
		sql.append(" GROUP BY	intentperiod1.bidPlanId,	intentperiod1.intentDate) AS  intentperiod ");
		sql.append(" LEFT JOIN invest_person_info info ON intentperiod.orderNo = info.orderNo ");
		sql.append(" LEFT JOIN pl_bid_plan pbp ON intentperiod.bidPlanId = pbp.bidId ");
		sql.append(" LEFT JOIN bp_fund_project bfp ON bfp.id = intentperiod.preceptId ");
		...
		//	return findObjectList(sql.toString(),null,pb,SlFundIntentPeriod.class);
			sql.append(" limit "+pb.getStart()+","+(pb.getStart()+pb.getPageSize()));
			System.out.println("sqlsqlsqlsql11111===="+sql);
			List<SlFundIntentPeriod> list = this.jdbcTemplate.query(sql.toString(),new rowMapper());
			  return list;
		}
	...
	public BigDecimal sumAllCompensatoryMoney(String planId, String peridId,String cardNo) {
		// TODO Auto-generated method stub
		String hql=" select sum(p.afterMoney)+ sum(p.accrualMoney) from bp_fund_intent as p where p.bidPlanId="+Long.valueOf(planId)+" and p.intentDate='"+peridId+"'";
