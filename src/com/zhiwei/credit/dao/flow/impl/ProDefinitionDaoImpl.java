...
public class ProDefinitionDaoImpl extends BaseDaoImpl<ProDefinition> implements ProDefinitionDao{
	...
	@Override
	public List<ProDefinition> getNormalFlowByProTypeId(Long proTypeId,String type) {
		String hql="";
		if(proTypeId!=null){
			if(type!=null&&!"".equals(type)){
				hql="select pd.defId as defId,pd.proTypeId as proType,gl.typeName as typename,pd.name as name from Pro_Definition pd LEFT JOIN global_type gl on pd.proTypeId=gl.proTypeId where pd.proTypeId = ? and (pd.branchCompanyId is null or pd.branchCompanyId=0) ";
				return ((SQLQuery) this.getSession().createSQLQuery(hql)
				.setParameter(0,proTypeId))
				.addScalar("defId",Hibernate.LONG)
				.addScalar("typename", Hibernate.STRING)
				.addScalar("name", Hibernate.STRING)
				.setResultTransformer(Transformers.aliasToBean(ProDefinition.class))
				.list();
			}else{
				hql="from ProDefinition pd where pd.proType.proTypeId=? and (pd.branchCompanyId is null or pd.branchCompanyId=0)";
				return findByHql(hql, new Object[]{proTypeId});
			}
		}else{
			hql="select pd.defId as defId,pd.proTypeId as proType,gl.typeName as typename,pd.name as name from Pro_Definition pd LEFT JOIN global_type gl on pd.proTypeId=gl.proTypeId where pd.branchCompanyId is null or pd.branchCompanyId=0 ";
	...
	public List<ProDefinition> getByBranchCompanyId(Long branchCompanyId,PagingBean pb,String flowName,String flowDesc){
		...
		if(flowDesc!=null&&!"".equals(flowDesc)){
			hql+=" and pd.description like '%"+flowDesc+"%'";
		}
