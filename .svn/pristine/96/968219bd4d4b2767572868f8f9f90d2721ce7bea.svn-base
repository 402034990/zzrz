 ...
 */
FinancialProductsDetail = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		FinancialProductsDetail.superclass.constructor.call(this, {
			id : 'FinancialProductsDetail'+this.reportKey,
			title : this.title,
			region : 'center',
			layout : 'anchor',
			iconCls : 'menu-report',
			...
			listeners : {
				'afterrender' : function(v) {
					...
					Ext.getCmp('FinancialProducts'+this.reportKey).getForm().submit({
						waitMsg : '正在提交查询',
						url : __ctxPath + '/system/reportPlmmInfoWealthReport.do?reportKey='+reportKey,
						method : 'post',
		...
		searchPanel = new HT.SearchPanel({
			...
			items : [{ 
				...
				items : [{
					...
					iconCls : 'btn-search',
					handler:function(){
						if (Ext.getCmp('FinancialProducts'+this.reportKey).getForm().isValid()) {
							Ext.getCmp('FinancialProducts'+this.reportKey).getForm().submit({
								waitMsg : '正在提交查询',
								url : __ctxPath + '/system/reportPlmmInfoWealthReport.do?reportKey='+reportKey,
								method : 'post',
					...
					handler:function(){
						Ext.getCmp('FinancialProducts'+this.reportKey).getForm().reset();
					}
				}]
			}]
		});
		
		var panel=Ext.getCmp('FinancialProducts'+reportKey);
