 ...
 * @createtime:
 */
SlFundIntentPrincipalPayAlready = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				SlFundIntentPrincipalPayAlready.superclass.constructor.call(this, {
							id : 'SlFundIntentPrincipalPayAlready'+this.businessType,
							title : this.businessType=='SmallLoan'?'已放款项':'已付款项',
							region : 'center',
							layout : 'border',
							iconCls :'btn-team2',
...
//						,{xtype:'label',html : '&nbsp;&nbsp已结清项】'}
				...
				this.gridPanel = new HT.GridPanel({
					...
					rowActions : false,
					loadMask : true,
					id : 'SlFundIntentGrid4principalpay',
					isautoLoad:true,
					url : __ctxPath + "/creditFlow/finance/listbyLedgerSlFundIntent.do?businessType="+this.businessType+this.getfundType()+"&typetab=actual",
					fields : [{
								name : 'fundIntentId',
								type : 'int'
							...
							'overdueRate', 'accrualMoney', 'status','remark','businessType','projectId','orgName','companyId'],
					columns : [{
								header : 'fundIntentId',
...
//			},
			...
			openliushuiwin1 : function(record,flag) {
				...
				if (s <= 0) {
					...
					var flag=1;
					new detailView({
						fundIntentId : s[0].data.fundIntentId,
						fundType : s[0].data.fundType,
						flag : flag,
						hidden1 : true,
						hidden2 :false,
						businessType: s[0].data.businessType
					}).show();
				}	
	      ...
	      pingAccount:function(record){
			...
			,
			  preview:function(record){debugger
				var s = this.gridPanel.getSelectionModel().getSelections();
				if (s <= 0) {
					Ext.ux.Toast.msg('操作信息','请选中一条记录');
					return false;
