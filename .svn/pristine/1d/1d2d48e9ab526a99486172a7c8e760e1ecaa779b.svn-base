 ...
 * @createtime:
 */
InvestFundIntentView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		InvestFundIntentView.superclass.constructor.call(this, {
					id : 'InvestFundIntentView',
					title : '全部款项明细台账',
					region : 'center',
					layout : 'border',
	...
	initUIComponents : function() {
		...
		this.gridPanel = new HT.GridPanel({
			...
			plugins : [summary],
			url : __ctxPath + "/creditFlow/finance/list2BpFundIntent.do",
			fields : [{
						name : 'fundIntentId'
					}, {
						name : 'fundType'
			...
			columns : [{
						...
						renderer:function(value){
							...
							if(objStore.getStore().getAt(idx)){
