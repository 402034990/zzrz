			...
			if(null!=projectId){
				...
				if(null!=project){
					...
					if(null!=project.getOppositeType() && project.getOppositeType().equals("company_customer")){
						...
						if(null!=project.getOppositeID()){
							Person person=personService.getById(project.getOppositeID().intValue());
							if(person.getBelongedId()!= null && !"".equals(person.getBelongedId())){
								String belongedName = "";
								String []str = person.getBelongedId().split(",");
								...
								person.setBelongedName(belongedName);
							}
							if(null!=person.getHomeland()){
								AreaDic areaDic=areaDicService.getById(person.getHomeland());
								person.setParentHomeland(areaDic.getParentId());
							}
							if(null!=person.getLiveCity()){
								AreaDic areaDic=areaDicService.getById(person.getLiveCity());
								person.setParentLiveCity(areaDic.getParentId());
							}
							...
							FileForm fileForm = fileFormService.getFileByMark("cs_person_tx."+person.getId());
							if(fileForm != null){
								person.setPersonPhotoId(fileForm.getFileid());
								person.setPersonPhotoUrl(fileForm.getWebPath());
								person.setPersonPhotoExtendName(fileForm.getExtendname());
							}
							FileForm fileForm2 = fileFormService.getFileByMark("cs_person_sfzz."+person.getId());
							if(fileForm2 != null){
								person.setPersonSFZZId(fileForm2.getFileid());
								person.setPersonSFZZUrl(fileForm2.getWebPath());
								person.setPersonSFZZExtendName(fileForm2.getExtendname());
							}
							FileForm fileForm3 = fileFormService.getFileByMark("cs_person_sfzf."+person.getId());
							if(fileForm3 != null){
								person.setPersonSFZFId(fileForm3.getFileid());
								person.setPersonSFZFUrl(fileForm3.getWebPath());
								person.setPersonSFZFExtendName(fileForm3.getExtendname());
							}
							map.put("person", person);
							if(null!=person){
								if(null!=person.getMarry() && person.getMarry()==317){
