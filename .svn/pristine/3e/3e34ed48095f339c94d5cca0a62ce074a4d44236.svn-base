 ...
 * @company 智维软件
 */
SlFundintentUrgeForm = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		Ext.applyIf(this, _cfg);
		// 必须先初始化组件
		this.initUIComponents();
		SlFundintentUrgeForm.superclass.constructor.call(this, {
					id : 'SlFundintentUrgeFormWin' + this.fundIntentId,
					layout : 'fit',
					...
					buttons : [{
								...
								handler : function() {
									...
									tabs.remove('SlFundintentUrgeFormWin'
											+ this.fundIntentId);
								}
							}]
				});
	...
	initUIComponents : function() {
		...
		if("LeaseFinance"==this.businessType){//update by gao
			this.htType = "flDunningLetter";//融资租赁的合同类型
		}else{
			this.htType="DunningLetter";//默认传小贷的
		}
	...
	urgegrid : function() {
		...
		var gridPanel2 = new HT.GridPanel({
			...
			id : 'SlFundIntentGrid',
			url : __ctxPath
					+ "/creditFlow/finance/listByfundIntentIdSlFundintentUrge.do?fundIntentId="
					+ this.fundIntentId,
			fields : [{
						name : 'fundIntentId',
						type : 'int'
						...
						items : [{
							...
							listeners : {
								...
								check : function(obj, checked) {
									var flag = Ext.getCmp("11"
											+ this.fundIntentId).getValue();
									if (flag == true) {
										this
