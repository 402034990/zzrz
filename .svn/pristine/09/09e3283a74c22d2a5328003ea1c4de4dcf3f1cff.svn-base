...
public class SlActualToChargeAction extends BaseAction{
	...
	public String check() {
		...
		if (payinmoney.compareTo(slActualToCharge.getAfterMoney()) == 0) {
			return SUCCESS;
		}
		int count = 0;
		if (ids != null) {
			for (String id : ids ) {
				if (!id.equals("") && count < 1) {
					...
					//NC推送调用
					ncPushRecordService.chargePush(slChargeDetail);
				}
			}
		}
	...
	public String editQlideCheck() {
		...
		if (slActualToCharge.getIncomeMoney().compareTo(lin) == 0) {
		 ...
		 //费用的初始化
		SlChargeDetail slChargeDetail = new SlChargeDetail();
		long day = CompareDate.compare_date(slActualToCharge.getIntentDate(), slFundQlide.getFactDate());
		if (day == -1 || day == 0) {
			if(null==slActualToCharge.getFactDate())
			{slActualToCharge.setIsOverdue("否");
			}
			...
			for (String id : ids ) {
				if (!id.equals("") && !id.equals("0")){
	                ...
	                if(slFundQlide.getMyAccount().equals("pingqlideAccount")){  //撤销平账
						...
						slActualToChargeService.save(slActualToCharge);
						//调用NC推送
						ncPushRecordService.backoutPush(slChargeDetail.getRequestNo());
				    }
				}
			}
		}
...
//		}
		...
		String[] tableHeader = { "序号", "项目名称", "项目编号", "手续费用收取类型", "计划收取金额",
				"计划到账日", "实际到账日", "已对账金额", "未对账金额",
				"核销金额","是否逾期","备注" };
		String[] fields = {"projectName","projectNumber","TypeName","incomeMoney",
...
//				}
	...
	public String manyIntentCheck(){
		if(fundQlideId!=null&&!"".equals(fundQlideId)){
...
//		 slActualToChargeService.evict(s);
		 ...
		 SlChargeDetail slChargeDetail = new SlChargeDetail();
			List<SlFundQlide> list = slFundQlideService.getcashQlide("ping");
			SlFundQlide slFundQlide = null;
			if(list!=null && list.size()>0){
				slFundQlide = (SlFundQlide)list.get(0);
			}
          
			s.setFlatMoney((s.getFlatMoney() == null ? lin : s.getFlatMoney()).add(slActualToCharge.getFlatMoney()));	
            s.setNotMoney(slActualToCharge.getNotMoney());
	//		s.setFactDate(new Date());
            
            slChargeDetail.setOperTime(new Date());
            slChargeDetail.setSlActualToCharge(s);
            slChargeDetail.setSlFundQlide(slFundQlide != null ? slFundQlide : null);
            slChargeDetail.setAfterMoney(slActualToCharge.getFlatMoney());
            slChargeDetail.setFactDate(slFundQlide != null ? slFundQlide.getFactDate() : new Date());
