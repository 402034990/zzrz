 ...
 * 是否为空 保存调用查看是否为空*/
	...
	var formSavePersonObj = function(formPanel ,winObj, storeObj,formObj){
	    ...
	    if(formPanel.getCmpByName('person.cardtype').getValue()==309&&!formPanel.getCmpByName('person.cardtype').allowBlank){
   ...
   var formSavePersonObj1 = function(formPanel ,winObj, storeObj,formObj){debugger
	    if(formPanel.getForm().isValid()){
	...
	var editEnterpriseInfo = function(enterpriseId,jStore_enterprise,formObj) {
		Ext.Ajax.request({
			...
			success : function(response,request) {
				...
				updateEnterprise(enterpriseData,personDate,jStore_enterprise,formObj);
			},
