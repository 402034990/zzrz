...
//			List<CsPersonCar> list = new ArrayList<CsPersonCar>(person.getPersonCars());
			if(projectId!=null&&!"undefined".equals(projectId)&&!"".equals(projectId)){
				List<CsPersonCar> list =csPersonCarService.getByPersonId(personId, Long.valueOf(projectId));
				StringBuffer buff = new StringBuffer("{success:true,'totalCounts':").append(list.size()).append(",result:");
				JSONSerializer json = JsonUtil.getJSONSerializer("factoryDate");
				json.transform(new DateTransformer("yyyy-MM-dd"), new String[] {"factoryDate","registrationDate"});
...
//			jsonString = buff.toString();
	 ...
	 */
	public String save(){
		String personId=getRequest().getParameter("csPersonCar.personId");
