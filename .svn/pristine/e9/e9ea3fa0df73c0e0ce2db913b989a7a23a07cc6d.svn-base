	...
	public Integer postponedApplyForNextStep(FlowRunInfo flowRunInfo) {
		...
		try {
			if (flowRunInfo.isBack()) {
				...
				BpFundProject fundProject =  bpFundProjectDao.getByProjectId(orgSlSuperviseRecord.getProjectId(),Short.valueOf("0"));
				...
				if (transitionName != null && !"".equals(transitionName)) {
					...
					if (transitionName.contains("终止") || transitionName.contains("结束")) {
						...
						orgSlSuperviseRecord.setCheckStatus(6);
									...
									if(null!=slist && slist.size()>0){	
										...
										if(null==f.getSlSuperviseRecordId() || (null!=f.getSlSuperviseRecordId() && !f.getSlSuperviseRecordId().toString().equals(orgSlSuperviseRecord.getId().toString()))){
											...
											if(f.getAfterMoney().compareTo(new BigDecimal(0))==0){
												//处理部分展期
												if(f.getIncomeMoney().compareTo(orgSlSuperviseRecord.getContinuationMoney())>0){
													SlFundIntent s=new SlFundIntent();
													s.setFundType("principalRepayment");
													s.setIncomeMoney(orgSlSuperviseRecord.getContinuationMoney());
													s.setPayMoney(new BigDecimal("0"));
													s.setIntentDate(f.getIntentDate());
													s.setFactDate(null);
													s.setAfterMoney(new BigDecimal("0"));
													s.setFlatMoney(new BigDecimal("0"));
													s.setNotMoney(orgSlSuperviseRecord.getContinuationMoney());
													s.setIsValid(Short.valueOf("1"));
													s.setIsCheck(Short.valueOf("1"));
													s.setPayintentPeriod(f.getPayintentPeriod());
													s.setAccrualMoney(new BigDecimal("0"));
													s.setBusinessType("SmallLoan");
													s.setRemark("");
													s.setProjectId(f.getProjectId());
													s.setProjectName(f.getProjectName());
													s.setProjectNumber(f.getProjectNumber());
													s.setInterestStarTime(f.getInterestStarTime());
													s.setInterestEndTime(f.getInterestEndTime());
													slFundIntentService.save(s);
													
													f.setIncomeMoney(f.getIncomeMoney().subtract(orgSlSuperviseRecord.getContinuationMoney()));
													f.setNotMoney(f.getIncomeMoney());
												...
												//处理部分展期
												if((f.getIncomeMoney().subtract(f.getAfterMoney())).compareTo(orgSlSuperviseRecord.getContinuationMoney())>0){
													sf.setFundType("principalRepayment");
													sf.setIncomeMoney(orgSlSuperviseRecord.getContinuationMoney());
													...
													sf.setNotMoney(orgSlSuperviseRecord.getContinuationMoney());
													...
													sf.setBusinessType("SmallLoan");
													sf.setRemark("");
													sf.setProjectId(f.getProjectId());
													sf.setProjectName(f.getProjectName());
													sf.setProjectNumber(f.getProjectNumber());
													sf.setInterestStarTime(f.getInterestStarTime());
													sf.setInterestEndTime(f.getInterestEndTime());
													slFundIntentService.save(sf);
													
													f.setIncomeMoney(f.getIncomeMoney().subtract(f.getAfterMoney()).subtract(orgSlSuperviseRecord.getContinuationMoney()));
													f.setNotMoney(f.getIncomeMoney());
													slFundIntentService.save(f);
												}else{
													sf.setFundType("principalRepayment");
