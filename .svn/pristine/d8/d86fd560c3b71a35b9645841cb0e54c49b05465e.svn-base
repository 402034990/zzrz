...
SmallLoanMortgageView = Ext.extend(Ext.Panel, {
			// 构造函数
			gridPanel:null,
			isHidden:false,
			...
			constructor : function(_cfg) {
				...
				// 调用父类构造
				SmallLoanMortgageView.superclass.constructor.call(this, {
							layout:'anchor',
		  					anchor : '100%',
							//title : '抵质押物管理',
							//region : 'center',
			...
			initUIComponents : function() {
				...
				var isHiddenSave=this.isHiddenSave;
		...
		//			)  
				...
				this.gridPanel = new HT.EditorGridPanel({
					...
					rowActions : false,
					//id : 'MortgageGridId',
					url : __ctxPath +'/credit/mortgage/ajaxGetMortgageAllDataByProjectId.do?projectId='+this.projectId+'&businessType='+this.businessType+'&isReadOnly='+this.isReadOnly+'&htType='+this.htType,
					fields : [{
								name : 'id',
								type : 'int'
			...
			addThirdContract : function(businessType, piKey) {
				...
				if (null == selected) {
					...
					if(this.htType=='baozContract'){
					 	title = selected.get('mortgagename')+'合同';
					}else{
						 title = selected.get('mortgagepersontypeforvalue')+'合同';
					}
