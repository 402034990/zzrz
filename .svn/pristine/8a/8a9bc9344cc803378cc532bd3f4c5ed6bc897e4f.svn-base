 ...
 * @createtime:
 */
LeaseFundIntentIncomeAll = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				LeaseFundIntentIncomeAll.superclass.constructor.call(this, {
							id : 'LeaseFundIntentIncomeAll'+this.businessType+this.type,
							title : '全部款项',
							region : 'center',
							layout : 'border',
							iconCls :'btn-team29',
			...
			initUIComponents : function() {
				...
				this.topbar = new Ext.Toolbar({
					items : [{
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_liushui_f_interestIncome_'+this.businessType+'_'+this.type)?false:true,
...
//					}
					,{
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_liushuisee_f_interestIncome_'+this.businessType+'_'+this.type)?false:true,
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_ping_f_interestIncome_'+this.businessType+'_'+this.type)?false:true,
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_fa_f_interestIncome_'+this.businessType+'_'+this.type)?false:true,
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_uploaddownpz_f_interestIncome_'+this.businessType+'_'+this.type)?false:true,
						handler : this.upload
			        	
			
					},new Ext.Toolbar.Separator({
						hidden : (isGranted('_uploaddownpz_f_interestIncome_'+this.businessType+'_'+this.type)?false:true)||(isGranted('_previewpz_f_'+this.businessType+'_'+this.type)?false:true)
					}),{
						iconCls : 'btn-setting',
						text :'预览凭证',
						xtype : 'button',
						scope : this,
						hidden : isGranted('_previewpz_f_interestIncome_'+this.businessType+'_'+this.type)?false:true,
...
//						,{xtype:'label',html : '&nbsp;&nbsp已结清项】'}
				...
				this.gridPanel = new HT.GridPanel({
					...
					fields : [{
							...
							'businessType','projectId','tabType','isInitialorId',
							'orgName','companyId','graceDay','continueDay','faxiAfterMoney','interestStarTime','interestEndTime'],
					columns : [{
								header : 'fundIntentId',
...
//			},
			...
			getfundType : function(){
				var sqlstr="&fundType=('loanInterest','consultationMoney','serviceMoney')";
				if(this.businessType!=null){
					businessType=this.businessType
					switch (businessType) {
					  case "SmallLoan": sqlstr="&fundType=('loanInterest','consultationMoney','serviceMoney')";
					    break;
					  ...
					  case "Pawn": sqlstr="&fundType=('pawnLoanInterest','pawnServiceMoney')";
					    break;
					  case "LeaseFinance": this.type=='marginCollection'?sqlstr="&fundType=('marginCollection')":(this.type=='retentionFee'?sqlstr="&fundType=('leaseObjectRetentionFee')":sqlstr="&fundType=('rentalFeeCharging','leaseFeeCharging','leasePrincipalRepayment')");
					    break;
					  default: sqlstr=" ";
					}
				}
