...
//						 Iterator it=bpCustProspectiveFollowups.iterator();
//					      while(it.hasNext())  {
//					    	  BpCustProspectiveFollowup oldObject=(BpCustProspectiveFollowup)it.next();
//					    	  Dictionary dicc=dictionaryService.get(Long.valueOf(oldObject.getCustomerSystematics()));
//					    	  temp.setCustomerSystematics(dicc.getItemValue());
//					    	  break;
//					      }
//					}
					//添加‘我的意向客户’页面展示的
					List<BpCustProspectiveFollowup> listBpCustProspectiveFollowup = bpCustProspectiveFollowupService.getListByPerId(temp.getPerId().toString(), null, null, null);
					if(listBpCustProspectiveFollowup.size()>0){
						BpCustProspectiveFollowup    bcpf = listBpCustProspectiveFollowup.get(0);
						Dictionary dicc=dictionaryService.get(Long.valueOf(bcpf.getCustomerSystematics()));
				    	temp.setCustomerSystematics(dicc.getItemValue());
					}
	...
	public String changePersonType(){
		...
		if(Prosperctive.getProsperctiveType().compareTo(Short.valueOf("1"))==0){//保存正式客户
			if(Prosperctive.getCustomerType()!=null&&Prosperctive.getCustomerType().compareTo(Short.valueOf("1"))==0){//属于正式企业客户
				Enterprise ent=new Enterprise();
				ent.setBelongedId(Prosperctive.getBelongId());
				ent.setCreaterId(Long.valueOf(Prosperctive.getCreatorId()));
				ent.setEnterprisename(Prosperctive.getCustomerName());
				ent.setTradetype(Prosperctive.getHangyeType());
				ent.setTelephone(Prosperctive.getTelephoneNumber());
				ent.setCompanyId(Prosperctive.getCompanyId());
				ent.setManagecity(Prosperctive.getAreaId());
				ent.setArea(Prosperctive.getPostaddress());
				ent.setEmail(Prosperctive.getEmail());
				enterpriseService.save(ent);
				Set<BpCustProspectiveRelation> bpCustProspectiveRelations =Prosperctive.getBpCustProspectiveRelations();
				while(bpCustProspectiveRelations.iterator().hasNext()){
					BpCustProspectiveRelation s =bpCustProspectiveRelations.iterator().next();
					EnterpriseRelationPerson  relation =new EnterpriseRelationPerson();
					relation.setEnterpriseid(ent.getId());
					relation.setCompanyId(ent.getCompanyId());
					relation.setRelationName(s.getRelateName());
					relation.setMark(Boolean.valueOf(s.getIsMain().toString()));
					relation.setRelationMovePhone(s.getRelationCellPhone());
					relation.setRelationJob(s.getRelatejob());
					relation.setRelationFamilyAddress(s.getRelateHouseDress());
					enterpriseRelationPersonService.save(relation);
				}
				
			}else {
		//		if(Prosperctive.getCustomerType().compareTo(Short.valueOf("2"))==0){//属于正式个人客户
				if(Prosperctive.getPersonType()==0){
					Person  p =new Person();
					p.setBelongedId(Prosperctive.getBelongId());
					p.setCreaterId(Long.valueOf(Prosperctive.getCreatorId()));
					p.setCompanyId(Prosperctive.getCompanyId());
					p.setName(Prosperctive.getCustomerName());
					p.setCellphone(Prosperctive.getTelephoneNumber());
					p.setHukou(Prosperctive.getArea());
					p.setPostaddress(Prosperctive.getPostaddress());
					p.setSelfemail(Prosperctive.getEmail());
					...
					while(bpCustProspectiveRelations.iterator().hasNext()){
						...
						relation.setCompanyId(p.getCompanyId());
						relation.setRelationName(s.getRelateName());
						relation.setRelationShip(s.getRelateship());
						relation.setRelationCellPhone(s.getRelationCellPhone());
						personRelationService.save(relation);
					}
				}else if(Prosperctive.getPersonType()==1){
					
					CsInvestmentperson ciPerson = new CsInvestmentperson();
					ciPerson.setBelongedId(Prosperctive.getBelongId());
					ciPerson.setCreaterId(Long.valueOf(Prosperctive.getCreatorId()));
					ciPerson.setCompanyId(Prosperctive.getCompanyId());
					ciPerson.setInvestName(Prosperctive.getCustomerName());
					ciPerson.setSex(Prosperctive.getSex());
					ciPerson.setCellphone(Prosperctive.getTelephoneNumber());
					ciPerson.setSelfemail(Prosperctive.getEmail());
					ciPerson.setPostcode(Prosperctive.getPostalcode());
					ciPerson.setPostaddress(Prosperctive.getPostaddress());
					csInvestmentpersonService.save(ciPerson);
	...
	public void getOnlineUserDepartment(){
		...
		try {
			...
			endorganization = findOrganization(department.getDepId());
...
//			}
	...
	/**
	 * 根据当前登录人的所在部门的id寻找代理商 名称
	 * @param departmentId
	 * @return
	 */
	public Organization findOrganization(Long  departmentId){
		Organization  organizationretu = null;
		Organization organization = organizationService.get(departmentId);
		if(organization.getOrgType()==1||organization.getOrgType()==0||organization.getOrgType()==3){
			organizationretu = organization;
		}else {
			organizationretu = 	findOrganization(organization.getOrgSupId());
		}
		return  organizationretu;
	}
