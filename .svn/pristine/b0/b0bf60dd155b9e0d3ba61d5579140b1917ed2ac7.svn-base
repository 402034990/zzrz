 ...
 * @author
 * @createtime
 * @class SlGenerationContractList
 * @extends Ext.Window
 * @description SlGenerationContractList
 * @company
 */
SlGenerationContractList = Ext.extend(Ext.Window, {
	panelContract : null,
	categoryId:null,
	piKey:null,
	productId : null,
	...
	constructor : function(_cfg) {
		...
		SlGenerationContractList.superclass.constructor.call(this, {
			id:'SlContractWin',
			layout : 'anchor',
...
//						this.htType = "XEDQ";
	...
	makeContract :function(temId,businessType,piKey,taskId,contractType,categoryId,contractId,a,b,htType,thirdRalationId,isApply,clauseId){
		...
		if(temId == null){
			...
			var args ={
				productId : piKey ,//产品编号
				contractId:contractCategoryText//合同ID
			}
			var orMake ='make';
			if(orMake == 'make'){
				...
				Ext.Ajax.request({
					url : __ctxPath+ '/system/saveContractBpProductParameter.do',
					method : 'POST',
					scope : this,
					success : function(response, request) {
						var obj = Ext.util.JSON.decode(response.responseText);
