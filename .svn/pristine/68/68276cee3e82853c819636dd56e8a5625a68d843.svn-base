...
var personObj = Ext.extend(Ext.form.FormPanel, {
	...
	constructor : function(_cfg) {
		...
		if(null!=_cfg.isReadSome){
		    this.isReadSome=true;
		};
		if(null!=_cfg.isViews){
		...
		this.PersonHouseView =new PersonHouseInfo({
			personId:null,
			isHiddenAddBtn:this.isRead,
			isHiddenDelBtn:this.isRead,
			isReadOnly:this.isRead,
							...
							items : [{
								...
								items : [{
									...
									items : [{
										...
										value : personData == null ? null : personData.personNumber,
										listeners : {
											...
											if(person==null){
												Ext.Ajax.request({
													url : __ctxPath
													+ '/creditFlow/customer/person/createNumberPerson.do',
													method : 'POST',
													scope : this,
									...
									
									{
										...
										name : 'person.name',
										blankText : '姓名为必填内容',
										readOnly : this.isReadSome==true?true:this.isRead,
										...
										listeners : {
											...
											'blur' : function(f) {
												if(this.getCmpByName('person.cardtype').getValue()==309){
													...
													var personSex =this.getCmpByName("person.sex");
													var personSexId =this.getCmpByName("person.sexId");
...
//						                            			if(!isEdit && personData==null){//只有新增才需要默认加载身份证上的出生年月日
							                            			...
							                            			penalAge.setValue(Number(nowYear)-Number(brith))//年龄d
							                            			//设置性别
							                            			if(cardNumber.length==18){
							                            				var sexdata = cardNumber.substring(16,17);
								                            			var aa = sexdata%2;
								                            			var  storeLength = personSex.store.data.items;
								                            			var  sexCh = "";
								                            			if(aa==0){
								                            				sexCh = "女";
								                            			}else{
								                            			...
								                            			for(var  i = 0; i<storeLength.length;i++){
								                            				if(storeLength[i].data.itemName==sexCh){
								                            					personSex.setValue(storeLength[i].data.itemId);
								                            					personSexId.setValue(storeLength[i].data.itemId);
								                            				}
								                            			}
							                            			}else{
							                            				personSexId.setValue("");
								                            			personSex.setValue("");
								                            			penal.setValue("");
								                            			penalAge.setValue("")//年龄d
							                            			}
//						                            			}
						                            		}
								                      }
...
//									},
										
									{
										...
										editable : false,
										readOnly : this.isRead,
										value : personData == null ? null : personData.belongedName,
										scope : this,
...
//								xtype : 'textfield',
						...
						items : [{
								...
								xtype : 'hidden',
								value : personData == null? null: personData.havehouse,
								...
								listeners : {
									...
									afterrender : function(com){
										 ...
										 if(linkmanKnown==1){
										 	linkmanKnownObj.setValue(1);
										 }else{
										 	linkmanKnownObj.setValue(0);
										 }
									}
								}
							}/*,{
...
//									var organizecode=this.getCmpByName('enterprise.organizecode').getValue();
									...
									new bplawsuit({litigationType:personId,enterpriseId:personId,isReadOnly:read}).show()
									   //bankInfoListWin(enterpriseOrganizecodeValue,isReadOnly); 此处企业id传后台无用功，只为保存成功
								}
							}/*,{
								text : '借款意向',
								tooltip : '借款意向信息',
								...
								handler : function() {
									...
									if (!flag
									{
									     ...
									     //new PersonAll({customerType:'person_customer',customerId :personId,personType:603,shareequityType:'person_shareequity',isReadOnly:read}).show()
										new CustomerBorrowingIntention().show();
									}
									new SeeBorrowingIntention().show();
								}
							},{
								text : '合同管理',
								tooltip : '合同信息',
