/**
 * @author
 * @class RaiseRateAssignIntent
 * @extends Ext.Panel
 * @description [PlBidInfo]管理
 * @company 智维软件
 * @createtime:
 */
RaiseRateAssignIntent = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		RaiseRateAssignIntent.superclass.constructor.call(this, {
					id : 'RaiseRateAssignIntent_'+this.userType+this.Type,
					title : this.title,
					iconCls : this.userType==0?"btn-tree-team17":"btn-tree-team18",
					region : 'center',
					layout : 'border',
	...
	initUIComponents : function() {
		...
		this.gridPanel = new HT.GridPanel({
			...
			id : 'raiserateGrid',
			plugins : [summary],
			url : __ctxPath + "/creditFlow/financingAgency/getCouponsListPlMmOrderAssignInterest.do?fundType=raiseinterest",
			fields : [ 'mmplanId', 'mmName', 'mmNumber', 'rebateType', 'rebateWay','periods','sumMoney','fundType','intentDate','factDate','raiseRate','sumAfterMoney'],
			columns : [{
						header : 'mmplanId',
						dataIndex : 'mmplanId',
						hidden : true
	...
	distributeMoney :function(){
		...
		if (s <= 0) {
			...
			Ext.Msg.confirm('派息确认','此次派息系统将会检查该项目该期尚未派息的奖励进行派息',function(btn){
		    	...
		    	Ext.Ajax.request({
					url : __ctxPath
							+ "/creditFlow/financingAgency/distributeMoneyPlMmOrderAssignInterest.do?planId="+planId+"&fundType=raiseinterest&periods=0",
					method : 'post',
	...
	couponsExcel : function(){
					...
					window.open( __ctxPath +"/creditFlow/financingAgency/excelRaiseAssignPlMmOrderAssignInterest.do?fundType=raiseinterest&bidName="+bidName+"&bidProNumber="+bidProNumber+"&" +
