...
DicManager=Ext.extend(Ext.Panel,{
    ...
    contextMenuItems : [
    	{  
                 ...
                 scope : this,  
                 iconCls:'btn-add',
                 hidden : isGranted('_DictionaryAddType')?false:true,
                 handler : function(){
             	...
             	scope:this,
             	iconCls:'btn-edit',
             	hidden : isGranted('_DictionaryEditType')?false:true,
             	handler:function(){
             		var dicType=Ext.getCmp('dicType');
             	...
             	scope:this,
             	iconCls:'btn-add',
             	hidden : isGranted('_DictionaryAdd')?false:true,
             	handler:function(){
					...
					new DictionaryForm({
                 	...
                 	scope:this,
                 	iconCls:'btn-del',
                 	hidden : isGranted('_DictionaryDelType')?false:true,
                 	handler:function(){
		...
		var tbar=new Ext.Toolbar({
			items:[{
					...
					handler:function(){
			            ...
			            if(selectedNode!=null){
			            	 parentId=selectedNode.id;
			            	 typeName=dicType.selectedNode.text;
			            }
			            if(parentId==0){
			            	Ext.ux.Toast.msg('操作信息','请从左选择字典分类');
			            	return;
			...
			'-',
				{
					...
					hidden : isGranted('_DictionaryDel')?false:true
				}),{
					text:'删除',
					iconCls:'btn-del',
					scope:this,
					...
					hidden : isGranted('_DictionaryChange')?false:true
				}),{
					text:'转移类型',
					iconCls:'',
					hidden : isGranted('_DictionaryChange')?false:true,
					...
					hidden : isGranted('_DictionaryEdit')?false:true
				}),{
					text:'编辑数据字典项',
					iconCls:'btn-edit',
					hidden : isGranted('_DictionaryEdit')?false:true,
