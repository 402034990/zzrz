...
public class TaskAction extends BaseAction{
	...
	public String all(){
		...
		/*if(tasks!=null&&tasks.size()!=0&&taskName!=null&&!"".equals(taskName)){
		...
		/*}else{
			setJsonString(gsonFormat(list, list==null?0:list.size()));
		}*/
		return SUCCESS;
	}
	...
	public String findByName(){
		String flowType=getRequest().getParameter("flowType");//流程key
		String taskSequence=getRequest().getParameter("taskSequence");//节点可以
		String type=getRequest().getParameter("type");//个人、企业
		String resource=getRequest().getParameter("resource");//区分线上、线下
		String busType=getRequest().getParameter("busType");//区分直投标、转让标
	...
	public String handler(){
		...
		if(userId!=null&&!"".equals(userId)&&!userId.equals("undefined")){
    ...
    public String display(){
		...
		PagingBean pb=new PagingBean(0, 17);//获取前七条数据
		//List<TaskInfo> tasks=flowTaskService.getTaskInfosByUserId(ContextUtil.getCurrentUserId().toString(),pb);
		List<TaskInfo> tasks=flowTaskService.getTasksByUserIdProcessNameTransfer(ContextUtil.getCurrentUserId().toString(), processName, pb,null,null);
    ...
    public void getCount(){
    	...
    	try{
    	    size=flowTaskService.getAllByUserIdProcessName(userId,processName);
    	}catch (Exception e) {
			e.printStackTrace();
		}
    ...
    public String userActivityAll(){
    	...
    	List<TaskInfo> tasks=flowTaskService.getTasksByUserIdProcessNameTransfer(userId, processName, pb,projectName,projectNumber);
    	if(tasks.size()>0){
    		for(int i=0;i<tasks.size();i++){
    			TaskInfo  taskInfo =tasks.get(i);
    			tasks.get(i).setTaskName(taskInfo.getTaskName());
    		}
    	}
    	setJsonString(gsonFormat(tasks, pb.getTotalItems()));
    	return SUCCESS;
     ...
     * @return
     */
    public String userActivityFirstAll(){
    	String userId = "";
    	if(assignId==null||"".equals(assignId)){
    		userId = ContextUtil.getCurrentUserId().toString();
    	}else{
    	...
    	PagingBean pb=new PagingBean(start, limit);//分页条件查询
    	List<TaskInfo> tasks=flowTaskService.getTasksByUserFirst(userId, processName, pb,projectName,projectNumber,"");
    	if(tasks.size()>0){
    		for(int i=0;i<tasks.size();i++){
    			TaskInfo  taskInfo =tasks.get(i);
    			tasks.get(i).setTaskName(taskInfo.getTaskName());
