/**
 * @author:
 * @class PersonBrowerRecord
 * @extends Ext.Panel
 * @description [PersonBrowerRecord]管理
 * @company 北京互融时代软件有限公司
 * @createtime:
 */
PersonBrowerRecord = Ext.extend(Ext.Panel, {
...
/*		if(typeof(_cfg.customerType)!="undefined"){
		...
		// 调用父类构造
		PersonBrowerRecord.superclass.constructor.call(this, {
			id : 'PersonBrowerRecord',
			title : '主借款人登记',
			region : 'center',
			layout : 'border',
	...
	initUIComponents : function() {
		...
		this.gridPanel = new HT.GridPanel( {
			...
			showPaging:true,
			autoScroll:true,
			url : __ctxPath + "/creditFlow/personBrowerProjectListCreditProject.do?oppositeType=person_customer",
			fields : [ {
				name : 'projectId',
				type : 'long'
...
/*		var businessType=this.searchPanel.getCmpByName('businessType');
	...
	outToExcel:function(){
		...
		window.open(__ctxPath + '/creditFlow/personBrowerOutExcelCreditProject.do?oppositeType=person_customer&businessType='+businessType+"&customerId="+customerId+"&startDate="+sdate+"&endDate="+edate,'_blank');
	}
