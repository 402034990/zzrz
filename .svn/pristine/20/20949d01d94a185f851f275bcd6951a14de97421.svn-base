...
public class ProcreditContractDaoImpl extends BaseDaoImpl<ProcreditContract> implements ProcreditContractDao{
	...
	public String getMaxContractNumber(){
		String hql = "SELECT MAX(contractNumber) FROM cs_procredit_contract";
		String contractNumber = (String) this.getHibernateTemplate().getSessionFactory().getCurrentSession().createSQLQuery(hql).uniqueResult();
