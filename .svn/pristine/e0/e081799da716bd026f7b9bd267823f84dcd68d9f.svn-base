...
public class PersonDaoImpl extends BaseDaoImpl<Person> implements PersonDao{
	...
	public void perList(PageBean<Person> pageBean,Map<String,String> map) {
		...
		StringBuffer hql=new StringBuffer("SELECT "
				...
				+"p.shopName,"
				+"p.age,"
				+"p.personNumber as personNumber,"
				+"o.org_name as companyName "
				+"from cs_person as p "
				+"left join  dictionary as d on p.sex=d.dicId "
				+"left join dictionary as d1 on d1.dicId=p.cardtype "
				+"left join organization as o on o.org_id=p.companyId where (p.isBlack=false or p.isBlack is null) ");
...
//		}
		...
		SQLQuery query=this.getSession().createSQLQuery(hql.toString())
				...
				.addScalar("personNumber",Hibernate.STRING);
	...
	public String findPersonNumber() {
		String sql="select MAX(personNumber) from cs_person";
	...
	@Override
	public String findPersonNumberById(Long personId) {
		String sql ="SELECT cardnumber FROM cs_person WHERE id = ?";
		return (String) this.getSession().createSQLQuery(sql).setParameter(0, personId).uniqueResult();
	}
}