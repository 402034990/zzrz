 ...
 * @author Jiang Wanyu
 */
 var SlProcreditGPSImageView = function(mark,readOnly,typeisfile,projId,businessType){
 	//alert(typeisfile);
    ...
    if(typeof(readOnly)!="undefined"){
 	...
 	var url=basepath();
 	url=url+"creditFlow/fileUploads/getAllFileListExtFileForm.do";
 	if(typeisfile !=null){
 	...
 	if(!isReadOnly){
			...
			'<tpl for=".">',
			    ...
			    '<div class="thumb">' +
			    '<img src="{[this.returnUrl(values.extendname,values.minCompressionFilePath)]}"  title="{filename}ddd"/>' +
			    '</div>',
			    '<span onclick="download1({[this.download(values.fileid)]})" style="text-decoration:underline;cursor:pointer;height:15px">下载</span><span   if="this.imgOrDir(type) == false" class="x-editable">{filename}</span></div>',
	        '</tpl>',
		 ...
		 returnUrl:function(extendname,webpath){
		  ...
		  download:function(v){
			...
			'<tpl for=".">',
	            '<div class="thumb-wrap" id="{fileid}">',
			    '<div class="thumb">' +
			    '<img src="{[this.returnUrl(values.extendname,values.webPath)]}"  title="{filename}ddd"/>' +
			    '</div>',
			    '<span onclick="download1({[this.download(values.fileid)]})" style="text-decoration:underline;cursor:pointer;height:15px">下载</span><span   if="this.imgOrDir(type) == false" class="">{filename}</span></div>',
	        '</tpl>',
	        '<div class="x-clear"></div>'
		,{
		 returnUrl:function(extendname,webpath){
