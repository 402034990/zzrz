...
public class BpFundIntentAction extends BaseAction {
	...
	public String list() {
		...
		if("0".equals(flag1)){
			...
			/*	String investIds = ownBpFundProject.getInvestorIds();
>>>>>>> .r25642
				if (investIds != null && investIds.length() != 0) {
					String[] ids = investIds.split(",");// 典当行只有一个投资人
					InvestEnterprise enter = investEnterpriseService.get(Long
							.parseLong(ids[0]));
					DecroratInvestPerson ip = new DecroratInvestPerson(enter);
					InvestPersonInfo person = ip.getInstantce();
					person.setInvestMoney(ownBpFundProject.getProjectMoney());
					List<FundIntent> filist = new ArrayList<FundIntent>();
					FundIntentGenerate sundIntentGeneraten = new FundIntentGenerate(
							ownBpFundProject, person, "yes");
					filist = sundIntentGeneraten.createInvesterFundIntent();
					list1.addAll(filist);
				}
...
/*				platFormBpFundProject.setYearFinanceServiceOfRate(new BigDecimal(0));
				...
				if (li != null && li.size() != 0) {
					for (InvestPersonInfo person : li) {
						...
						filist = sundIntentGeneraten.createInvesterFundIntent();
				/*		for(FundIntent l:filist){
							l.setInvestPersonId(person.getInvestId());
							
						}*/
						/*
						 * if(platFormBpFundProject.getAccrualtype().equals("singleInterest"
						 * )){ filist =FundIntentGenerate.getSingleInterest(
						 * platFormBpFundProject, person); }else
						 * if(platFormBpFundProject
						 * .getAccrualtype().equals("sameprincipalsameInterest"
						 * )){ filist =
						 * FundIntentGenerate.getSameprincipalsameInterest
						 * (platFormBpFundProject, person); }
						 */
						list1.addAll(filist);
					}
				}
				/*
				 * if(investors==null||"".equals(investors)){
				 * 
				 * }else{ String[] ids = investors.split(",");
				 * List<InvestPersonInfo> li = new
				 * ArrayList<InvestPersonInfo>(); for(int i=0;
				 * i<ids.length;i++){ if(null==ids[i]||ids[i].equals(""))
				 * continue; InvestPersonInfo personInfo =
				 * investPersonInfoService.get(Long.parseLong(ids[i]));
				 * li.add(personInfo); } list1 =
				 * fundIntentService.createList(platFormBpFundProject, project,
				 * li, null); }
				 */
			}
			if (list1 != null) {
