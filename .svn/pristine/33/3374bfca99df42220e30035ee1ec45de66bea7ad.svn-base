										...
										/*
										 ...
										 if(!this.ownerCt.isHiddenArchive && !this.ownerCt.isProjectInfoEdit){//项目保前归档节点并且不在项目信息编辑页面中
											 ...
											 loadData=function(size){    
												 ...
												 Ext.Ajax.request({
														url : __ctxPath + '/p2pMaterials/updateDataNumPlWebShowMaterials.do',
														method : 'POST',
														...
														params : {
															'plWebShowMaterials.datumNums' :size,
															'plWebShowMaterials.webMaterialsId' :grid.getStore().getAt(rowIndex).get("webMaterialsId")
														}
												    }) 
											 }
										 }
										 ...
										 uploadfile("上传材料",mark,uploadFileCounts,null,null,loadData);
									*/}
...
//										 });                  
			...
			createRs : function() {
				new P2pShowProcreditMaterialsForm({projectId : this.projId,operateObj:this.gridPanel,show:false,businessType:this.businessType,operationType:this.operationType,isDelete:false}).show();
			},
			updateRs : function(){
			    new P2pShowProcreditMaterialsForm({projectId : this.projId,operateObj:this.gridPanel,show:true,businessType:this.businessType,operationType:this.operationType,isDelete:true}).show(); //改回原来的  gao
			},
			imageView : function(){
