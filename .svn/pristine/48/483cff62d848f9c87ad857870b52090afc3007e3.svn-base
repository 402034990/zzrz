RelationPersonView = Ext.extend(Ext.Panel, {
	...
	initUIComponents : function() {
		...
		this.cModel_PanelReliation = new Ext.grid.ColumnModel([
				...
				header: "是否知晓",fixed: true,width:100,dataIndex: 'familyKnow',renderer : receiveRender,hidden:false,editor:
		...
		this.gridPanel = new Ext.grid.EditorGridPanel({
					...
					afteredit : function(e) {
										...
										if (e.originalValue != e.value) {//编辑行数据发生改变
											if(e.field == 'isKnow') {
													args = {'personRelation.isKnow': value,'personRelation.id': id};
												}
											Ext.Ajax.request({
												url : __ctxPath+'/creditFlow/customer/person/renewalRelationPerson.do',
												method : 'POST',
												scope : this,
