...
ReportFFLoanSumDetail = Ext.extend(Ext.Panel, {
	panel:null,
	url:'',
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ReportFFLoanSumDetail.superclass.constructor.call(this, {
			id : 'ReportFFLoanSumDetail'+this.reportKey,
			title : this.title,
			region : 'center',
			layout : 'anchor',
	...
	initUIComponents : function() {
			...
			items : [{ 
				...
				items : [ {
					...
					store : new Ext.data.SimpleStore({
						fields : ["name", "id"],
						data : [["个人", "person_customer"],["公司", "company_customer"]]
					})
				}] 
			},{
				columnWidth : .07,
		...
		this.url =  __ctxPath + '/system/getFFLoanSumDetail2LoanReport.do';
		
		pdfButton.on('click', function(pdfButton) {
