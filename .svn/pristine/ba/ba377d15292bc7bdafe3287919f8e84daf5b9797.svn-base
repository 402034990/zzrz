...
public class UserOrgDaoImpl extends BaseDaoImpl<UserOrg> implements UserOrgDao{
	...
	public List<UserOrg> getDepOrgsByUserId(Long userId){
		String hql="from UserOrg uo where uo.appUser.userId=? and (uo.organization.orgType=? or uo.organization.orgType=? or uo.organization.orgType=?)";
		return findByHql(hql,new Object[]{userId,Organization.ORG_TYPE_COMPANY,Organization.ORG_TYPE_DEPARTMENT,Organization.ORG_TYPE_ORTHER_ORG});
	}
