...
//				BigDecimal  principalMoney1=slFundIntentService.getPrincipalMoney1(projectId,businessType,earlyDate);
				if(project.getOwnJointMoney()!=null&&!"".equals(project.getOwnJointMoney())){
					money=project.getOwnJointMoney().subtract(new BigDecimal(earlyProjectMoney));
				}
				...
				if(null!=penaltyDays && !penaltyDays.equals("")){
					...
					buff.append(",earlyProjectMoney:"+earlyProjectMoney);
				}
			}
			buff.append("}");
			jsonString=buff.toString();
