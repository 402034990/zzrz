...
public class MortgageServiceImpl implements MortgageService{
	...
	public void deleteAllObjectDatas(Class entityClass,String mortgageIds) throws Exception {
		...
		try{
			...
			if(isDeleteAll){
				JsonUtil.jsonFromObject("删除反担保主表及附表信息成功!!!", true);
			}else{
				JsonUtil.jsonFromObject("删除反担保主表及附表信息失败!!!", false);
			}
		}catch(Exception e){
	...
	public ProcreditMortgage updateProcreditMortgage(int mortgageid,ProcreditMortgage procreditMortgage) {
		...
		try{
			...
			if(procreditMortgage.getIsGPS()!=null){
				projMort.setIsGPS(procreditMortgage.getIsGPS());//是否安装GPS
			}
	 ...
	 * @return
	 */
	public  List<ProcreditMortgageCar> getCarData(String projectNum,String  projectName ,String mortgageName,
			String danbaoType,String minMoney, String maxMoney,int status,int start,int limit){
		String objectName="";
		String proName="";
		List list=new ArrayList();
		String  sql = "select  cc.id , " +
				"( CASE cm.personType WHEN 602 THEN ( SELECT en.enterprisename AS enterprisename FROM cs_enterprise en WHERE ( en.id = cm.assureofname )) " +
				" WHEN 603 THEN ( SELECT p.name AS name FROM cs_person p WHERE ( p.id = cm.assureofname ))END  ) AS assureofnameEnterOrPerson," +
				" (case cm.mortgagenametypeid when 19 then '车辆车易贷(有合格证)' when 20 then '车辆车易贷(无合格证)' else '车辆' END ) AS mortgageTypeName,"+
				"cc.carManufacturer," +
				"cc.carStyle, " +
				" cc.carModel, "+
				"cm.finalprice," +
				"cm.assuremoney," +
				"cm.isunchain,cm.businessType, " +
				" cc.status," +
				"cc.isStorage," +
				"cc.storageNum ,cm.id   as  mortgageId,  " +
				"cc.isOut, " +
				"cc.outNum,  " +
				"cc.isHander, dic.itemValue AS assuretypeidValue, " +
				"cc.handerTime," +
				" sp.projectName as  projname," +
				"sp.projectId  as projid" +
				",sp.projectNumber  as  projnum " +
				"  from  cs_procredit_mortgage_car  as  cc " +
				" LEFT   JOIN  cs_procredit_mortgage as cm  ON cc.mortgageid = cm.id    " +
				" LEFT JOIN  sl_smallloan_project as sp  ON   sp.projectId = cm.projid  " +
				"  LEFT JOIN  dictionary  as  dic  on  dic.dicId =  cm.assuretypeid  where  1=1    "
			;
		if(projectNum!=null && !"".equals(projectNum)){
			sql = sql+ "  and  sp.projectNumber   LIKE  '%"+projectNum+"%'";
		}
		if(projectName!=null && !"".equals(projectName)){
			sql = sql+  "  and   sp.projectName  like   '%"+projectName+"%';";
		}
		if(mortgageName!=null  && !"".equals(mortgageName)){
			sql = sql+  "  and   cm.mortgagename  like   '%"+mortgageName+"%';";
		}
		if(danbaoType!=null  && !"".equals(danbaoType)){
			sql = sql+  "  and  cm.mortgagenametypeid ="+danbaoType;
		}
		if(null!=minMoney && !"".equals(minMoney) && null!=maxMoney && !"".equals(maxMoney)){
			sql=sql+" and cm.finalprice between "+Double.parseDouble(minMoney)+" and "+Double.parseDouble(maxMoney); 
		}
		sql=sql+" ORDER BY cc.id DESC ";
		if(status==0){
			sql = sql + " limit " +start+ ","+limit  ;
		}
		 return creditBaseDao.findSQL(sql)
		  .addScalar("id", Hibernate.INTEGER)
		  .addScalar("assureofnameEnterOrPerson", Hibernate.STRING)
		  .addScalar("mortgageTypeName", Hibernate.STRING)
		  .addScalar("carManufacturer", Hibernate.STRING)
		  .addScalar("carStyle", Hibernate.STRING)
		  .addScalar("carModel", Hibernate.STRING)
		  .addScalar("finalprice", Hibernate.DOUBLE)
		  .addScalar("assuremoney", Hibernate.DOUBLE)
		  .addScalar("isunchain", Hibernate.BOOLEAN)
		  .addScalar("status", Hibernate.SHORT)
		  .addScalar("isStorage", Hibernate.SHORT)
		  .addScalar("storageNum", Hibernate.STRING)
		  .addScalar("isOut", Hibernate.SHORT)
		  .addScalar("outNum", Hibernate.STRING)
		  .addScalar("isHander", Hibernate.SHORT)
		  .addScalar("handerTime", Hibernate.DATE)
		  .addScalar("projname", Hibernate.STRING)
		  .addScalar("projnum", Hibernate.STRING)
		  .addScalar("projid", Hibernate.LONG)
		  .addScalar("assuretypeidValue", Hibernate.STRING)
		  .addScalar("mortgageid", Hibernate.INTEGER)
		  .addScalar("businessType", Hibernate.STRING)
		 .setResultTransformer(Transformers.aliasToBean(ProcreditMortgageCar.class)).list();
	}
...
//				和流程状态无关抵质押物
//				String hql="from "+objectName+" v where (v.assuretypeid=604 or v.assuretypeid=605) and v.projectStatus=1 ";
				...
				hql=hql+" and (v.mortgagename" +
						" like '%"+mortgagename+"%' or v.mortgagename is null) and v.projname like '%"+projectName+"%' and v.projnum like '%"+projectNum+"%'";
	...
	public List getMortgageOfDZ(Long projectId, String businessType) {
		...
		if("SmallLoan".equals(businessType) || "ExhibitionBusiness".equals(businessType)||"BankCredit".equals(businessType)){
			objectName = "VProcreditDictionary";
		}else if("Guarantee".equals(businessType)){
			objectName = "VProcreditDictionaryGuarantee";
		}else if("Financing".equals(businessType)){
	...
	@Override
	public List getMortgageList(String businessType, String mortgagename,
			String projectName, String projectNum, String mortgageType,
			String danbaoType, String minMoney, String maxMoney, int start,
			int limit, int status, String mortgageStatus, String companyId,
			String type) {
		...
		try {
			...
			/*	if(null!=type && "car".equals(type)){
					hql=hql+" and v.typeid in (1,19,20)";
				}
				else{
					hql=hql+" and v.typeid not in (1,19,20)";
				}*/
				hql=hql+" order by v.id desc";
			    list = creditBaseDao.queryHql(hql, start, limit);
			    String counthql="select count(*) "+hql.toString();
	...
	@Override
	public VProcreditDictionary getVProcreditDictionaryByMortgageId(
			String mortgageId) {
		VProcreditDictionary vp=null;
		try{
			String hql="from VProcreditDictionary vp where vp.id = ? ";
			List<VProcreditDictionary> list=creditBaseDao.queryHql(hql, new Object[]{Integer.parseInt(mortgageId)});
			if(list != null && list.size() !=0){
				vp =(VProcreditDictionary) list.get(0);
			}
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	@Override
	public ProcreditMortgage getProjectId(Long projectId, String businessType,String mortgageId){
		//Object params[] = new Object[]{projectId,mortgageId} ;
		ProcreditMortgage procreditMortgage=null;
		String hql="from ProcreditMortgage as p where p.projid=? and p.businessType='"+businessType+"' and p.id='"+mortgageId+"'";
		try{
			List<ProcreditMortgage> list = creditBaseDao.queryHql(hql, new Object[]{projectId});
			if(list!=null && list.size()>0){
				procreditMortgage = (ProcreditMortgage)list.get(0);
			}
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	@Override
	public List<ProcreditMortgageCar> getByProjectIdCar(Long projetcId) {
		List list=new ArrayList();
		String  sql = "select  cc.id , " +
				"cc.modelName, " +
				"cc.carNumber, " +
				...
				"  from  cs_procredit_mortgage_car  as  cc " +
				" LEFT   JOIN  cs_procredit_mortgage as cm  ON cc.mortgageid = cm.id    " +
				" LEFT JOIN  sl_smallloan_project as sp  ON   sp.projectId = cm.projid  where sp.projectId="+projetcId
			;
		System.out.println("sql查询呢=============="+sql);
		 return creditBaseDao.findSQL(sql)
		  .addScalar("id", Hibernate.INTEGER)
		  .addScalar("engineNo", Hibernate.STRING)
		   .addScalar("carNumber", Hibernate.STRING)
		  .addScalar("modelName", Hibernate.STRING)
		  .addScalar("vin", Hibernate.STRING)
		  .addScalar("pledgenumber", Hibernate.STRING)
		 .setResultTransformer(Transformers.aliasToBean(ProcreditMortgageCar.class)).list();
	}
}
