...
DbRecommendPerformanceView = Ext.extend(Ext.Panel, {
	constructor : function(config) {
		Ext.applyIf(this, config);
		this.initUIComponents();
		DbRecommendPerformanceView.superclass.constructor.call(this, {
			id : 'DbRecommendPerformanceView',
			height : 450,
			autoScroll : true,
			layout : 'border',
			title : '推荐业绩列表',
	...
	initUIComponents : function() {
		...
		this.store = new Ext.data.JsonStore({
			url : __ctxPath+ '/system/dbuserListAppUser.do',
			...
			fields : [{name : 'userId'},{name:'fullname'},{name : 'plainpassword'},{name:'mobile'},{name:'email'},
					  ...
					  {name:'mmplanMoneyone'},{name:'mmplanMoneytwo'},{name:'registrationDate'},{name:'bidNumOne'},{name:'bidNumSecond'}]
		});
