...
var baseMortgageInfo = function(MortgageData,flag){
	...
	return new Ext.form.FieldSet({
	            ...
	            items : [{
					...
					items : [{
							xtype : 'label',
							html : MortgageData.vProcreditDictionary.businessType=='BankCredit'? '<b>授信编号：</b>': '<b>项目编号：</b>'
					},{
					...
					items : [{
							xtype : 'label',
							html : MortgageData.vProcreditDictionary.businessType=='BankCredit'?'<b>授信名称：</b>':'<b>项目名称：</b>'   
					},{
					...
					items : [{
							xtype : 'label',
							html : MortgageData.vProcreditDictionary.oppositeType=='person_customer' ?'<b>个人名称：</b>':'<b>企业名称：</b>'
					},{
					...
					items : [{
						...
						items : [{
							...
							html : '<b>市场价值获取方式：</b>' +MortgageData.vProcreditDictionary.valuationMechanism
						},{
						...
						items : [{
							...
							html : '<b>市场价值：</b>' + numTurnToString(MortgageData.vProcreditDictionary.finalCertificationPrice,8)
						},{
...
DZYMortgageView = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
		...
		if(typeof(_cfg.isGPSReadOnly)!="undefined"){
			this.isGPSReadOnly=_cfg.isGPSReadOnly
		}
		if(typeof(_cfg.delCarGps)!="undefined"){
			this.delCarGps=_cfg.delCarGps
		}
		if(typeof(_cfg.shifoubanliHidden)!="undefined"){
			this.shifoubanliHidden=_cfg.shifoubanliHidden
		}
		if(typeof(_cfg.isgpsAddEdi)!="undefined"){
			this.isgpsAddEdi=_cfg.isgpsAddEdi
		}
	...
	initUIComponents : function() {
		this.topbar = new Ext.Toolbar( {
			items : [{
				...
				iconCls : this.issee?"btn-readdocument":'btn-edit',
				text : 'GPS安装',
				xtype : 'button',
				scope : this,
				hidden : this.gpsAdd,
				handler : this.addCarGps
			},{
				iconCls : this.issee?"btn-readdocument":'btn-edit',
				text : 'GPS拆除',
				xtype : 'button',
				scope : this,
				hidden : this.delCarGps,
				handler : this.delCarGps
			}/*,{
				iconCls : this.issee?"btn-readdocument":'btn-edit',
				text : 'GPS安装查看',
				xtype : 'button',
				scope : this,
//				hidden : this.gpsAdd,
				handler : this.gpsInstallRegister
			}*/]
		});
...
//			isAllReadOnly:this.isAllReadOnly,
			...
			fields : [{
						name : 'id',
						type : 'int'
					}, 'mortgagename','mortgagepersontypeforvalue','assureofname', 'mortgagenametypeid',
			           'assuremoney','finalprice','finalCertificationPrice','remarks','assuretypeidValue','personTypeValue',
			           'mortgagepersontypeidValue','assuremodeidValue','enregisterDatumList','isGPS',
			           'pledgenumber','bargainNum','isAuditingPassValue','planCompleteDate','imei',
...
//				dataIndex : 'mortgagepersontypeforvalue'
				...
				renderer : function(v){
					if(v==''||v=='null'||v==null){
						...
						return Ext.util.Format.number(v,'0,000.00')+"万元"	
					}
				}
			}, {
				header : '市场价值',
			...
			listeners : {
				...
				afteredit : function(e) {
					...
					/*	if(e.field == 'transactDate') {//修改列为"办理时间"列
						...
						if(e.field == 'isGPS') {
							args= {
								'procreditMortgage.isGPS': e.value,
								'procreditMortgage.id': id,
								'procreditMortgage.businessType': businessType
							}
						}
...
//										        slSmallloanProject
				...
				}else if(this.formPanel.getCmpByName('projectMoney1')!=null){
					var  projectMoney = this.formPanel.getCmpByName('projectMoney1').getValue();
					new AddDzyMortgageWin({projectId:this.projectId,businessType:this.businessType,gridPanel:this.gridPanel,projectMoney:projectMoney}).show()
				}else{
					var projectMoney="";
					var slSuperviseRecordContinuationMoney= this.formPanel.getCmpByName('slSuperviseRecord.continuationMoney');
					if(slSuperviseRecordContinuationMoney  !=null || slSuperviseRecordContinuationMoney=="undefined"){
						projectMoney = slSuperviseRecordContinuationMoney.getValue();
					}
	...
	editMortgage : function(record) {
		...
		if (s <= 0) {
			...
			if(this.businessType=="SmallLoan"){
				...
				if(this.formPanel!=null&&this.formPanel!='undefined'){
					if(this.formPanel.getCmpByName('slSmallloanProject.projectMoney')!=null){
						projectMoney=this.formPanel.getCmpByName('slSmallloanProject.projectMoney').getValue()
					}else if(this.formPanel.getCmpByName('slSuperviseRecord.continuationMoney')!=null){
						projectMoney=this.formPanel.getCmpByName('slSuperviseRecord.continuationMoney').getValue()
					}
				}
				record=s[0]
	...
	seeMortgages : function(record) {
		...
		if (s <= 0) {
			...
			if(this.businessType=="SmallLoan"){
				...
				record=s[0]
				new UpdateDzyMortgageWin({id:record.data.id,projectId:this.projectId,businessType:this.businessType,gridPanel:this.gridPanel,type:record.data.typeid,projectMoney:projectMoney,isReadOnly:true}).show()
			}else if(this.businessType=="Guarantee"){
				record=s[0]
				new UpdateDzyMortgageWin({id:record.data.id,projectId:this.projectId,businessType:this.businessType,gridPanel:this.gridPanel,type:record.data.typeid}).show()
			}else if(this.businessType=="Financing"){
	...
	seeMortgage :function(record) {
		...
		var businessType = selected.get('businessType');
			...
			if(typeId==1){
				...
				seeQualifiedCarInfo(mortgageId,businessType);  //fixed ok by gao
			}else if(typeId==20){
				seeNoQualifiedCarInfo(mortgageId,businessType);  //fixed ok by gao
			}else if(typeId==2){
	...
	carDetail : function() {
			...
			if (null == selected) {
				...
				if(typeof(imei) =="undefined" || imei ==''){
					Ext.ux.Toast.msg('状态', 'IMEI不能为空!');
				}else{
					var carNumber = selected.get('carNumber');
					window.open( __ctxPath + '/mapDetail.do?imei='+imei+'&carLicenseNumber='+carNumber
						+'&mortgageid='+mortgageid+'&projectId='+this.projectId);
		/*			var win_save = new Ext.Window({
						title : '车辆GPS信息',
						resizable : false, // 变大小
						width : 1100,
						height : 700,
						...
						buttonAlign : 'center',
						html : '<iframe id="main"  width=1100 height=700 frameborder=0 scrolling=auto src="'
								+ __ctxPath + '/mapDetail.do?imei='+deviceId+'&carLicenseNumber='+carLicenseNumber+'"></iframe>',
						listeners:{ 
							maximize:function(){
							//var subWeb = document.frames ? document.frames["main"].document : ifm.contentDocument;   
							//if(ifm != null && subWeb != null) {
					  			// ifm.height = subWeb.body.scrollHeight;
								//window.frames["main"].dom.width=1200;
								//win_save.frames["main"].dom.height=1500;  
					  			 window.frames["main"].width="100%";  
			  					window.frames["main"].height="100%"; 
	    ...
	    // 添加车辆gps登记信息
	addCarGps : function() {debugger
		var  isAllReadOnly=this.isgpsAddEdi;
		var gpsAdd=this.gpsAdd;
		var selected = this.gridPanel.getSelectionModel().getSelected();
		if (null == selected) {
			Ext.ux.Toast.msg('状态', '请选择一条记录!');
		}else {
			var mortgageId = selected.get('id');
			var businessType=selected.get('businessType');
		    new ProcreditMortgageCarGpsForm ({
					    gp:this.gridPanel,
	...
	gpsInstallRegister : function(){
		...
		if (null == selected) {
			...
			if(this.businessType=="Financing"){
		     ...
		     new gpsInstallRegister({
			    mortgageId:mortgageId,
			    businessType:businessType,
			    isAllReadOnly:viewTypes,
			    gridPanel:grid
		    }).show()
