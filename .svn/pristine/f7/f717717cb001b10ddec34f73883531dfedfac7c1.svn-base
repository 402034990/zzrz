...
public class SlEarlyRepaymentRecordDaoImpl extends BaseDaoImpl<SlEarlyRepaymentRecord> implements SlEarlyRepaymentRecordDao{
	...
	public List allList(HttpServletRequest request, Integer start, Integer limit) {
		StringBuffer sb=new StringBuffer("SELECT "
				...
				+" LEFT JOIN (select sum(fi.incomeMoney) as incomeMoney,fi.slEarlyRepaymentId from bp_fund_intent as fi left join sl_earlyrepayment_record as se on se.slEarlyRepaymentId=fi.slEarlyRepaymentId where fi.fundType = 'loanInterest' and fi.slEarlyRepaymentId is not null) AS f on f.slEarlyRepaymentId=e.slEarlyRepaymentId"
				+" LEFT JOIN (select sum(fi.incomeMoney) as incomeMoney,fi.slEarlyRepaymentId from bp_fund_intent as fi left join sl_earlyrepayment_record as se on se.slEarlyRepaymentId=fi.slEarlyRepaymentId where fi.fundType = 'interestPenalty'  and fi.slEarlyRepaymentId is not null) AS f1 on f1.slEarlyRepaymentId=e.slEarlyRepaymentId"
				+" LEFT JOIN (select sum(fi.incomeMoney) as incomeMoney,fi.slEarlyRepaymentId from bp_fund_intent as fi left join sl_earlyrepayment_record as se on se.slEarlyRepaymentId=fi.slEarlyRepaymentId where fi.fundType = 'consultationMoney'  and fi.slEarlyRepaymentId is not null) AS f2 on f2.slEarlyRepaymentId=e.slEarlyRepaymentId"
				+" LEFT JOIN (select sum(fi.incomeMoney) as incomeMoney,fi.slEarlyRepaymentId from bp_fund_intent as fi left join sl_earlyrepayment_record as se on se.slEarlyRepaymentId=fi.slEarlyRepaymentId where fi.fundType = 'serviceMoney'  and fi.slEarlyRepaymentId is not null) as f3 on f3.slEarlyRepaymentId=e.slEarlyRepaymentId"
