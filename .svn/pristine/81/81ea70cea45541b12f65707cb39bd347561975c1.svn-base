...
function changeWestTitle(){
	var westTitle="导航";
	 ...
	 if(SystemType=="team"){
		...
		westTitle="导航-活期理财业务";
	 }else if(SystemType=="yun"){
		westTitle="导航-一元云购";
	 }else if(SystemType=="intermediary"){
		westTitle="导航-银行中介";
	 }
   ...
   //初始化系统类型   
	function changeSystem(type){
		//按照权限 修改头部导航 
			//客户中心
		/*if(!isSystemType('Mod_bank_intermediary')){
			try{
    	...
    	if(!isSystemType('Mod_erp')){
    		try{
    			...
    			Ext.get("DKManger").setStyle("display", "none");//贷款试算器
    		}catch (e) { }
    	}
    	//贷款管理//财富中心//积分夺宝
		...
		if(!isSystemType('Mod_customer')){
			try{
    	    	...
    	    	//积分夺宝
    	if(!isSystemType('Mod_yun')){
    		try{
    			Ext.get("yiYuanYunGou").setStyle("display", "none");
    		}catch (e) { }
    	}
		//loadAppHome("PersonalDesktop");
	    SystemType=type;
	    loadWestMenu(false);
	    changeWestTitle();
	    changeDesk();
...
/**
	...
	function changeDesk(){
		...
		//默认展示的桌面类型
		var loadItems="PersonalTeamDesktop";
  ...
  var menu = new Ext.menu.Menu();
   ...
   //  menu.add({id: 'menu5',text:'典当系统',handler:pawnHandler,iconCls:'menu-guarantee-info'});
    ...
    function reportManger(){
      ...
      //贷款计算器
    function LoanToolBox(){
    	 new LoanTrialForm({ 
    	 }).show();
    }
    //客户查询器
    function CustomerToolBox(){
    	 new CustomerToolForm({ 
    	 }).show();
    }
          //客户中心
    function customerManger(){
    	if(!isSystemType('Mod_customer')){
    		Ext.Msg.alert('提示', "抱歉，您没有此权限。");
    		return;
    	}
...
App.clickTopTab=function(id,params,precall,callback,activeid){
	...
	}else if("CreateNewProjectFroms"==id){
		$ImportJs('CreateNewProjectFroms', function() {
			new CreateNewProjectFroms({operationType:params.operationType,history:params.history}).show();
		}, params);
	}else if("P2pAddProduct"==id){
		$ImportJs('P2pAddProduct', function() {
			new P2pAddProduct().show();
		}, null);
	}else if("businessManagementFrom"==id){
		$ImportJs('businessManagementFrom', function() {
			new businessManagementFrom({}).show();
		}, params);
	}else if("enterpriseFileManagement"==id){//合作企业档案管理
		$ImportJs('enterpriseFileManagement', function() {
			new enterpriseFileManagement({}).show();
		}, params);
	}else if("telephoneNO"==id){//手机号未检测到
		$ImportJs('telephoneNO', function() {
			new telephoneNO().show();
		}, null);
	}else if("quickPerson"==id){//客户款式查询
		$ImportJs('quickPerson', function() {
			new quickPerson().show();
		}, null);
