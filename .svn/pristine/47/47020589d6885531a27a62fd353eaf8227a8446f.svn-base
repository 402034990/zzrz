...
public class SlSmallloanProjectServiceImpl extends
	...
	@Resource
	private OrganizationDao organizationDao;//车产信息
	@Resource
	...
	// 结清流程
	@Override
	public String stateRbjrSettlementFlow( Long  projectId,String processType){
		String customerName = "";
		String str = "";
	...
	/*	if (isGroupVersion != null && Boolean.valueOf(isGroupVersion)) {
			pdf = proDefinitionDao.getByProcessName("rbjrSettlementFlow"
					+ org.getKey());
		} else {
		}*/
		if(processType!=null&&!"".equals(processType)){
			pdf = proDefinitionDao.getByProcessName(processType);
			...
			if (run != null && run.getPiId() != null) {  // by 2017 1.12 liu
...
//								List<BpFundProject> fundList= bpFundProjectDao.getByProjectId(Long.valueOf(projectId));
	...
	//启动资金匹配流程
	@Override
	public String startMatchingFunds(Long bidPlanId,String flowType){
		...
		newVars.put("flowType",project.getFlowType());//合同分类会用到
		newFlowRunInfo.getVariables().putAll(newVars);
		newFlowRunInfo.setBusMap(newVars);
		newFlowRunInfo.setDefId(String.valueOf(pdf.getDefId()));
		//招标项目的名字
	...
	public String startRenewalProcess(Long projectId) {
		...
		BpFundProject project = this.bpFundProjectDao.getByProjectId(projectId,Short.valueOf("0"));
		...
		slSuperviseRecord.setIsActualDay(project.getIsActualDay());
	...
	public Integer saveSmallProjectInfoNextStep(FlowRunInfo flowRunInfo) {
		try {
			if (flowRunInfo.isBack()) {
				...
				String projectId=flowRunInfo.getRequest().getParameter("slSmallloanProject.projectId");
				String projectState=flowRunInfo.getRequest().getParameter("slSmallloanProject.projectState");
				String loanId=flowRunInfo.getRequest().getParameter("loanId");
				String flag=flowRunInfo.getRequest().getParameter("flag");
				String transitionName = flowRunInfo.getTransitionName();
				/*//授信信息
				BankCredit  bank = new BankCredit();
				BeanUtil.populateEntity(flowRunInfo.getRequest(), bank, "bankCredit");*/
			/*	//保证金
				CashDeposit  cd = new CashDeposit();
				BeanUtil.populateEntity(flowRunInfo.getRequest(), cd, "cashDeposit");*/
				
				SlSmallloanProject project=dao.get(Long.valueOf(projectId));
				if (transitionName != null && !"".equals(transitionName)) {
					Map<String, Object> vars = new HashMap<String, Object>();
					if (transitionName.contains("结束1")) {  // 睿本车贷 只有结束1
						flowRunInfo.setStop(true);
						project.setProjectStatus(Constants.PROJECT_STATUS_STOP);
						dao.merge(project);
						Short i=0;
						// 提前终止查询
						BpFundProject bp=bpFundProjectService.getByProjectId(Long.valueOf(projectId), i);
						bp.setProjectStatus(Constants.PROJECT_STATUS_STOP);
						bpFundProjectService.merge(bp);
					}else if (transitionName.contains("结束2")) {
						PlProjectArchives pa=new PlProjectArchives();
						BeanUtil.populateEntity(flowRunInfo.getRequest(), pa, "plProjectArchives");
						if(pa.getProjtoarchiveId()==null){
							pa.setBusinessType(project.getBusinessType());
							pa.setProjectId(project.getProjectId());
							plProjectArchivesDao.save(pa);
						}else{
							PlProjectArchives orgPlProjectArchives=plProjectArchivesDao.get(pa.getProjtoarchiveId());
							try{
								BeanUtil.copyNotNullProperties(orgPlProjectArchives, pa);
								plProjectArchivesDao.merge(orgPlProjectArchives);
							}catch(Exception ex){
								logger.error(ex.getMessage());
							}
						}
						...
						if (currentForm != null) {
							...
							if (!isToNextTask) {// true表示流程正常往下流转,false则表示打回。
								...
								SlSmallloanProject slSmallloanProject=new SlSmallloanProject();
								BeanUtil.populateEntity(flowRunInfo.getRequest(), slSmallloanProject, "slSmallloanProject");
								project=dao.get(slSmallloanProject.getProjectId());
								BeanUtil.copyNotNullProperties(project, slSmallloanProject);
								
//								project.setBankCreditId(bank.getId());//保存授信表的主键
//								project.setBaozhengjinId(cd.getId());//保存保证金表的主键
								//1.修改银行卡信息
								if(enterpriseBank!=null&&enterpriseBank.getId()!=null){
									...
									enterpriseBank.setIscredit(Short.valueOf("0"));
									if("person_customer".equals(project.getOppositeType())){
										enterpriseBank.setEnterpriseid(p1.getId());
										enterpriseBank.setIsEnterprise(Short.valueOf("1"));
									}else{
										enterpriseBank.setIsEnterprise(Short.valueOf("0"));
										Enterprise e=new Enterprise();
										BeanUtil.populateEntity(flowRunInfo.getRequest(), e, "enterprise");
										enterpriseBank.setEnterpriseid(e.getId());
									}
									enterpriseBank.setIsInvest(Short.valueOf("0"));
								...
								BeanUtil.populateEntity(flowRunInfo.getRequest(), workCompany, "workCompany");
								if(null!=thisTaskName && thisTaskName.equals("saveInfo")){
									personDao.savePersonInfo(p1, spouse, ((null!=thisTaskName && thisTaskName.equals("saveInfo"))?workCompany:null));
								}
								
								//3.保存费用信息
								String chargeJson=flowRunInfo.getRequest().getParameter("chargeJson");
								if(null!=chargeJson && !chargeJson.equals("")){
									slActualToChargeService.savejson(chargeJson, project.getProjectId(), project.getBusinessType(), Short.valueOf("0"), project.getCompanyId());
								}
							...
							/*	//4.保存共同借款人
								...
								/*	if(projectState.equals("6")){
										fundProject.setProjectStatus(Short.valueOf(projectState));
									}*/
									bpFundProjectDao.merge(fundProject);
									// 5.借款需求
									if(projectId!=null&&!"".equals(projectId)){
										BpMoneyBorrowDemand  bpMoneyBorrowDemand=new BpMoneyBorrowDemand();
										BpMoneyBorrowDemand	bb=bpMoneyBorrowDemandService.getByProjectId(Long.valueOf(projectId));
										if(bb!=null){
											bpMoneyBorrowDemand=bb;
										}
										BeanUtil.populateEntity(flowRunInfo.getRequest(), bpMoneyBorrowDemand, "bpMoneyBorrowDemand");
										bpMoneyBorrowDemand.setProjectID(slSmallloanProject.getProjectId());
										if (bpMoneyBorrowDemand.getBorrowid() == null) {
								...
								if(null!=thisTaskName && thisTaskName.equals("fkfh")){
									BpFundProject fundProject=bpFundProjectDao.getByProjectId(Long.valueOf(projectId),Short.parseShort("0"));
									if(null==fundProject){
										fundProject=new BpFundProject();
									}
								...
								if(null!=thisTaskName && thisTaskName.equals("fkqr")){
									BpFundProject ownBpFundProject=new BpFundProject();
									BeanUtil.populateEntity(flowRunInfo.getRequest(), ownBpFundProject, "slSmallloanProject");
									// 自有资金款项信息
									...
									if (null != ownFund) {
										...
										ownFund.setProjectStatus(Short.valueOf("1"));
										ownFund.setOwnJointMoney(project.getProjectMoneyPass());
										bpFundProjectDao.merge(ownFund);
									}
									
									project.setProjectStatus(Short.valueOf("1"));
									dao.merge(project);
									
									List<SlFundIntent> fundList = slFundIntentService.getByProjectId(project.getProjectId(),project.getBusinessType());
									if(null != fundList && fundList.size() > 0){
										for(SlFundIntent fund : fundList){
											fund.setIsCheck(Short.valueOf("0"));
										}
									}
								}
								if(null!=thisTaskName && thisTaskName.equals("czzy")){
									//授信信息操作
								/*	//添加授信使用记录信息
									if(null!=bank.getId() && !"".equals(bank.getId())){
										BankCredit  bcr = 	bankCreditService.get(bank.getId());
										BankCreditUserLog  bcu =  new BankCreditUserLog();
										if(project.getBankCreditId()!=null  && !"".equals(project.getBankCreditId())){//如果使用授信金额
											bcu.setProjectId(project.getProjectId());//保存主键
											bcu.setProjectName(project.getProjectName());//项目名称
											bcu.setProjectNum(project.getProjectNumber());//项目编号
											bcu.setCreditType(Short.valueOf("2"));//授信类型主键
											bcu.setCreditMoney(project.getProjectMoneyPass());//设置使用授信金额
											bcu.setCommerType(bcr.getOppositeType());//保存客户类型
											bcu.setCommerId(bcr.getOppositeID());//保存客户主键
											bcu.setStatus(Short.valueOf("1"));//设置占用中
											bcu.setCreatetime(new Date());//设置创建时间
											bcu.setBankCreditId(project.getBankCreditId());
											bankCreditUserLogService.save(bcu);
											BigDecimal   aa = bcr.getShengyuMoney();
											BigDecimal  bb = aa.subtract(project.getProjectMoneyPass());
											BigDecimal cc  = bcr.getUseMoney();
											if(null==cc){
												cc=new BigDecimal(0);
											}
											BigDecimal  dd = cc.add(project.getProjectMoneyPass());
											bcr.setShengyuMoney(bb);
											bcr.setUseMoney(dd);
											bankCreditService.merge(bcr);
										}
									}*/
								...
								/*	//添加保证金使用记录====新添加的
									if(project.getBaozhengjinId()!=null && !"".equals(project.getBaozhengjinId())){
										...
										CashDeposit  cdp = 	cashetService.get(cd.getId());
										
										//计算保证金
										BigDecimal zhanyongMoney = cdp.getCashzhanyong();//保证金占用金额
										if(null==zhanyongMoney){
											zhanyongMoney=new BigDecimal(0);
										}
										project.setCashrate(cdp.getCashrate());
										BigDecimal useMoney = cdp.getCashshengyu();//保证金剩余金额
										BigDecimal aaa = project.getProjectMoneyPass().multiply( project.getCashrate()).divide(new BigDecimal("100"));
										//保存保证金使用纪录
										CashDetail  detail = new CashDetail();
										detail.setCashdepositid(cdp.getId());//保证金表的主键
										detail.setCreateDate(new Date());//创建时间
										detail.setCashType(3);//设置操作类型为占用
										detail.setProjectId(project.getProjectId());//保存项目表主键
										detail.setProjectName(project.getProjectName());
										detail.setProjectNum(project.getProjectNumber());
										detail.setState(1);
										detail.setCashmoney(aaa);
										detail.setDeclibtion("放款占用");
										detail.setOprateName(currentUser.getFullname());//操作人
										detail.setOprateNameId(currentUser.getUserId());//操作人id
										cashdetailService.save(detail);
										//更新保证金数据信息
										cdp.setCashzhanyong(zhanyongMoney.add(aaa));//设置占用金额
										cdp.setCashshengyu(useMoney.subtract(aaa));//设置剩余金额
										cashetService.merge(cdp);
									}*/
									...
									BpFundProject ownFund = bpFundProjectService.getByProjectId(project.getProjectId(), Short.valueOf("0"));
									if (null != ownFund) {
										if(projectState!=null&&!"".equals(projectState)){
											if(projectState.equals("6")){  // 项目归档到完成
												ownFund.setProjectStatus(Short.valueOf(projectState));
											}
										}
										BeanUtil.copyNotNullProperties(ownFund, ownBpFundProject);
										ownFund.setOwnJointMoney(project.getProjectMoneyPass());
								...
								/*	if(projectState!=null&&!"".equals(projectState)){
									...
									if (null != fundIntentJsonData && !"".equals(fundIntentJsonData)) {
										if(ownFund!=null){
											List<SlFundIntent> oldList = slFundIntentService.getByProjectId(project.getProjectId(),"SmallLoan");
											for (SlFundIntent sfi : oldList) {
												 if(sfi.getAfterMoney()!=null&&!"".equals(sfi.getAfterMoney())){
													 if (sfi.getAfterMoney().compareTo(new BigDecimal(0)) == 0) {
														 slFundIntentService.remove(sfi);
													 }
												 }
										...
										for (int i = 0; i < shareequityArr.length; i++) {
											...
											//获得宽限日
											Date newEnd=DateUtil.addDaysToDate(SlFundIntent1.getIntentDate(),AppUtil.getGraceDayNum());
									...
									if(projectState!=null&&!"".equals(projectState)){
										project.setProjectStatus(Short.valueOf(projectState));
									}
									dao.merge(project);
								}
							}
	...
	@Override
	public Integer saveZZYYSmallProjectInfoNextStep(FlowRunInfo flowRunInfo) {
		try {
			if (flowRunInfo.isBack()) {
				return 1;
			} else {
...
//								project.setBaozhengjinId(cd.getId());//保存保证金表的主键
								...
								if(enterpriseBank!=null){
									if (enterpriseBank.getId() == null || enterpriseBank.getId() == 0) {/*
										...
										enterpriseBank.setIsInvest(Short.valueOf("0"));
									 */} else {
										 EnterpriseBank bank1 = (EnterpriseBank) creditBaseDao.getById(
												 EnterpriseBank.class, enterpriseBank.getId());
										 bank1.setAccountnum(enterpriseBank.getAccountnum());
										 bank1.setAccountType(enterpriseBank.getAccountType());
										 bank1.setBankid(enterpriseBank.getBankid());
										 bank1.setBankname(enterpriseBank.getBankname());
										 bank1.setName(enterpriseBank.getName());
										 bank1.setOpenType(enterpriseBank.getOpenType());
										 bank1.setAreaId(enterpriseBank.getAreaId());
										 bank1.setAreaName(enterpriseBank.getAreaName());
										 bank1.setBankOutletsName(enterpriseBank.getBankOutletsName());
										 creditBaseDao.updateDatas(bank1);
										 project.setBankId(Long.valueOf(enterpriseBank.getId().toString()));
										 dao.merge(project);
									 }
								}
	 ...
	 * @return
	 */
	public  String  saveOrUpdateBankCreditUserLog(Long projectId,String projectName,
			String  projectNum, BigDecimal creditMoney,
			Long bankCreditId,Short  isNomalOver){
		BankCredit bankCredit =	bankCreditService.get(bankCreditId);
		//项目主键  客户 类型  客户主键   授信类型  授信主键
		List<BankCreditUserLog> list =   bankCreditUserLogService.getListBankCreditUserLog(bankCredit.getOppositeType(), bankCredit.getOppositeID(),
				bankCreditId, bankCredit.getCeditTypeId(), projectId);
		System.out.println("list.size(0====="+list.size());
		if(list.size()>0){//当流程进行完成时，进行更新操作
			BankCreditUserLog   bc  = new BankCreditUserLog();
			bc = list.get(0);
			if(isNomalOver==0){//非正常结束，授信记录表中剩余金额添加，已使用金额减少，更改授信使用记录中的状态（失败）
				bc.setStatus(Short.valueOf("3"));
				bankCredit.setShengyuMoney(bankCredit.getShengyuMoney().add(creditMoney));//剩余金额进行添加
				bankCredit.setUseMoney(bankCredit.getUseMoney().subtract(creditMoney));//使用金额进行添加
				bankCreditUserLogService.merge(bc);
				bankCreditService.merge(bankCredit);
			}else{//正常结束，只用更新授信流程使用记录的中的状态
			}
		}else{
			//进行保存操作
			BankCreditUserLog   bc  = new BankCreditUserLog();
			...
			bc.setProjectNum(projectNum);
			bc.setCommerType(bankCredit.getOppositeType());
			bc.setCreditMoney(creditMoney);
			bc.setCommerId(bankCredit.getOppositeID());
			bc.setStatus(Short.valueOf("2"));
			bc.setCreatetime(new  Date());
			bc.setBankCreditId(bankCreditId);
			bc.setCreditType(bankCredit.getCeditTypeId());
			bankCreditUserLogService.save(bc);
			System.out.println("要保存的金额=============="+creditMoney);
			bankCredit.setShengyuMoney(bankCredit.getShengyuMoney().subtract(creditMoney));
			System.out.println("数据库中存在的金额=============="+bankCredit.getUseMoney());
			if(bankCredit.getUseMoney()!=null &&!"".equals(bankCredit.getUseMoney())){
				bankCredit.setUseMoney(bankCredit.getUseMoney().add(creditMoney));
			}else{
				bankCredit.setUseMoney(creditMoney);
			}
			System.out.println("数据库需要更新的数据============="+bankCredit.getUseMoney());
			bankCreditService.merge(bankCredit);
		}
		return  "";
	}
	...
	@Override
	//个人快速审批流程
	public Integer saveFastApprovalSmallProjectInfoNextStep(FlowRunInfo flowRunInfo) {
		try {
			if (flowRunInfo.isBack()) {
				return 1;
			} else {
				...
				if (transitionName != null && !"".equals(transitionName)) {
					...
					if (transitionName.contains("终止") || transitionName.contains("结束")) {
						...
						dao.merge(project);
						List<BpFundProject>  list  = bpFundProjectService.getProject(Long.valueOf(projectId), "");
						if(list!=null && list.size()>0){
							for(int  i=0;i<list.size();i++){
								BpFundProject bp =	list.get(i);
//								bp.setFlag(Short.valueOf("0"));
								bp.setProjectStatus(Short.valueOf("3"));
								bpFundProjectService.merge(bp);
							}
						}
...
//										project.setProjectStatus(Short.valueOf("2"));
	...
	//企业快速审批流程
	public Integer saveEnterpriseFastProjectInfoNextStep(FlowRunInfo flowRunInfo) {
		try {
			if (flowRunInfo.isBack()) {
				return 1;
			} else {
...
//										project.setProjectStatus(Short.valueOf("2"));
	...
	//个人贷款标准流程
	@Override
	public Integer savePersonCreditSmallProjectInfoNextStep(FlowRunInfo flowRunInfo){
		try {
			if (flowRunInfo.isBack()) {
				return 1;
			} else {
				...
				if (transitionName != null && !"".equals(transitionName)) {
					...
					if (transitionName.contains("终止") || transitionName.contains("结束")) {
						...
						dao.merge(project);
						//更新项目表的状态信息---结束
						//更新不bp_fund_project项目表的状态信息---开始
						System.out.println("项目主键-------------"+projectId);
...
//								bp.setFlag(Short.valueOf("0"));
						...
						if (currentForm != null) {
							...
							if (!isToNextTask) {// true表示流程正常往下流转,false则表示打回。
								...
								//放款确认
								if(null!=thisTaskName && thisTaskName.equals("fangkuanqueren")){
									project.setProjectStatus(Short.valueOf("1"));
								}
								dao.merge(project);
							}
	...
	@Override
	public List<SlSmallloanProject> customerStatistical() {
		// TODO Auto-generated method stub
		return dao.customerStatistical();
	}
}