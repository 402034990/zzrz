 ...
 * */
	...
	public FundAlterAccrualFundIntentCreate(BpFundProject project,SlAlterAccrualRecord slAlterAccrualRecord,Integer payintentPeriod,Date afterDate,Date beforeDate,BigDecimal principalMoney,SlFundIntent bf){
		 ...
		 * yes按实际天数计算，no按30天计算
		 */
		this.isActualDay=project.getIsActualDay();
		/**
		 *  算头不算尾0 算头又算尾1 AppUtil.getInterest()
		 */
		this.isccalculateFirstAndEnd=AppUtil.getInterest(); 
		this.payintentPeriod=payintentPeriod;
		this.slFundIntent=bf;
		this.projectMoney=(project.getOwnJointMoney()==null ? project.getProjectMoney(): project.getOwnJointMoney()).subtract(principalMoney); 
	}  
   ...
   public void create(List list){
		...
		if(isPreposePayAccrual==0){
			...
			SlFundIntent s=calculslfundintent(this.ProjectLoadAccrual,this.alelrtDate,beforeDate,afterDate,BigDecimal.valueOf(0),money1.add(money2).subtract(allMoney),payintentPeriod);
