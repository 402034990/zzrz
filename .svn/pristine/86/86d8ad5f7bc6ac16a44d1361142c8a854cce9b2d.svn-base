...
public class CreditRatingDaoImpl extends BaseDaoImpl<CreditRating> implements CreditRatingDao {
	...
	public List getRatingOptionListOrder(int id) {
		List<RatingOption> list = null;
		String hql = "from RatingOption ro where ro.indicatorId=? order by ro.optionName+1 desc";
		try {
			list = getHibernateTemplate().find(hql,id);
		} catch (Exception e) {
			e.printStackTrace();
	...
	@Override
	public CreditRating getCreditRating(String projectId) {
		String hql = "from CreditRating cr where cr.projectId = ? ";
		List list=getSession().createQuery(hql).setParameter(0, projectId).list();
