...
public class VProcreditContractServiceImpl extends
			...
			if (id != null) {
				...
				if (vprocreditContract.getId() != null) {
					if (vprocreditContract.getId() != 0) {
						...
						//if (clist != null && clist.size() > 0) {
							//List<FileForm> list = (List<FileForm>) clist;
						List<FileForm> fileForms = fileFormDao.getFileList(mark);
							for (int i = 0; i < fileForms.size(); i++) {
								FileForm f = fileForms.get(i);
								// 删除附件文件
	...
	public List<VProcreditContract> listByHtType(String businessType,
		...
		if(businessType!=null &&!"".equals(businessType)){
			if(businessType.equals("BankCredit")){
				return dao.listByHtType(businessType,projId, htType, type);
			}else{
				BpFundProject bpFundProject =bpFundProjectDao.getByProjectId(Long.valueOf(projId), Short.valueOf("0"));
				if(bpFundProject!=null){
					return dao.listByHtType(businessType, bpFundProject.getProjectId().toString(), htType, type);
				}else{
					return null;
				}
			}
	...
	@Override
	public List<VProcreditContract> listContractByHtType(String businessType,String projId, String htType,String type){
		if(businessType!=null &&!"".equals(businessType)){
			if(businessType.equals("BankCredit")){
				return dao.listByHtType(businessType,projId, htType, type);
			}else{
