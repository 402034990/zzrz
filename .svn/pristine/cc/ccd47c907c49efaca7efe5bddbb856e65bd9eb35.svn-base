/**
 * @author:
 * @class PersonAssureRecord
 * @extends Ext.Panel
 * @description [PersonAssureRecord]管理
 * @company 北京互融时代软件有限公司
 * @createtime:
 */
PersonAssureRecord = Ext.extend(Ext.Panel, {
...
/*		if(typeof(_cfg.customerType)!="undefined"){
		...
		// 调用父类构造
		PersonAssureRecord.superclass.constructor.call(this, {
			id : 'PersonAssureRecord',
			title : '保证人登记',
			region : 'center',
			layout : 'border',
	...
	initUIComponents : function() {
		...
		this.gridPanel = new HT.GridPanel( {
			...
			showPaging:true,
			autoScroll:true,
			url : __ctxPath + "/creditFlow/personAssureProjectListCreditProject.do?oppositeType=person_customer",
			fields : [ {
				name : 'projectId',
				type : 'long'
...
/*		var businessType=this.searchPanel.getCmpByName('businessType');
	...
	outToExcel:function(){
		...
		window.open(__ctxPath + '/creditFlow/personAssureOutExcelCreditProject.do?oppositeType=person_customer&businessType='+businessType+"&customerId="+customerId+"&startDate="+sdate+"&endDate="+edate,'_blank');
	}
