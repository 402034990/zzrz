...
public class TaskServiceImpl extends BaseServiceImpl<TaskImpl> implements TaskService{
	...
	protected List<TaskInfo> constructTaskInfos(List<TaskImpl> taskImpls){
		...
		for(TaskImpl taskImpl:taskImpls){
			...
			if(processRun!=null){
				taskInfo.setTaskName(processRun.getSubject() + "-"+processRun.getProDefinition().getName()+"-"+ taskImpl.getActivityName());
...
//		}
	 ...
	 * add by lu 2012.03.01
	 */
	public List<TaskInfo> getTasksByUserFirst(String userId,String processName,PagingBean pb,String projectName,String projectNumber,String activityName){
		List<TaskImpl> list=getTasksByUserFirstInfo(userId,processName,pb,projectName,projectNumber,activityName);
		return constructTaskInfos(list);
	}
	 ...
	 * add by lu 2012.03.01
	 */
	public List<TaskImpl> getTasksByUserFirstInfo(String userId,String processName,PagingBean pb,String projectName,String projectNumber,String activityName){
		return dao.getTasksByUserFirst(userId, processName, pb,projectName,projectNumber,activityName);
	}
}
