		...
		if(personInfo.getRebateType()!=null&&personInfo.getRebateWay()!=null){
			if(personInfo.getRebateWay()==1||personInfo.getRebateWay()==3){//立返,到期
				...
				if(personInfo.getRebateType()==1){//返现
					...
					couponInterest = calculBpFundIntent(CouponInterest,DateUtil.addDaysToDate(startDate, -1),manyStartDate,DateUtil.addDaysToDate(startDate, -1),BigDecimal.valueOf(0),  projectMoney.multiply(personInfo.getSubjoinInterest()).multiply(days),startPeriod);
				}else if(personInfo.getRebateType()==0){//普通加息
					couponInterest = calculBpFundIntent(CouponInterest,DateUtil.addDaysToDate(startDate, -1),manyStartDate,DateUtil.addDaysToDate(startDate, -1),BigDecimal.valueOf(0),  projectMoney.multiply(personInfo.getSubjoinInterest()).multiply(days),startPeriod);
				}
			}
