	...
	initUIComponents : function() {
		...
		editqlidecheckRs : function() {
			...
			if (s <= 0) {
				...
				var penaltyMoney = this.data.punishMoney;//重新计算后的罚息
				var overdueMoney = this.data.overdureMoney;//重新计算后的逾期
				//修改款项未对账金额
				notSynthesizeMoney=notSynthesizeMoney-(penaltyMoney+overdueMoney);
				...
				Ext.Ajax.request({
					...
					success : function(response, request) {
						...
						var penaltyMoney=alarm.data.penaltyMoney;
						var overdueMoney=alarm.data.overdueMoney;
						notSynthesizeMoney+=(penaltyMoney+overdueMoney);
