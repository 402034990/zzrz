 ...
 */
ReportPlBidFullDate = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ReportPlBidFullDate.superclass.constructor.call(this, {
			id : 'ReportPlBidFullDate'+this.reportKey,
			title : this.title,
			region : 'center',
			layout : 'anchor',
			iconCls : 'menu-report',
			...
			listeners : {
				'afterrender' : function(v) {
					...
					Ext.getCmp('reportSearchPanel'+this.reportKey).getForm().submit({
						waitMsg : '正在提交查询',
						url : __ctxPath + '/system/getReportPlBidFullDateInternetReport.do?reportKey='+reportKey,
						method : 'post',
