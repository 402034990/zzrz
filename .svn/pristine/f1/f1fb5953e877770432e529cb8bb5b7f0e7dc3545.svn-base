...
ProDefinitionSettingForCustomer=Ext.extend(Ext.Panel,{
	
	constructor:function(conf){
		Ext.applyIf(this,conf);
		this.initUIs();
		ProDefinitionSettingForCustomer.superclass.constructor.call(this,{
			id : 'ProDefinitionSettingForCustomer'+this.defId,
...
/**
  ...
  * @return
  */
 ProDefinitionSettingForCustomer.prototype.usersPanel=function(defId){
	...
	var grid = new Ext.grid.EditorGridPanel({
				...
				columns : [new Ext.grid.RowNumberer(),{
							...
							renderer : function(value,metadata,record,rowIndex,colIndex){
								...
								return value == 1 ? '<button class="btn-setting" title ="会签设置" onclick="ProDefinitionSettingForCustomer.setTaskAssign('+assignId+',\''+av+'\')"/>':'';
							}
						},{
							header : '时限类型',
...
*/};
  ...
  * @param {} defId
  */
 ProDefinitionSettingForCustomer.roleSelect=function(rowIndex,colIndex,defId){
 ...
 */
 ProDefinitionSettingForCustomer.setTaskAssign = function(assignId,av){
	 new TaskSignForm({
		 assignId : assignId,
		 activityName : av
		}).show();
