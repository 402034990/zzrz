 ...
 */
ReportPlBidOrIntentExpire = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ReportPlBidOrIntentExpire.superclass.constructor.call(this, {
			id : 'ReportPlBidOrIntentExpire'+this.reportKey,
			title : this.title,
			region : 'center',
			layout : 'anchor',
			iconCls : 'menu-report',
			...
			listeners : {
				'afterrender' : function(v) {
					...
					Ext.getCmp('reportSearchPanel'+this.reportKey).getForm().submit({
						waitMsg : '正在提交查询',
						url : __ctxPath + '/system/orIntentExpireInternetReport.do?reportKey='+reportKey,
						method : 'post',
