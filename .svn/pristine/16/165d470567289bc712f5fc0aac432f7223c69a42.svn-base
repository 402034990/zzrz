...
public class SlFundIntentAction extends BaseAction {
	...
	public String cancelAccount() {
		try{
			...
			String[] ids = getRequest().getParameterValues("detailIds");//对账明细id
			if (ids != null) {
				for (String id : ids) {
					if (!id.equals("") && !id.equals("0")) {
						SlFundDetail detail=slFundDetailService.get(new Long(id));
						if (null == detail) {
							return SUCCESS;
						}
						//当前款项对应的流水记录
						SlFundQlide slFundQlide = slFundDetailService.get(new Long(id)).getSlFundQlide();
						...
						//重新计算利息的利息税
						if("loanInterest".equals(oldSlFundIntent.getFundType())){
							if(null==oldSlFundIntent.getInterestTax()){
								oldSlFundIntent.setInterestTax(new BigDecimal(0));
							}
							oldSlFundIntent.setInterestTax(oldSlFundIntent.getInterestTax().subtract(detail.getAfterMoney().multiply(Constants.TAX)));
						}
						
						slFundIntentService.merge(oldSlFundIntent);
						
						//调用NC推送
						//ncPushRecordService.backoutPush(detail.getRequestNo());
			...
			if (ids != null) {
				for (String id : ids) {
					if (!id.equals("") && !id.equals("0")) {
						...
						//4.修改款项罚息
						if(slPunishInterest.getType()==0){//罚息
							oldSlFundIntent.setFaxiAfterMoney(oldSlFundIntent.getFaxiAfterMoney().subtract(slPunishDetail.getAfterMoney()));
							oldSlFundIntent.setPenaltyTax(oldSlFundIntent.getPenaltyTax().subtract(slPunishDetail.getAfterMoney().multiply(Constants.TAX)));
						}else if(slPunishInterest.getType()==1){//逾期
							oldSlFundIntent.setOverdueAfterMoney(oldSlFundIntent.getOverdueAfterMoney().subtract(slPunishDetail.getAfterMoney()));
							oldSlFundIntent.setOverdureTax(oldSlFundIntent.getOverdureTax().subtract(slPunishDetail.getAfterMoney().multiply(Constants.TAX)));
						}
						slFundQlideService.merge(slFundQlide);
						...
						//调用NC推送
						ncPushRecordService.backoutPush(slPunishDetail.getRequestNo());
	...
	public String editAfterMoney() {
		try{
			...
			s.setFlatMoney(s.getFlatMoney()==null ? (new BigDecimal("0")).add(slFundIntent.getFlatMoney()) : s.getFlatMoney().add(slFundIntent.getFlatMoney()));
			...
			//3.获得罚息利率
			s.setPenaltyRate(loan.getPenaltyReceiveRate());
			if (null == s.getPenaltyRate()) {
				s.setPenaltyRate(new BigDecimal(0));
			}
			//4.获得逾期利率
			...
			penaltyMoney=s.getPenaltyRate().multiply(day).multiply(slFundIntent.getFlatMoney()).divide(new BigDecimal(100), 2);
			slFundDetail.setOverdueAccrual(overdueMoney);
			slFundDetail.setPenaltyAccrual(penaltyMoney);
	...
	public String check() {
		...
		try{
			...
			if(oldSlFundIntent.getAfterMoney()==null||"".equals(oldSlFundIntent.getAfterMoney())){
				oldSlFundIntent.setAfterMoney(new BigDecimal(am));
			}
...
//						long day = compare_date(DateUtil.parseDate(oldSlFundIntent.getGraceDay()),slFundQlide.getFactDate());
						...
						if(oldSlFundIntent.getGraceDay()!=null  &&!"".equals(oldSlFundIntent.getGraceDay())){
						...
						if(!"principalLending".equals(oldSlFundIntent.getFundType())){
							...
							//3.获得罚息利率
							oldSlFundIntent.setPenaltyRate(loan.getPenaltyReceiveRate());
							if (null == oldSlFundIntent.getPenaltyRate()) {
								oldSlFundIntent.setPenaltyRate(new BigDecimal(0));
							}
							//4.获得逾期利率
							...
							if(day2.compareTo(new BigDecimal(0))<0){
								overdueMoney=oldSlFundIntent.getOverdueRate().multiply(new BigDecimal(0)).multiply(detailAfterMoney).divide(new BigDecimal(100), 2);
								penaltyMoney=oldSlFundIntent.getPenaltyRate().multiply(new BigDecimal(0)).multiply(detailAfterMoney).divide(new BigDecimal(100), 2);
							}else{
								overdueMoney=oldSlFundIntent.getOverdueRate().multiply(day2).multiply(detailAfterMoney).divide(new BigDecimal(100), 2);
								penaltyMoney=oldSlFundIntent.getPenaltyRate().multiply(day2).multiply(detailAfterMoney).divide(new BigDecimal(100), 2);
							}
							slFundDetail.setOverdueAccrual(overdueMoney);
							slFundDetail.setPenaltyAccrual(penaltyMoney);
						...
						if("loanInterest".equals(oldSlFundIntent.getFundType())){
							...
							oldSlFundIntent.setInterestTax(oldSlFundIntent.getInterestTax().add(detailAfterMoney.multiply(Constants.TAX)));
						}
						...
						//调用NC推送
						if("principalLending".equals(oldSlFundIntent.getFundType())){
							ncPushRecordService.loanPush(slFundDetail);
						}else{
							ncPushRecordService.repaymentPush(slFundDetail);
						}
	...
	public String editQlideCheck() {
		try{
			...
			if(oldSlFundIntent.getAfterMoney()!=null&&!"".equals(oldSlFundIntent.getAfterMoney())){
				...
				oldSlFundIntent.setAfterMoney(inIntentMoney);
			}
			...
			long day =Long.valueOf("0");
			if(oldSlFundIntent.getGraceDay()!=null  &&!"".equals(oldSlFundIntent.getGraceDay())){
			    day = compare_date(DateUtil.parseDate(oldSlFundIntent.getGraceDay()),oldSlFundQlide.getFactDate());
			} 
//			long day = compare_date(DateUtil.parseDate(oldSlFundIntent.getGraceDay()),oldSlFundQlide.getFactDate());
			...
			if(!"principalLending".equals(oldSlFundIntent.getFundType())){
				...
				if(day2.compareTo(new BigDecimal(0))<0){
					overdueMoney=oldSlFundIntent.getOverdueRate().multiply(new BigDecimal(0)).multiply(inIntentMoney).divide(new BigDecimal(100), 2);
					penaltyMoney=oldSlFundIntent.getPenaltyRate().multiply(new BigDecimal(0)).multiply(inIntentMoney).divide(new BigDecimal(100), 2);
				}else{
					overdueMoney=oldSlFundIntent.getOverdueRate().multiply(day2).multiply(inIntentMoney).divide(new BigDecimal(100), 2);
					penaltyMoney=oldSlFundIntent.getPenaltyRate().multiply(day2).multiply(inIntentMoney).divide(new BigDecimal(100), 2);
				}
				slFundDetail.setOverdueAccrual(overdueMoney);
				slFundDetail.setPenaltyAccrual(penaltyMoney);
				...
				if("loanInterest".equals(oldSlFundIntent.getFundType())){
					...
					oldSlFundIntent.setInterestTax(oldSlFundIntent.getInterestTax().add(inIntentMoney.multiply(Constants.TAX)));
				}
			}
			...
			// 如果为本金偿还，则调用保证金和授信释放流程
			if (null != oldSlFundIntent.getFundType() && "principalRepayment".equals(oldSlFundIntent.getFundType())) {
				slFundIntentService.releaseCashBankRepayment(oldSlFundIntent.getBusinessType(), fundIntentId, slFundDetail.getFundDetailId());
			}
					...
					if (oldSlFundIntent.getNotMoney().compareTo(new BigDecimal("0")) == 0) {
						...
						if(!"principalLending".equals(oldSlFundIntent.getFundType())){
							...
							if(day>0){
								...
								penaltyMoney=oldSlFundIntent.getPenaltyRate().multiply(new BigDecimal(day)).multiply(detailAfterMoney).divide(new BigDecimal(100), 2);
								slFundDetail.setOverdueAccrual(overdueMoney);
								slFundDetail.setPenaltyAccrual(penaltyMoney);
						...
						// 如果为本金偿还，则调用保证金和授信释放流程
						if (null != oldSlFundIntent.getFundType() && "principalRepayment".equals(oldSlFundIntent.getFundType())) {
							slFundIntentService.releaseCashBankRepayment(oldSlFundIntent.getBusinessType(), oldSlFundIntent.getFundIntentId(), slFundDetail.getFundDetailId());
						}
	...
	public String complexCheck(){
		try{
			...
			BigDecimal penaltyMoney=new BigDecimal(0);//罚息金额
			BigDecimal overdueMoney=new BigDecimal(0);//逾期金额
			
			String overDay=this.getRequest().getParameter("overDay");//逾期天数
			String penaltyStr=this.getRequest().getParameter("penaltyMoney");//罚息金额
			String overdueStr=this.getRequest().getParameter("overdueMoney");//逾期金额
			
			if(null!=penaltyStr && !"".equals(penaltyStr)){
				penaltyMoney=new BigDecimal(penaltyStr);
			}
			if(null!=overdueStr && !"".equals(overdueStr)){
				overdueMoney=new BigDecimal(overdueStr);
			}
			...
			for(SlFundIntent s:list){
				...
				if(null==s.getFactDate()){
					...
					if("loanInterest".equals(s.getFundType())){
						s.setInterestTax(yingdui.multiply(Constants.TAX));
					}
					
					//3.修改款项
					s.setAfterMoney(s.getAfterMoney()==null?(new BigDecimal("0")).add(yingdui):s.getAfterMoney().add(yingdui));//已对账金额
					...
					if(!"principalLending".equals(s.getFundType())){
						//修改款项罚息金额
						s.setAccrualMoney(penaltyMoney);//罚息金额
						s.setFaxiAfterMoney(penaltyMoney);//罚息已对账
						s.setOverdureMoney(overdueMoney);//逾期金额
						s.setOverdueAfterMoney(overdueMoney);//逾期已对账金额
						...
						slFundDetail = createDetail(yingdui,s,overDays,penaltyMoney,overdueMoney);//款项明细
						
						if(penaltyMoney.compareTo(new BigDecimal("0"))>0){
							yingdui=yingdui.add(createPunishDetail(penaltyMoney, s, overDays, "0"));//罚息明细
						}
						if(overdueMoney.compareTo(new BigDecimal("0"))>0){
							yingdui=yingdui.add(createPunishDetail(overdueMoney, s, overDays, "1"));//逾期明细
						}
						//如果是利息对账则需要计算罚息税、逾期利息税
						if("loanInterest".equals(s.getFundType())){
							if(null==s.getPenaltyTax()){
								s.setPenaltyTax(new BigDecimal(0));
							}
							if(null==s.getOverdureTax()){
								s.setOverdureTax(new BigDecimal(0));
							}
							s.setPenaltyTax(s.getPenaltyTax().add(penaltyMoney.multiply(Constants.TAX)));
							s.setOverdureTax(s.getOverdureTax().add(overdueMoney.multiply(Constants.TAX)));
						}
	 ...
	 */
	public BigDecimal createPunishDetail(BigDecimal money,SlFundIntent s,long overDay,String flag){
		//查询罚息记录
		List<SlPunishInterest> listfaxi = slPunishInterestService.listbyisInitialorId(s.getFundIntentId(),flag);
		SlPunishInterest faxi= listfaxi.get(0);
		//判断罚息是否已经对过账
	 ...
	 * @param overDay  逾期天数
	 */
	public SlFundDetail createDetail(BigDecimal money,SlFundIntent s,long overDay,BigDecimal penaltyMoney,BigDecimal overdueMoney){
		SlFundDetail slFundDetail = new SlFundDetail();
		slFundDetail.setOperTime(new Date());
		slFundDetail.setSlFundIntent(s);
		slFundDetail.setSlFundQlide(slFundQlide);
	...
	public String reSignOverMoneyTo(){
		...
		List<SlFundIntent> list=slFundIntentService.getComplexList(projectId,payintentPeriod,"SmallLoan");
		//2.循环集合，分别对账
		BigDecimal overdueMoney = new BigDecimal(0.00);//逾期总额
		BigDecimal penaltyMoney = new BigDecimal(0.00);//罚息总额
		for(SlFundIntent s:list){
			reSignOverMoney(s,slFundQlide, map);
			if(map.size()>0){
				overdueMoney=overdueMoney.add(map.get("overdueMoney"));
				penaltyMoney=penaltyMoney.add(map.get("penaltyMoney"));
			}
		}
		map.put("penaltyMoney",penaltyMoney);//罚息金额
	...
	public void reSignOverMoney(SlFundIntent owe,SlFundQlide slFundQlide,Map<String,BigDecimal> map) {
		...
		try {
			//判断是否逾期
			if(StringUtils.isNotEmpty(owe.getGraceDay()) 
					&& (!"".equals(slFundQlide.getFactDate()) && slFundQlide.getFactDate() != null)){
			...
			BigDecimal baseMoney = new BigDecimal(0.00);//罚息本金
			BigDecimal baseOverMoney = new BigDecimal(0.00);//逾期本金
			//1.获得罚息收取方式(0按应还未还金额收取、1按合同金额收取)
			if("principalRepayment".equals(owe.getFundType())){
				baseMoney=owe.getNotMoney();
				baseOverMoney=owe.getNotMoney();
			}else{
				if(0==loan.getPenaltyReceiveWay()){
					baseMoney=owe.getNotMoney();
				}else{
					baseMoney=loan.getSecondProjectMoney();
				}
				//2.获得逾期收取方式(0按应还未还金额收取、1按合同金额收取)
				if(0==loan.getOverdueReceiveWay()){
					baseOverMoney=owe.getNotMoney();
				}else{
					baseOverMoney=loan.getSecondProjectMoney();
				}
			}
			//3.获得罚息利率
			owe.setPenaltyRate(loan.getPenaltyReceiveRate());
			if (null == owe.getPenaltyRate()) {
				owe.setPenaltyRate(new BigDecimal(0));
			}
			//4.获得逾期利率
			owe.setOverdueRate(loan.getOverdueRateLoan());
			...
			if (owe.getNotMoney().compareTo(new BigDecimal(0)) == 1) {
				...
				if (DateUtil.compare_date(overdueDate,newBegin) == -1) {
					...
					if(sortday==-1){
						...
						//计算逾期金额
						overdueMoney = owe.getOverdueRate().multiply(day1).multiply(baseOverMoney==null ? new BigDecimal(0.00) : baseOverMoney).divide(new BigDecimal(100), 2);
						//计算罚息金额
						penaltyMoney = owe.getPenaltyRate().multiply(day1).multiply(baseMoney==null? new BigDecimal(0.00) : baseMoney).divide(new BigDecimal(100), 2);
			...
			map.put("penaltyMoney",penaltyMoney);//罚息金额
			map.put("overdueMoney",overdueMoney);//逾期金额
			map.put("notMoney",owe.getNotMoney());//本金/利息未对账金额
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
				...
				if (list != null) {
					for (SlFundIntent l : list) {
						...
						if (businessType.equals("Financing")) {
							FlFinancingProject flFinancingProject = flFinancingProjectService.get(l.getProjectId());
							l.setOverdueRate(flFinancingProject.getOverdueRate());
						}
						if (businessType.equals("Pawn")) {
							PlPawnProject pawn = plPawnProjectService.get(l.getProjectId());
	...
	public String list2() { // listbySmallLoan and listbyFinancing
		try {
			...
			if (flag1 == 1) { // 正常从数据库获得listslSmallloanProject==null为款项确认，flag1==1尽职调查
				...
				list = slFundIntentService.getByProjectAsc(projectId,
						businessType, null);
				if (list != null) {
					for (SlFundIntent l : list) {
						slFundIntentService.evict(l);
	...
	public String superviselist() { // listbySmallLoan and listbyFinancing
		try {
			...
			if (slSuperviseRecord != null && flag1 == 0) {
				SuperviseFundIntentCreate superviseFundIntentCreate = new SuperviseFundIntentCreate(
						slSuperviseRecord, "no");
				list = superviseFundIntentCreate.createBorrowerSlFundIntent();
				/*
				 * // 生成款项计划 SimpleDateFormat sd = new
				 * SimpleDateFormat("yyyy-MM-dd");
				 * 
				 * if (calcutype.equals("ExhibitionBusiness")) { list =
				 * FundIntentListPro3 .getFundIntentDefaultList( calcutype,
				 * slSuperviseRecord.getAccrualtype(), slSuperviseRecord
				 ...
				 * sd.format(slSuperviseRecord.getEndDate()), slSuperviseRecord
				 * .getDayManagementConsultingOfRate()
				 * .divide(BigDecimal.valueOf(100)), slSuperviseRecord
				 * .getDayFinanceServiceOfRate()
				 * .divide(BigDecimal.valueOf(100)),
				 * slSuperviseRecord.getPayintentPeriod(),
				 * slSuperviseRecord.getIsStartDatePay(),
				 * slSuperviseRecord.getPayintentPerioDate(),
				 * slSuperviseRecord.getDayOfEveryPeriod(), "no", null,
				 * slSuperviseRecord .getIsInterestByOneTime(),
				 * slSuperviseRecord.getDateMode()); }
				 * 
				 * for (SlFundIntent s : list) { s.setBusinessType(calcutype); }
				 * List<SlFundIntent> listafter = slFundIntentService
				 * .getByProjectId(projectId, businessType); for (SlFundIntent t
				 * : listafter) { slFundIntentService.evict(t); if
				 * (t.getAfterMoney().compareTo(new BigDecimal(0)) != 0) {
				 * list.add(t); } } size = list.size();
				 */}
	...
	public String listbyAlterAccrualRecord() {
		...
		if (slAlterAccrualRecord == null || flag1 == 1) {
			if ((Boolean.valueOf(isUnLoadData))) {
				...
				list = slFundIntentService.getByProjectAsc(projectId,
						businessType, ((null != preceptId && !preceptId
								.equals("")) ? Long.valueOf(preceptId) : null));
			} else {
				if (isThisAlterAccrualRecordIdstring != null
						&& !"".equals(isThisAlterAccrualRecordIdstring)
						&& !"null".equals(isThisAlterAccrualRecordIdstring)) {
					Long alterAccrualRecordId = Long.parseLong(getRequest()
							.getParameter("isThisAlterAccrualRecordId"));
					if (isThisAlterAccrualRecord.equals("yes")) {
						list = slFundIntentService
								.getlistbyslslAlteraccrualRecordId(
										alterAccrualRecordId, businessType,
										projectId);
					} else {
						List<SlFundIntent> listall = slFundIntentService
								.getByProjectAsc(projectId, businessType,
										((null != preceptId && !preceptId
												.equals("")) ? Long
												.valueOf(preceptId) : null));
						for (SlFundIntent l : listall) {
							if (l.getSlAlteraccrualRecordId() == null) {
								list.add(l);
							}
							if (l.getSlAlteraccrualRecordId() != null
									&& !l.getSlAlteraccrualRecordId().equals(
											alterAccrualRecordId)) {
								list.add(l);
							}
						}
					}
		...
		if (slAlterAccrualRecord != null && flag1 == 0) { // 生成款项计划
			...
			if (calcutype.equals("SmallLoan")) {
				...
				BpFundProject project = bpFundProjectService.get(Long
						.valueOf(preceptId));

				BigDecimal principalMoney = slFundIntentService
						.getPrincipalMoney(projectId, businessType, sd
								.format(slAlterAccrualRecord.getAlelrtDate()),
								project.getId());
				List<SlFundIntent> flist = slFundIntentService.listByEarlyDate(
						projectId, businessType, sd.format(slAlterAccrualRecord
								.getAlelrtDate()), "loanInterest", project
								.getId());
				SlFundIntent f = null;
				...
				if (null != f) {
					...
					if (AppUtil.getInterest().equals("0")) {// 算头不算尾
						interestEndTime = DateUtil.addDaysToDate(f
								.getInterestEndTime(), 1);
					}
					SlFundIntent bf = null;
					if (project.getAccrualtype().equals(
							"sameprincipalandInterest")) {
						List<SlFundIntent> principalist = slFundIntentService
								.listByEarlyDate(projectId, businessType, sd
										.format(slAlterAccrualRecord
												.getAlelrtDate()),
										"principalRepayment", project.getId());
						if (null != principalist && principalist.size() > 0) {
							bf = principalist.get(0);
						}
					}
					FundAlterAccrualFundIntentCreate alterAccrualFundIntentCreate = new FundAlterAccrualFundIntentCreate(
							project, slAlterAccrualRecord, f
									.getPayintentPeriod(), interestEndTime, f
									.getInterestStarTime(), principalMoney, bf);
				...
				if (project.getAccrualtype().equals("singleInterest")) {
					if (project.getIsInterestByOneTime() != 1) {
						...
						int sdays = DateUtil.getDaysBetweenDate(sd
								.format(startDate), sd
								.format(slAlterAccrualRecord.getAlelrtDate()));
						int edays = DateUtil.getDaysBetweenDate(sd
								.format(slAlterAccrualRecord.getAlelrtDate()),
								sd.format(intentDate));
						List<SlFundIntent> ulist = slFundIntentService
								.getListByTypeAndDate(
										projectId,
										businessType,
										"FinancingRepay",
						...
						if (project.getIsPreposePayAccrual() == 0) {
							if (sdays > 0) {
								...
								s.setPayMoney(repayMoney.multiply(
										project.getAccrualNew().divide(
												new BigDecimal(100)).multiply(
												new BigDecimal(sdays)))
										.setScale(2, BigDecimal.ROUND_HALF_UP));
								s.setIncomeMoney(new BigDecimal(0));
								s.setIntentDate(slAlterAccrualRecord
										.getAlelrtDate());
								s.setPayintentPeriod(period);
								s.setInterestStarTime(startDate);
								s.setInterestEndTime(DateUtil.addDaysToDate(
										slAlterAccrualRecord.getAlelrtDate(),
										-1));
								if (s.getPayMoney()
							...
							if (edays > 0) {
								...
								s.setFundType("FinancingInterest");
								s
										.setPayMoney(repayMoney
												.multiply(
														slAlterAccrualRecord
																.getAccrual()
																.divide(
																		new BigDecimal(
																				100))
								...
								s.setPayintentPeriod(period);
								s.setInterestStarTime(slAlterAccrualRecord
										.getAlelrtDate());
								s.setInterestEndTime(DateUtil.addDaysToDate(
										intentDate, -1));
								if (s.getPayMoney()
												...
												.multiply(
														(slAlterAccrualRecord
																...
																.divide(
																		...
																		10,
																		BigDecimal.ROUND_HALF_UP))
																.subtract(project .getAccrualNew().divide(new BigDecimal(100)))).multiply(new BigDecimal(edays))
												.setScale(2,BigDecimal.ROUND_HALF_UP));
								s.setIncomeMoney(new BigDecimal(0));
								s.setIntentDate(slAlterAccrualRecord
										.getAlelrtDate());
						...
						List<SlFundIntent> sflist = slFundIntentService
								.getListByType(
										projectId,
										businessType,
										"FinancingInterest",
						...
						for (SlFundIntent s : sflist) {
							s.setPayMoney(s.getPayMoney().multiply(
									slAlterAccrualRecord.getAccrual().divide(
											project.getAccrualNew().multiply(
													new BigDecimal(30)), 10,
											BigDecimal.ROUND_HALF_UP))
									.setScale(2, BigDecimal.ROUND_HALF_UP));
							list.add(s);
						...
						if (project.getIsPreposePayAccrual() == 0) {
							int sdays = DateUtil.getDaysBetweenDate(sd
									.format(project.getStartDate()), sd
									.format(slAlterAccrualRecord
											.getAlelrtDate()));
							int edays = DateUtil.getDaysBetweenDate(sd
									.format(slAlterAccrualRecord
											.getAlelrtDate()), sd
									.format(project.getIntentDate()));
							List<SlFundIntent> ulist = slFundIntentService
									.getListByTypeAndDate(
											projectId,
											businessType,
				...
				String prepayintentPeriod = this.getRequest().getParameter(
						"alterpayintentPeriod");
				String slSuperviseRecordId = prepayintentPeriod.substring(0,
						prepayintentPeriod.lastIndexOf("."));
				int prepayintentPeriod1 = slAlterAccrualRecord
						.getAlterpayintentPeriod() - 1;
				Date intentDate = null;
				Date edate = null;
				if (slSuperviseRecordId.equals("0")) {
					FlLeaseFinanceProject project = flLeaseFinanceProjectService
							.get(projectId);
					List<SlFundIntent> flist = slFundIntentService
							.getByIntentPeriod(projectId, "LeaseFinance",
									"leaseFeeCharging", null,
									prepayintentPeriod1);
					if (null != flist && flist.size() > 0) {
							...
							if ((null != sf.getSlAlteraccrualRecordId() && !sf
									...
									.equals(
											slAlterAccrualRecord
													.getSlAlteraccrualRecordId()
													.toString()))) {
								intiprojectMoney = intiprojectMoney.add(sf
										.getIncomeMoney());
							}
						} else {
							intiprojectMoney = intiprojectMoney.add(sf
					...
					int payintentPeriod = project.getPayintentPeriod()
							- prepayintentPeriod1;
					if (payintentPeriod > 0) {
						...
						if (project.getIsStartDatePay().equals("2")) {
							/*
							 * if
							 * (!project.getPayaccrualType().equals("owerPay"))
						...
						if (AppUtil.getInterest().equals("0")) {
							listearly = LeaseFinanceFundIntentListPro
									.getFundIntentDefaultList(
											calcutype,
											...
											intiprojectMoney,
											sd.format(date),
											project.getIntentDate() == null ? null
													: sd.format(project
															.getIntentDate()),
											new BigDecimal(0),
											new BigDecimal(0), payintentPeriod,
											project.getIsStartDatePay(),
											project.getPayintentPerioDate(),
											project.getDayOfEveryPeriod(),
											"no", null, 0, null,
											new BigDecimal(0), project
													.getFeePayaccrualType(),
											project.getEachRentalReceivable());
						} else {
							listearly = LeaseFinanceFundIntentListProtw
									.getFundIntentDefaultList(
											calcutype,
											project.getAccrualtype(),
											0,
					...
					for (SlFundIntent si : listearly) {
						...
						si.setPayintentPeriod(si.getPayintentPeriod()
								+ prepayintentPeriod1);
						if (project.getAccrualtype().equals(
								"sameprincipalandInterest")
								|| project.getAccrualtype().equals(
										"matchingRepayment")) {
							list.add(si);
						} else {
					...
					SlSuperviseRecord record = slSuperviseRecordService
							.get(Long.valueOf(slSuperviseRecordId));

					List<SlFundIntent> flist = slFundIntentService
							.getByIntentPeriod(projectId, "LeaseFinance",
									"leaseFeeCharging", Long
					...
					List<SlFundIntent> slist = slFundIntentService
							.getListOfSupervise(projectId, "LeaseFinance",
									intentDate, Long
											.valueOf(slSuperviseRecordId));
					BigDecimal intiprojectMoney = new BigDecimal(0);
					for (SlFundIntent sf : slist) {
						if (null != sf.getSlAlteraccrualRecordId()) {
							if (sf.getFundType().equals(
									"leasePrincipalRepayment")
									&& (!sf
											.getSlAlteraccrualRecordId()
											.toString()
					...
					int payintentPeriod = record.getPayintentPeriod()
							- prepayintentPeriod1;
					if (payintentPeriod > 0) {
						Date date = intentDate;
						if (record.getIsStartDatePay().equals("2")) {
							/*
							 * if
							 * (!record.getPayaccrualType().equals("owerPay")) {
							 * date = DateUtil.addDaysToDate(date, 1); }
							 */
						} else {
							String date1 = sd.format(date);
						...
						earlyprojectMoney = record.getContinuationMoney()
								.subtract(intiprojectMoney);
						if (AppUtil.getInterest().equals("0")) {
							listearly = LeaseFinanceFundIntentListPro
									.getFundIntentDefaultList(
											calcutype,
											...
											slAlterAccrualRecord
													...
													.divide(
															BigDecimal
																	.valueOf(100)),
											earlyprojectMoney, sd.format(date),
											record.getEndDate() == null ? null
													: sd.format(record
															.getEndDate()),
											new BigDecimal(0),
											new BigDecimal(0), payintentPeriod,
											record.getIsStartDatePay(), record
													.getPayintentPerioDate(),
											record.getDayOfEveryPeriod(), "no",
											null, 0, null, new BigDecimal(0),
						...
						for (SlFundIntent si : listearly) {
							...
							si.setPayintentPeriod(si.getPayintentPeriod()
									+ prepayintentPeriod1);
							si.setSlSuperviseRecordId(Long
									.valueOf(slSuperviseRecordId));
							if ((record.getAccrualtype().equals(
									"sameprincipalandInterest") || record
									.getAccrualtype().equals(
											"matchingRepayment"))) {
								list.add(si);
							} else {
								if (si.getFundType().equals("leaseFeeCharging")) {
	...
	public String listbyEarlyRepaymentRecord() {
		...
		if (slEarlyRepaymentRecord != null && flag1 == 0) {
			String preceptId=this.getRequest().getParameter("preceptId");//项目id
			BpFundProject project=bpFundProjectService.get(Long.valueOf(preceptId));
			SlSmallloanProject slSmallloanProject=slSmallloanProjectService.get(project.getProjectId());
			//查询提前还款日期之前的所有应还本金之和
			...
			if (null != f) {
				...
				FundPrepaymentFundIntentCreate prepaymentFundIntentCreate = new FundPrepaymentFundIntentCreate(project, slEarlyRepaymentRecord,f.getPayintentPeriod(), interestEndTime, f.getInterestStarTime(), principalMoney, bf,slSmallloanProject);
		...
		if (relateIntentOrCharge != null && relateIntentOrCharge.equals("charge")) { // 取费用相关
			for (SlFundIntent l : list) {
				if (businessType.equals("SmallLoan")) {
					if (l.getFundType().equals("serviceMoney") || l.getFundType().equals("consultationMoney") || l.getFundType().equals("loanInterest")) {
						list1.add(l);
					}
				}
			}
		}
		if (relateIntentOrCharge != null && relateIntentOrCharge.equals("intent")) { // 取本金相关
			for (SlFundIntent l : list) {
				if (businessType.equals("SmallLoan")) {
					if (l.getFundType().equals("principalRepayment") || l.getFundType().equals("principalLending")) {
						list1.add(l);
					}
				}
			}
	...
	public String listbySuperviseRecord() {
		...
		if (slSuperviseRecord == null || flag1 == 1) {
			...
			} else if (isThisSuperviseRecord.equals("no")
					&& slSuperviseRecordIdstring.equals("noid")) {
				list = slFundIntentService.getByProjectAsc(projectId,
						businessType, ((null != preceptId && !preceptId
								.equals("")) ? Long.valueOf(preceptId) : null));
			} else {
			...
			if (list != null) {
				for (SlFundIntent l : list) {
					...
					//注释掉展期流程查询款项时计算罚息金额
					/*if (null != l.getNotMoney() && l.getNotMoney().compareTo(new BigDecimal(0)) == 1) {
						BigDecimal sortoverdueMoney = new BigDecimal(0.00);
						long sortday = new Long(0);
						SimpleDateFormat from = new SimpleDateFormat("yyyy-MM-dd 00:00:00");
						String newdate = from.format(new Date());
		...
		if (slSuperviseRecord != null && flag1 == 0) { // 生成款项计划
			...
			if (calcutype.equals("SmallLoan")) {
				...
				list = superviseFundIntentCreate.createBorrowerSlFundIntent();
				/*
				 * if (AppUtil.getInterest().equals("0")) { listinti =
				 * FundIntentListPro3.getFundIntentDefaultList( calcutype,
				 ...
				 * slSuperviseRecord.getIsInterestByOneTime(),
				 * slSuperviseRecord.getDateMode()); } else { listinti =
				 * FundIntentListProtw.getFundIntentDefaultList( calcutype,
				 * slSuperviseRecord.getAccrualtype(),
				 * slSuperviseRecord.getIsPreposePayAccrualsupervise(),
				 * slSuperviseRecord.getPayaccrualType(),
				 * slSuperviseRecord.getContinuationRateNew().divide(
				 ...
				 * .getDayOfEveryPeriod(), "no", null,
				 * slSuperviseRecord.getIsInterestByOneTime(),
				 * slSuperviseRecord.getDateMode()); } for (SlFundIntent s :
				 * listinti) {
				 * 
				 ...
				 * s.setBusinessType(calcutype); list.add(s); } }
				 */} else if (calcutype.equals("LeaseFinance")) {
				...
				if (AppUtil.getInterest().equals("0")) {
					list = LeaseFinanceFundIntentListPro
							.getFundIntentDefaultList(
									...
									0,
									slSuperviseRecord.getPayaccrualType(),
									slSuperviseRecord.getContinuationRate()
											.divide(BigDecimal.valueOf(100)),
									slSuperviseRecord.getContinuationMoney(),
									sd.format(slSuperviseRecord.getStartDate()),
									slSuperviseRecord.getEndDate() == null ? null
											: sd.format(slSuperviseRecord
													.getEndDate()),
									slSuperviseRecord
											.getManagementConsultingOfRate()
											.divide(BigDecimal.valueOf(100)),
									new BigDecimal(0), slSuperviseRecord
											.getPayintentPeriod(),
									slSuperviseRecord.getIsStartDatePay(),
									slSuperviseRecord.getPayintentPerioDate(),
									slSuperviseRecord.getDayOfEveryPeriod(),
									"no", null, 0, null, new BigDecimal(0),
	...
	public String saveprojectreleadfinance(Long projectId, String businessType) {// 更新项目表的财务字段
		BigDecimal loanInterest = new BigDecimal(0);//利息
		BigDecimal consultationMoney = new BigDecimal(0);//管理咨询费
		BigDecimal serviceMoney = new BigDecimal(0);//财务服务费
		BigDecimal payAccrualMoney = new BigDecimal(0);//已还金额(利息)
		BigDecimal payProjectMoney = new BigDecimal(0);//已还金额(本金)
		BigDecimal flatMoney = new BigDecimal(0);//平账金额
		//1.查询除本金挪用的所有有效款项记录
		List<SlFundIntent> slFundIntents = slFundIntentService.getByProjectId1(projectId, businessType);
		//2.查询有效但不允许对账的款项并且是已对过账的
		List<SlFundIntent> isvalidandchecklist = slFundIntentService.getByisvalidAndaftercheck(projectId, businessType);
		if (businessType.equals("SmallLoan")) {
			...
			project.setAccrualMoney(loanInterest);//利息总额
			project.setConsultationMoney(consultationMoney);//管理咨询费
			project.setServiceMoney(serviceMoney);//财务服务费
			project.setPayAccrualMoney(payAccrualMoney);//已还金额(利息)
			project.setPayProjectMoney(payProjectMoney);//已还金额(本金)
			project.setFlatMoney(flatMoney);//平账金额
	...
	public String cancel() {
		...
		if(slFundIntent.getAfterMoney().compareTo(new BigDecimal("0")) == 0){
			...
			slFundIntent.setIsManual(b);
			slFundIntentService.save(slFundIntent);
			saveprojectreleadfinance(slFundIntent.getProjectId(), slFundIntent.getBusinessType());
			setJsonString("{success:true,msg:'取消成功！'}");
		}else{
			setJsonString("{success:false,msg:'取消失败，已流水对账，请先撤销流水对账！'}");
		}
		return SUCCESS;
	}
	...
	public String back() {
		SlFundIntent slFundIntent = slFundIntentService.get(fundIntentId);
		Short b = 1;
		if(null != slFundIntent.getIsManual() && slFundIntent.getIsManual().equals(b)){
			Short a = 0;
			slFundIntent.setIsValid(a);
			slFundIntentService.save(slFundIntent);
			saveprojectreleadfinance(slFundIntent.getProjectId(), slFundIntent.getBusinessType());
			setJsonString("{success:true,msg:'还原成功！'}");
		}else{
			setJsonString("{success:false,msg:'还原失败，不能还原此款项台账！'}");
		}
		return SUCCESS;
	}
	...
	// 罚息对账
	public void checkPunishIntent(Long qlideId, String fundIntentId,String type) {
		...
		List<SlPunishInterest> punish = slPunishInterestService.listbyisInitialorId(Long.valueOf(fundIntentId),type);
		if (punish.size() > 0) {// 有罚息台账就对账
			...
			if("1".equals(type)){//罚息
				...
				slFundIntent.setOverdueAfterMoney(slPunishInterest.getAfterMoney());
			}
			slFundIntentService.save(slFundIntent);
		}
	}
		...
		try {
			for (SlFundDetail s : slFundDetails) {
				if (null == s.getSlFundQlide().getIsCash()) {
					...
					if (sllist != null) {
						CsBank cb = csBankService.get(sllist.get(0)
								.getOpenBankId());
	...
	public String MoneyDetail() {
		try {
			String projectId=this.getRequest().getParameter("projectId");//项目Id
			String payintentPeriod=this.getRequest().getParameter("payintentPeriod");//期数
			//List<SlFundIntent> list1=slFundIntentService.getComplexList(projectId,payintentPeriod,"SmallLoan");
			String type=this.getRequest().getParameter("type");//标识查询资金明细的范围(0查所有既包括罚息、1只查相关款项)
			List<SlFundDetail> list =slFundDetailService.getlistbySlFundIntentId(fundIntentId,type);
			//List<SlFundDetail> list =slFundDetailService.getlistbyProject(type, projectId, payintentPeriod, "SmallLoan");
					...
					if (s.getIsOverdue() == null
						...
						if (compare_date(s.getIntentDate(), new Date()) > 0) {
							...
							sortoverdueMoney = OverdueMoney.multiply(
									...
									new BigDecimal(100), 2);
							overduebreakMoney = (s.getAccrualMoney()==null ?new BigDecimal(0):s.getAccrualMoney()).add(
									sortoverdueMoney).setScale(2,
									BigDecimal.ROUND_HALF_UP);
							overduebreakMoneysum = overduebreakMoneysum
									.add(overduebreakMoney);
	...
	public  String  overdueMoney(){
		...
		List<SlFundIntent> sl=slFundIntentService.overdueMoneys();
		
		StringBuffer buff = new StringBuffer("{success:true,'totalCounts':").append(sl.size()>0?0:sl.size()).append(",result:");
		JSONSerializer json = JsonUtil.getJSONSerializer("intentDate","factDate", "startDate", "interestEndTime");
		json.transform(new DateTransformer("yyyy-MM-dd"),new String[] {"intentDate","factDate","startDate","interestEndTime"});
		buff.append(json.serialize(sl));
		buff.append("}");
		jsonString = buff.toString();
		return SUCCESS;
	}
	...
	public void intentDownLoad() {
		...
		String typetab = getRequest().getParameter("typetab");
		String fundType = getRequest().getParameter("fundType");
		if (map.size() == 5 && fundType.equals("('principalDivert')")) {
			try {
				List<SlFundIntent> listbyowe = slFundIntentService.listbyOwe(
						businessType, fundType, null);
				for (SlFundIntent owe : listbyowe) {
					slFundIntentService.evict(owe);
					SlSmallloanProject loan = slSmallloanProjectService.get(owe
							.getProjectId());
					BigDecimal sortoverdueMoney = new BigDecimal(0.00);
					Date overdueintentDate = from
					...
					if (loan.getOverdueRateType().equals("1")
						...
						if (owe.getFactDate() == null) {
							...
							if (compare_date(d, owe.getIntentDate()) == -1
									&& owe.getPunishAccrual() != null) {
								sortday = compare_date(owe.getIntentDate(),
										new Date()) + 1;
								BigDecimal day1 = new BigDecimal(sortday);
								BigDecimal OverdueMoney = owe
										.getPunishAccrual().multiply(day1);
								sortoverdueMoney = OverdueMoney.multiply(
							...
							if (listfaxi == null || listfaxi.size() == 0) {
								...
								faxiSlFundIntent
										.setIntentDate(overdueintentDate);
								faxiSlFundIntent
										.setIncomeMoney(sortoverdueMoney);
								faxiSlFundIntent
										.setPayMoney(new BigDecimal("0"));
								faxiSlFundIntent.setNotMoney(sortoverdueMoney);
								faxiSlFundIntent.setAfterMoney(new BigDecimal(
										"0"));
								faxiSlFundIntent.setBusinessType(owe
										.getBusinessType());
								faxiSlFundIntent.setFundIntentId(owe
										.getFundIntentId());
								faxiSlFundIntent.setNotMoney(sortoverdueMoney);
								faxiSlFundIntent.setProjectId(owe
										.getProjectId());
								faxiSlFundIntent.setFundIntentId(owe
										.getFundIntentId());
								faxiSlFundIntent.setFundType(owe.getFundType());
								faxiSlFundIntent.setCompanyId(owe
										.getCompanyId());
								faxiSlFundIntent.setPunishDays(Integer
										.valueOf(sortday.toString()));
							} else {

								faxiSlFundIntent = listfaxi
										.get(listfaxi.size() - 1);

								faxiSlFundIntent
										.setIncomeMoney(sortoverdueMoney);
								faxiSlFundIntent.setNotMoney(sortoverdueMoney
										.subtract(faxiSlFundIntent
												.getAfterMoney()));
								faxiSlFundIntent
										.setIntentDate(overdueintentDate);
		...
		// 初始化罚息 5表示第一次进页面
		// 本金收款 台账
		if (map.size() == 5 && fundType.equals("('principalRepayment')")
				&& typetab.equals("all")) {
			try {
				List<SlFundIntent> listbyowe = slFundIntentService.listbyOwe(
						businessType, fundType, null);
				...
				for (SlFundIntent owe : listbyowe) {
					...
					if (loan.getOverdueRateType().equals("1")
							|| loan.getOverdueRateType().equals("2")) { // 按天
						owe.setOverdueRateLoan(loan.getOverdueRateLoan());
						if (null == owe.getOverdueRateLoan()) {
							owe.setOverdueRateLoan(new BigDecimal(0));
						}
						// 本金罚息计算
						streamPunishMethInit(owe, from);
					}

					// 保存或更新罚息详细信息
					defaultInterest(owe, overdueintentDate, owe
							.getAccrualMoney(), owe.getContinueDates(), owe
							.getAmountPayable(), owe.getContinueInterest(),"0");
				}
		...
		if (map.size() == 5
				&& fundType
						.equals("('loanInterest','consultationMoney','serviceMoney')")
				&& typetab.equals("all")) {
			try {
				List<SlFundIntent> listbyowe = slFundIntentService.listbyOwe(
						businessType, fundType, null);
				for (SlFundIntent owe : listbyowe) {
					...
					if (loan.getOverdueRateType().equals("1")
						...
						// 利息罚息计算
						streamInterestPunishMethInit(owe, from);

					} else { // 按期计算

						List<SlFundIntent> listbyproject = slFundIntentService
		...
		if (fundType.equals("('principalLending')")) {// 本金
			String[] tableHeader = { "序号", "项目名称", "项目编号", "资金类型", "放款金额",
					"计划放款日", "实际放款日", "已对账金额", "未对账金额", "核销金额", "放款状态", "备注" };
			String[] fields = { "projectName", "projectNumber", "fundTypeName",
					"incomeMoney", "intentDate", "factDate", "afterMoney",
					"notMoney", "flatMoney", "tabType", "remark" };
			try {
			...
			String[] tableHeader = { "序号", "项目名称", "项目编号", "资金类型", "放款金额",
					...
					"核销金额", "顺延至", "罚息金额", "顺延息", "罚息金额", "罚息已对账金额", "放款状态",
					"备注" };
			String[] fields = { "projectName", "projectNumber", "fundTypeName",
					"incomeMoney", "intentDate", "factDate",
					"interestStarTime", "interestEndTime", "afterMoney",
					"notMoney", "flatMoney", "continueDay", "amountPayable",
					"continueInterest", "accrualMoney", "faxiAfterMoney",
					"tabType", "remark" };
			try {
			...
			String[] tableHeader = { "序号", "项目名称", "项目编号", "资金类型", "应付金额",
					"计划到账日", "实际到账日", "已对账金额", "未对账金额", "平账金额", "核销金额", "顺延至",
					"罚息金额", "顺延息", "罚息金额", "罚息已对账金额", "放款状态", "备注" };
			String[] fields = { "projectName", "projectNumber", "fundTypeName",
					"payMoney", "intentDate", "factDate", "afterMoney",
					"notMoney", "flatMoney", "flatMoney", "continueDay",
					"amountPayable", "continueInterest", "accrualMoney",
					"faxiAfterMoney", "tabType", "remark" };
			try {
			...
			String[] fields = { "projectName", "projectNumber", "fundTypeName",
					"incomeMoney", "intentDate", "factDate",
					"interestStarTime", "interestEndTime", "afterMoney",
					"notMoney", "flatMoney", "continueDay", "graceDay",
					"accrualMoney", "faxiAfterMoney", "tabType", "remark" };
			try {
				ExportExcel.export(tableHeader, fields, list, "利息收款台账",SlFundIntent.class);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
...
//		String[] tableHeader = { "序号", "借款项目名称", "项目编号","期数","本金" ,
//				"利息","罚息","管理咨询费","财务服务费","合计费用",
//				"未对账金额","逾期天数","计划还款日","实际到账金额","实际到账日"};
//		String[] fields = {"projectName","projectNumber","payintentPeriod","principalRepayment",
//				"loanInterest","punishMoney", "consultationMoney","serviceMoney","synthesizeMoney",
//				"notSynthesizeMoney","punishDays","intentDate","synthesizeAfterMoney","factDate"};
	...
	public void leaseDownload() {
		...
		try {
			 ...
			 * "payaccrualType"); String payintentPeriod =
			 * this.getRequest().getParameter( "payintentPeriod"); String
			 * dayOfEveryPeriod = this.getRequest().getParameter(
			 * "dayOfEveryPeriod"); String leaseDepositRate =
			 * this.getRequest().getParameter( "leaseDepositRate"); String
			 * leaseDepositMoney = this.getRequest().getParameter(
			 * "leaseDepositMoney"); String rentalFeeRate =
			 * this.getRequest().getParameter( "rentalFeeRate"); String
			 * rentalFeeMoney = this.getRequest().getParameter(
			 * "rentalFeeMoney"); String leaseRetentionFeeRate =
			 * this.getRequest().getParameter( "leaseRetentionFeeRate"); String
			 * leaseRetentionFeeMoney = this.getRequest().getParameter(
			 * "leaseRetentionFeeMoney"); String rentalRate =
			 * this.getRequest().getParameter("rentalRate"); String rentalMoney
			 * = this.getRequest().getParameter("rentalMoney"); String
			 * eachRentalReceivable = this.getRequest().getParameter(
			 * "eachRentalReceivable"); String projectMoney = this.getRequest()
			 * .getParameter("projectMoney"); String allMoney =
			 * this.getRequest().getParameter("allMoney"); String
			 * connotationRate = this.getRequest().getParameter(
			 * "connotationRate"); String accrualtype =
			 * this.getRequest().getParameter("accrualtype"); String startDate =
			 * this.getRequest() .getParameter("startDate"); String intentDate =
			 * this.getRequest() .getParameter("intentDate");
			 * project.setPayaccrualType(payaccrualType);
			 * project.setPayintentPeriod(Integer.valueOf(payintentPeriod)); if
			 * (payaccrualType.equals("owerPay")) {
			 ...
			 * leaseRetentionFeeRate)); project.setLeaseRetentionFeeMoney(new
			 * BigDecimal( leaseRetentionFeeMoney)); project.setRentalRate(new
			 * BigDecimal(rentalRate)); project.setRentalMoney(new
			 * BigDecimal(rentalMoney)); project.setProjectMoney(new
			 * BigDecimal(projectMoney)); project.setAllMoney(new
			 * BigDecimal(allMoney)); if (null != eachRentalReceivable &&
			 * !eachRentalReceivable.equals("")) {
			 * project.setEachRentalReceivable(new BigDecimal(
			 * eachRentalReceivable)); } project.setConnotationRate(new
			 * BigDecimal(connotationRate));
			 */
			...
			if (project.getAccrualtype().equals("matchingRepayment")) {
				List<SlFundIntent> slist = slFundIntentService
						.getListByProjectId(projectId, businessType);
				int j = 1;
				for (int i = 0; i < slist.size(); i++) {
					...
					if (SlFundIntent1.getFundType().equals(
						...
						buff.append("{\"ri\":");
						buff.append(sd.format(SlFundIntent1.getIntentDate()));
						buff.append(",\"yszj\":");
						if (j == project.getPayintentPeriod()) {
							buff.append(SlFundIntent1.getIncomeMoney().add(
									SlFundIntent0.getIncomeMoney()).subtract(
									project.getLeaseDepositMoney()));
						} else {
							buff.append(SlFundIntent1.getIncomeMoney().add(
						...
						buff.append(",\"wfbj\":");
						money = money.subtract(SlFundIntent1.getIncomeMoney());
						if (j == project.getPayintentPeriod()) {
							 ...
							 * buff.
							 * append(money.subtract(project.getLeaseDepositMoney
							 * ())); } else {
							 */
							buff.append(money);
							// }
						...
						if (j == project.getPayintentPeriod()) {
							buff.append(SlFundIntent1.getIncomeMoney().add(
									SlFundIntent0.getIncomeMoney()).subtract(
									project.getLeaseDepositMoney()).subtract(
									SlFundIntent0.getIncomeMoney().multiply(
	...
	public String downloadurge() {
		...
		PagingBean pb = new PagingBean(start, limit);
		Object ids = this.getRequest().getSession().getAttribute("userIds");
		Map<String, String> tempMap = GroupUtil.separateFactor(getRequest(),
				ids);
		map.put("companyId", tempMap.get("companyId"));
		map.put("userId", tempMap.get("userId"));
		...
		List<VSmallloanProject> projectList = vSmallloanProjectService
				.getProjectList(tempMap.get("userId"), projectStatuses, pb,
						getRequest());
		PageBean<SlFundIntent> pageBean = new PageBean<SlFundIntent>(start,
				limit, getRequest());
		slFundIntentService.searchurge(pageBean, map, businessType);

		String[] tableHeader = { "序号", "客户名称", "联系电话", "资金类型", "计划收入金额'",
				"计划到帐日", "实际到帐日", "催收时间" };
		try {
	 ...
	 * 罚息详细
	 */
	private void defaultInterest(SlFundIntent sf, Date overdueintentDate,BigDecimal totalSortoverdueMoney, Integer continueDates,BigDecimal amountPayable, BigDecimal continueInterest,String type) {
		List<SlPunishInterest> listfaxi = slPunishInterestService.listbyisInitialorId(sf.getFundIntentId(), type);
		SlPunishInterest faxiSlFundIntent = new SlPunishInterest();
		if (listfaxi == null || listfaxi.size() == 0) {
			...
			faxiSlFundIntent.setNotMoney(totalSortoverdueMoney.subtract(faxiSlFundIntent.getAfterMoney()));
			faxiSlFundIntent.setIntentDate(overdueintentDate);
			faxiSlFundIntent.setContinueDates(continueDates);
			if (sf.getContinueDay() != null && !sf.getContinueDay().equals("")) {
				if (compare_date(DateUtil.parseDate(sf.getContinueDay(),"yyyy-MM-dd"), sf.getIntentDate()) == 0) {
	...
	/**
	 * 
	 * @param startDate 罚息起始时间
	 * @param endDate   罚息结束时间
	 * @param m         金额
	 * @param mRate     利率 为 贷款项目填写的值
	 * @return
	 */
	private BigDecimal interestPenaltyMeth(Date startDate, Date endDate,BigDecimal m, BigDecimal mRate) {
		BigDecimal money = new BigDecimal(0);
		Long sortdaty = compare_date(startDate, endDate);
		money = m.multiply(new BigDecimal(sortdaty).multiply(mRate)).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_EVEN);
		return money;
	}
	...
	private BigDecimal resetInterestPenaltyMeth(SlFundIntent slFundIntent,
		...
		if (slFundIntent.getBusinessType().equals("SmallLoan")) {
			...
			if (loan.getOverdueRateType() != null
					&& loan.getOverdueRateType().equals("2")) {
		...
		if (compare_date(slFundIntent.getFactDate(), slFundIntent
			...
			defaultInterest(slFundIntent, overdueintentDate, sortoverdueMoney,
					...
					slFundIntent.getContinueInterest(),"0");
		}
	...
	public void outToExcel() {
		try {
			...
			String[] tableHeader = { "序号", "项目编号", "客户名称", "联系电话", "资金类型",
					"计划收入金额", "未对帐金额", "逾期金额","罚息金额", "计划到帐日", "实际到帐日", "最后催收时间" };
			...
			Map<String, String> tempMap = GroupUtil.separateFactor(
					getRequest(), null);
			map.put("companyId", tempMap.get("companyId"));
			PageBean<SlFundIntent> pageBean = new PageBean<SlFundIntent>(null,
					null, getRequest());
			slFundIntentService.searchurge(pageBean, map, businessType);
			...
			ExcelHelper.export(pageBean.getResult(), tableHeader,
					tableHeaderName);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String listObligation() {
		/*
		 * String accrualtype
		 * =this.getRequest().getParameter("accrualtype");//计息方式（等额本金。。） String
		 * payaccrualType
		 * =this.getRequest().getParameter("payaccrualType");//计息周期（按月。。） String
		 * projectMoney =this.getRequest().getParameter("projectMoney");//投资额
		 * String startDate
		 * =this.getRequest().getParameter("startDate");//投资开始时间 String
		 * intentDate =this.getRequest().getParameter("startDate");//投资结束时间
		 * String accrualnew
		 * =this.getRequest().getParameter("accrualnew");//投资人利率 String
		 * isStartDatePay
		 * =this.getRequest().getParameter("isStartDatePay");//是否固定在哪一天收息 String
		 * payintentPerioDate
		 * =this.getRequest().getParameter("payintentPerioDate");//固定在那一天收息
		 * String payintentPeriod
		 * =this.getRequest().getParameter("payintentPeriod");//收息期限 String
		 * dayOfEveryPeriod
		 * =this.getRequest().getParameter("dayOfEveryPeriod");//自定义周期长度
		 */String investPersonId = this.getRequest().getParameter(
				"investPersonId");// 投资人id
		String keyValue = this.getRequest().getParameter("keyValue");// 判断是生成多个投资人款项还是一个人的款项
		String obligationInfoId = this.getRequest().getParameter(
				"obligationInfoId");// 投资人id
		try {
			...
			if (flag1 == 1) {// 不生成款项只查询
				if (projectId != null && !"".equals(projectId)
					...
					if (obligationInfoId != null
							&& !"".equals(obligationInfoId)) {
						obligationInfosId = Long.valueOf(obligationInfoId);
					}
					list = slFundIntentService.getListByTreeId(projectId,
							investMentPersonId, obligationInfosId);
					size = list.size();
				}
			...
			if (flag1 == 0 && "oneSlFundIntentCreat".equals(keyValue)) {// 给单个投资人生成款项
				...
				if (investPersonId != null && !"".equals(investPersonId)) {
					person = csInvestmentpersonService.get(Long
							.valueOf(investPersonId));
				}
				SimpleDateFormat sd = new SimpleDateFormat("yyyy-MM-dd");
				list = obligationFundIntenList.getFundIntentDefaultList(
						obObligationProject.getAccrual().divide(
								BigDecimal.valueOf(100)), obObligationProject
								.getPayaccrualType(), sd
								.format(obObligationProject.getStartDate()), sd
								.format(obObligationProject.getIntentDate()),
						obObligationProject.getAccrual().divide(
								BigDecimal.valueOf(100)), obObligationProject
								.getPayintentPerioDate(), obObligationProject
								.getPayintentPeriod(), obObligationProject
								.getIsStartDatePay(), obObligationProject
								.getIsPreposePayAccrual(), isHaveLending,
						obObligationProject.getProjectMoney(),
			...
			if (flag1 == 0 && "manySlFundIntentCreat".equals(keyValue)) {// 给多个投资人生成款项
				if (projectId != null && !"".equals(projectId)) {
					ObObligationProject obligationProject = obObligationProjectService
							.get(projectId);
					List<ObObligationInvestInfo> listInvest = obObligationInvestInfoService
							.getInfoByobObligationProjectId(projectId, "1");
					if (listInvest != null && listInvest.size() > 0) {
						for (ObObligationInvestInfo temp : listInvest) {
							List<SlFundIntent> lists = null;
							CsInvestmentperson person = null;
							if (temp.getInvestMentPersonId() != null
									&& !"".equals(temp.getInvestMentPersonId())) {
								person = csInvestmentpersonService.get(temp
										.getInvestMentPersonId());
							}
							if (temp.getFundIntentStatus() == 0) {
								SimpleDateFormat sd = new SimpleDateFormat(
										"yyyy-MM-dd");
								lists = obligationFundIntenList
										.getFundIntentDefaultList(
												temp
														.getObligationAccrual()
														.divide(
																BigDecimal
																		.valueOf(100)),
												obObligationProject
								...
								for (SlFundIntent s : lists) {
									s.setObligationId(projectId);// 债权项目表的id
									s.setInvestPersonId(temp
											.getInvestMentPersonId());// 投资人的id
									s.setInvestPersonName(temp
											.getInvestPersonName());// 保存债权人姓名
									s.setObligationInfoId(temp.getId());
									list.add(s);
								}
							} else {
								lists = slFundIntentService.getListByTreeId(
										projectId,
										temp.getInvestMentPersonId(), temp
												.getId());
			...
			for (SlFundIntent s : list) {
				if (null != s.getFundType() && !"".equals(s.getFundType())) {
					List<DictionaryIndependent> dlist = dictionaryIndependentService
	...
	public void assignmentDownload() {
		String investPersonId = this.getRequest()
				.getParameter("investPersonId");// 投资人id
		String keyValue = this.getRequest().getParameter("keyValue");// 判断是生成多个投资人款项还是一个人的款项
		String obligationInfoId = this.getRequest().getParameter(
				"obligationInfoId");// 投资人id
		List<SlFundIntent> list = new ArrayList();
		if (projectId != null && !"".equals(projectId) && projectId != 0) {
			Long investMentPersonId = 0l;
			Long obligationInfosId = 0l;
			if (!"undefined".equals(investPersonId) && investPersonId != null
					&& !"".equals(investPersonId)) {
				investMentPersonId = Long.valueOf(investPersonId);
			}
			if (!"undefined".equals(obligationInfoId)
					&& obligationInfoId != null && !"".equals(obligationInfoId)) {
				obligationInfosId = Long.valueOf(obligationInfoId);
			}
			list = slFundIntentService.getListByTreeId(projectId,
					investMentPersonId, obligationInfosId);
		...
		for (SlFundIntent s : list) {
			List<DictionaryIndependent> dicList = dictionaryIndependentService
					.getByDicKey(s.getFundType());
		...
		// .valueOf(0) : loan.getOverdueRate());
		// }
		String[] tableHeader = { "序号", "投资人", "期数", "资金类型", "计划收入金额", "计划支出金额",
				"到帐日", "对账日" };
		try {
	...
	public String borrowerToInvestor() {/*
										 ...
										 * StringBuffer(); StringBuffer sb = new
										 * StringBuffer(); // 转账列表 // 借款人第三方标识
										 * String borrowerFlag = "";
										 * slFundIntent =
										 * slFundIntentService.get
										 * (fundIntentId); PlBidPlan plBidPlan =
										 * new PlBidPlan(); plBidPlan =
										 * plBidPlanService
										 * .get(slFundIntent.getBidPlanId());
										 * if(
										 * configMap.get("thirdPayType").toString
										 * ().equals("1")){buff.append(
										 * "{success:true,result:'只有第三方支付托管模式才能使用从借款人账户直还功能'"
										 * ); }else{
										 * if(configMap.get("thirdPayConfig"
										 * ).toString
										 * ().equals(Constants.MONEYMOREMORE)){
										 * borrowerFlag =
										 * getCustFlag(plBidPlan); // 如果账户余额大于
										 * 要还金额 进行还款 否则提示平台进行垫付 if
										 * (getPayMoney(borrowerFlag
										 * ).compareTo(slFundIntent
										 * .getIncomeMoney()) >= 0) {
										 * MoneyMoreMore moneyMoreMore =
										 * createMoneyMoreMore
										 * (plBidPlan.getBidId().toString(),
										 * slFundIntent, borrowerFlag); sb =
										 * getLoanList
										 * (plBidPlan.getBidId().toString
										 * (),slFundIntent); // 转账成功以后 进行 审核接口
										 * // 判断是否已经存在 转账列表 if (sb == null ||
										 * sb.length() <= 0) {
										 * iPayService.transfer(moneyMoreMore,
										 * this.getBasePath()); sb =
										 * getLoanList(
										 * plBidPlan.getBidId().toString
										 * (),slFundIntent); }
										 * 
										 * // 生成 还款 转账列表 if (sb.length() > 0) {
										 * sb = sb.deleteCharAt(sb.length() -
										 * 1); transferaudit(sb.toString()); }
										 * buff
										 * .append("{success:true,result:'转账成功'"
										 * ); } else {buff.append(
										 * "{success:false,result:'账户余额不足'"); }
										 * }else
										 * if(configMap.get("thirdPayConfig"
										 * ).toString
										 * ().equals(Constants.FUIOU)){
										 * buff=this
										 * .repayByLoanFuiou(plBidPlan,slFundIntent
										 * ,buff); }else
										 * if(configMap.get("thirdPayConfig"
										 * ).toString
										 * ().equals(Constants.HUIFU)){
										 * 
										 * }else{buff.append(
										 * "{success:false,result:'请检查是否正确配置第三方支付配置文件,如果正确配置,那就尚未对接此类型的第三方支付接口,请联系金智万维软件公司进行咨询!'"
										 * ); } }
										 * 
										 * buff.append("}"); jsonString =
										 * buff.toString(); return SUCCESS;
										 */
		return SUCCESS;
	}
	...
	private StringBuffer getLoanList(String bidPlanID, SlFundIntent slFundIntent) {
		...
		filter.addFilter("Q_BatchNo_S_EQ", "hk_"
				+ slFundIntent.getFundIntentId() + "_" + bidPlanID);
	...
	private MoneyMoreMore createMoneyMoreMore(String bidPlanID,
		...
		for (int i = 0; i < slActual.size(); i++) {
			...
			if (slFundIntent.getFundType().equals("principalRepayment")
				...
				mlib.setLoanInMoneymoremore(cust.getMoneymoremoreId());// 收款人
																		// 本金和利息收款人为
																		// 投资人
			} else {
				mlib.setLoanInMoneymoremore(AppUtil.getSysConfig().get(
						"MM_PlatformMoneymoremore").toString());// 其它 为 平台
			}
			mlib.setOrderNo(UUIDGenerator.getUUID());
			mlib.setBatchNo("hk_" + slFundIntent.getFundIntentId() + "_"
					+ bidPlanID);// hk 代表是还款的动作 加上 款项台帐ID + 标ID
	...
	public String selectInvestPerson() {
		...
		Type type = new TypeToken<List<CsBank>>() {
		}.getType();
		StringBuffer buff = new StringBuffer("{success:true,'totalCounts':")
	...
	public String listPeriodbyLedger() {
		...
		List<SlFundIntentPeriod> list = slFundIntentService
				.listByBidPlanIdAndpayintentPeriod(pb, map);
		System.currentTimeMillis();
		List<SlFundIntentPeriod> sizelist = slFundIntentService
				.listByBidPlanIdAndpayintentPeriod(null, map);
	...
	public String getCouponsList() {
		...
		List<SlFundIntentPeriod> list = slFundIntentService.getCouponsList(pb,
				map);
		System.currentTimeMillis();
		List<SlFundIntentPeriod> sizelist = slFundIntentService.getCouponsList(
				null, map);
	...
	public String getraiseRateList() {
		...
		List<SlFundIntentPeriod> list = slFundIntentService
				.getRaiseinterestList(pb, map);
		System.currentTimeMillis();
		List<SlFundIntentPeriod> sizelist = slFundIntentService
				.getRaiseinterestList(null, map);
	...
	public void excelCouponsBpfundDetail() {
		...
		String[] tableHeader = { "序号", "招标项目名称", "招标项目编号", "返利类型", "返利方式",
				"还款期数", "奖励金额", "资金类型", "计划奖励日期", "实际奖励日期" };
		String[] fields = { "bidPlanName", "bidPlanProjectNum",
				"rebateTypeName", "rebateWayName", "payintentPeriod",
				"incomeMoney", "fundTypeName", "intentDate", "factDate" };
		try {
			ExportExcel.export(tableHeader, fields, list, excelName,
					SlFundIntentPeriod.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void excelRaiseBpfundDetail() {
		...
		String[] tableHeader = { "序号", "招标项目名称", "招标项目编号", "返利类型", "返利方式",
				"募集期利率", "奖励金额", "资金类型", "计划奖励日期", "实际奖励日期" };
		String[] fields = { "bidPlanName", "bidPlanProjectNum",
				"rebateTypeName", "rebateWayName", "raiseRate", "incomeMoney",
				"fundTypeName", "intentDate", "factDate" };
		try {
			ExportExcel.export(tableHeader, fields, list, "募集期奖励台账",
					SlFundIntentPeriod.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void excelCouponsIncome() {
		...
		List<SlFundIntentPeriod> list = bpFundIntentService
				.getCouponsIncome(null);
		String[] tableHeader = { "序号", "投资人", "用户名", "优惠券收益奖励", "普通加息收益奖励",
				"累计收益奖励", "未收收益" };
		String[] fields = { "receiverName", "receiverP2PName", "couponsIncome",
				"addRateIncome", "sumIncome", "notIncome" };
		try {
			ExportExcel.export(tableHeader, fields, list, "优惠券收益查询",
					SlFundIntentPeriod.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String listPeriodbyOverDue() {
		...
		List<SlFundIntentPeriod> list = slFundIntentService
				.listOverDueByBidPlanIdAndpayintentPeriod(pb, map);
		List<SlFundIntentPeriod> sizelist = slFundIntentService
				.listOverDueByBidPlanIdAndpayintentPeriod(null, map);
	...
	public void listOverDueToExcel() {
		...
		List<SlFundIntentPeriod> list = slFundIntentService
				.listOverDueByBidPlanIdAndpayintentPeriod(pb, map);
		for (SlFundIntentPeriod sl : list) {
			BigDecimal total = sl.getPrincipalRepayment().getIncomeMoney().add(
					(sl.getLoanInterest().getIncomeMoney())).add(
					(sl.getConsultationMoney().getIncomeMoney())).add(
					(sl.getServiceMoney().getIncomeMoney())).add(
					(sl.getInterestPenalty().getIncomeMoney())).add(
					(sl.getPrincipalRepayment().getAccrualMoney())).add(
					(sl.getLoanInterest().getAccrualMoney())).add(
					(sl.getConsultationMoney().getAccrualMoney())).add(
					(sl.getServiceMoney().getAccrualMoney()));
			sl.setAllTotalMoney(total);
		}
		String[] tableHeader = { "序号", "借款人", "招标项目名称", "招标项目编号", "期数", "本金",
				"利息", "管理咨询费", "财务服务费", "补偿息", "逾期天数", "罚息", "合计", "计划还款日" };
		try {
	...
	public String createPunishByTiming(){
		slFundIntentService.createPunishByTiming();
		return SUCCESS;
	}
	
	public String getListLoanTrial(){
		List list = new ArrayList();
	    Map params = new HashMap();
	    String projectId = getRequest().getParameter("projectId");
	    String businessType = getRequest().getParameter("businessType");
	    if ((projectId != null) && (businessType != null) && (!"".equals(projectId)) && (!"".equals(businessType))) {
	      params.put("projectId", projectId);
	      params.put("businessType", businessType);
	      list = slFundIntentService.getFundListByParams(params);
	    }
	    StringBuffer buff = new StringBuffer("{\"success\":true,\"result\":");
	    JSONSerializer json = JsonUtil.getJSONSerializer(new String[] { "intentDate", "factDate", "interestStarTime", "interestEndTime" });
	    json.transform(new DateTransformer("yyyy-MM-dd"), new String[] { "intentDate", "factDate", "interestStarTime", "interestEndTime" });
	    buff.append(json.serialize(list) + "}");
