var getbplawsuitData = function(grid) {
	...
	var vRecords = grid.items.get(0).getStore().getRange(0,
			grid.items.get(0).getStore().getCount()); // 得到修改的数据（记录对象）
...
bplawsuitList = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
		...
		if (typeof(_cfg.litigationType) != "undefined") {
			this.litigationType = _cfg.litigationType;
		}
	...
	initUIComponents : function() {
		...
		this.grid_bplawsuitListInfo = new Ext.grid.EditorGridPanel({
			...
			store : new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({// this.ownerCt.ownerCt.ownerCt.ownerCt.PersonCarView.get(0)
					url : __ctxPath
							+ '/creditFlow/customer/enterprise/listBpCustEntLawsuit.do?enterpriseId='
							+ this.enterpriseId+'&litigationType='+this.litigationType,
					method : "POST"
				}),
				reader : new Ext.data.JsonReader({
							fields : Ext.data.Record.create([{
										name : 'lawsuitId',
										type : 'int'
									}, 'registerDate', 'registerReason','relatedMoney', 'role', 'recordUser','recordTime']),
							root : 'result'
						})
			}),
	...
	createbplawsuitListInfo : function() {
		...
		p.data = {};
		/*
		 * for ( var i = 1; i < keys.length; i++) { p.data[keys[i]] = '';
		 * p.data[keys[6]] = ''; }
		 */
		p.data[keys[0]] = null;
			...
			if (btn == "yes") {
				...
				for (var i = 0; i < s.length; i++) {
					...
					if (row.data.lawsuitId == null || row.data.lawsuitId == '') {
						...
						deleteFun(
								__ctxPath
										+ '/creditFlow/customer/enterprise/multiDelBpCustEntLawsuit.do',
								{
