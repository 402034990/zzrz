/**
 * @author:
 * @class EnterPriseAssureRecord
 * @extends Ext.Panel
 * @description [EnterPriseAssureRecord]管理
 * @company 北京互融时代软件有限公司
 * @createtime:
 */
EnterPriseAssureRecord = Ext.extend(Ext.Panel, {
...
/*		if(typeof(_cfg.customerType)!="undefined"){
		...
		// 调用父类构造
		EnterPriseAssureRecord.superclass.constructor.call(this, {
			id : 'EnterPriseAssureRecord',
			title : '保证人登记',
			region : 'center',
			layout : 'border',
	...
	initUIComponents : function() {
		...
		this.gridPanel = new HT.GridPanel( {
			...
			showPaging:true,
			autoScroll:true,
			url : __ctxPath + "/creditFlow/personAssureProjectListCreditProject.do?oppositeType=company_customer",
			fields : [ {
				name : 'projectId',
				type : 'long'
			...
			'intentDate','name','cardnumber','mortgagepersontypeforvalue',
			'ownJointMoney',
...
/*		var businessType=this.searchPanel.getCmpByName('businessType');
	...
	outToExcel:function(){
		...
		window.open(__ctxPath + '/creditFlow/personAssureOutExcelCreditProject.do?oppositeType=company_customer&businessType='+businessType+"&customerId="+customerId+"&startDate="+sdate+"&endDate="+edate,'_blank');
	}
