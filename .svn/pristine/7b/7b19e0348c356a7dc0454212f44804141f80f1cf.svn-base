 ...
 * @createtime:
 */
SlFundIntentPrincipalPayAll = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				{
					this.idstr=this.businessType;
				}
				else{this.idstr=this.businessType+this.type}
				// 初始化组件
				this.initUIComponents();
				// 调用父类构造
				SlFundIntentPrincipalPayAll.superclass.constructor.call(this, {
							id : 'SlFundIntentPrincipalPayAll'+this.businessType+this.type,
							title : '全部款项',
							region : 'center',
							layout : 'border',
							iconCls :'btn-team2',
			...
			initUIComponents : function() {
				...
				this.topbar = new Ext.Toolbar({
					items : [{
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_liushui_f_principalPay_'+this.idstr)?false:true,
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_liushuisee_f_principalPay_'+this.idstr)?false:true,
						handler : this.openliushuiwin1
			
					},new Ext.Toolbar.Separator({
						hidden : isGranted('_liushuisee_fprincipalPay__'+this.idstr)?false:true
					}),{
						iconCls : 'btn-ok',
						text :'核销',
						xtype : 'button',
						scope : this,
						hidden : isGranted('_ping_f_principalPay_'+this.idstr)?false:true,
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_uploaddownpz_f_principalPay_'+this.idstr)?false:true,
						handler : this.upload
			        	
			
					},new Ext.Toolbar.Separator({
						hidden : (isGranted('_uploaddownpz_f_principalPay_'+this.idstr)?false:true)||(isGranted('_previewpz_f_'+this.businessType)?false:true)
					}),{
						iconCls : 'btn-setting',
						text :'预览凭证',
						xtype : 'button',
						scope : this,
						hidden : isGranted('_previewpz_f_principalPay_'+this.idstr)?false:true,
				...
				this.gridPanel = new HT.GridPanel({
					...
					fields : [{
							...
							'overdueRate', 'accrualMoney', 'status','remark','businessType','projectId','tabType','orgName','companyId','payintentPeriod','ids','bidPlanId'],
					columns : [{
								header : 'fundIntentId',
...
//							}
							, {
									header : this.businessType=='SmallLoan'?'放款金额':'计划支出金额',
								dataIndex : 'payMoney',
								align :'right',
								summaryType: 'sum',
								width : 150,
								...
								header : this.businessType=='SmallLoan'?'计划放款日':'计划支出日期',
								width : 100,
								dataIndex : 'intentDate',
								align : 'center'
						//		sortable:true
							}, {
								header : this.businessType=='SmallLoan'?'实际放款日':'实际支出日期',
								width : 100,
								dataIndex : 'factDate'
						//		sortable:true
							}, {
				...
				if(this.businessType!=null){
					...
					switch (businessType) {
					  ...
					  case "Guarantee": sqlstr=" ";
					    break;
					  case "Pawn": sqlstr="&fundType=('pawnPrincipalLending')";
					    break;
