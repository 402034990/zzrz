						...
						items : [{
							...
							items : [{
										...
										listeners : {
															...
															var obj = Ext.getCmp('accountTypeid1');
															obj.enable();
															var arrStore = null;
															if(v==0){
...
//										fieldLabel : '账户类型'
				...
				this.topbar = new Ext.Toolbar({
							items : [{
										...
										xtype : 'button',
										scope : this,
										hidden : isGranted('_see_b')?false:true,
										handler : this.seeRs
									},new Ext.Toolbar.Separator({
										hidden :  isGranted('_see_b') ? false : true
									}), {
										...
										xtype : 'button',
										scope : this,
										hidden : isGranted('_zhlsdetail_b')?false:true,
				...
				this.gridPanel = new HT.GridPanel({
					...
					// 使用RowActions
					rowActions : false,
					id : 'SlBankAccountGrid',
					plugins: [summary],
					 viewConfig: {   
                        forceFit:false    //横向滚动条
                         },
					...
					fields : [{
							...
							'account', 'rawMoney', 'finalMoney', 'recordTimeString','remarks','address','finalDate','frozenfinalMoney','surplusfinalMoney','orgName'],
					columns : [{
...
//								}
			...
			removeRs : function(id) {
				$postDel({
							url : __ctxPath
									+ '/creditFlow/finance/multiDelSlBankAccount.do',
							ids : id,
							grid : this.gridPanel,
							error : '确定要删除？'
						});
