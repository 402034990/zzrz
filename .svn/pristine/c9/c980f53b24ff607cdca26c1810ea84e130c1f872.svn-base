...
public class BpCustEntLawsuitAction extends BaseAction{
	 ...
	 */
	public String list(){
		 String litigationType=this.getRequest().getParameter("litigationType");  
		...
		try {
			   if(litigationType!=null&&!"".equals(litigationType)&&litigationType!="undefined"){
				   list=bpCustEntLawsuitService.getAll(filter);
			   }else {
			...
			if(bplawsuitListData!=null&&!"".equals(bplawsuitListData)){
				...
				for (int i = 0; i < personCarInfoArr.length; i++) {
					...
					try {
						...
						if(litigationType!=null&&!"".equals(litigationType)&&litigationType!="undefined"){
							bpCustEntLawsuit.setLitigationType(litigationType);
						}
