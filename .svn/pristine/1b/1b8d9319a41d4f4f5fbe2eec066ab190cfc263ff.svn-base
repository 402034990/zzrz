...
public class VehicleAction extends CreditBaseAction {
	...
	public void seeVehicleForUpdate() throws Exception{
		...
		if(null!=list && list.size()!=0){
			...
			/*manufacturer = vProjMortCar.getManufacturer();
			if(manufacturer != 0){
				vCarDic = vehicleService.getProcessCarDataById(manufacturer);
				if(vCarDic != null){*/
					/*vProjMortCar.setCarNumber(vCarDic.getCarNumber());
					vProjMortCar.setCarStyleName(vCarDic.getCarStyleName());
					vProjMortCar.setCarModelName(vCarDic.getCarModelName());*/
			      if(null!=vProjMortCar.getDisplacement() && !"".equals(vProjMortCar.getDisplacement())){
			    	  vProjMortCar.setDisplacementValue(dictionaryService.get(vProjMortCar.getDisplacement().longValue()).getItemValue());  
			      }
			      if(null!=vProjMortCar.getSeating() && !"".equals(vProjMortCar.getSeating())){
			    		vProjMortCar.setSeatingValue(dictionaryService.get(vProjMortCar.getSeating().longValue()).getItemValue());  
			      }
					
				//	vProjMortCar.setConfiguration(vCarDic.getConfiguration());
				
//					if(null!=vProjMortCar.getCarColor() && !"".equals(vProjMortCar.getCarColor())){视图直接存入颜色
//						vProjMortCar.setCarColorValue(dictionaryService.get(vProjMortCar.getCarColor().longValue()).getItemValue());
//					}
			/*	}
			}*/
