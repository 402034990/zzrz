 ...
 * @createtime:
 */
CarMortgageManagement = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		CarMortgageManagement.superclass.constructor.call(this, {
			id:'CarMortgageManagement'+this.businessType,
...
//			}
       ...
       this.reader = new Ext.data.JsonReader({
			...
			totalProperty : "totalCounts"
						...
						name : 'id',
						type : 'int'
					}, 'assureofnameEnterOrPerson','carManufacturer','carStyle', 'businessType',
			           'finalprice','assuremoney','isunchain','status','isStorage',
			           'storageNum','isOut','outNum','mortgageTypeName','carModel',
			           'isHander','handerTime','projname','projid',
			           'projnum','assuretypeidValue','mortgageid']);

       this.jStore_mortgage = new Ext.data.GroupingStore({
			url : __ctxPath +'/credit/mortgage/getCarData.do?businessType='+this.businessType,
			reader : this.reader,	
			autoLoad:true,
			groupField : 'projid'
		});
		...
		this.gridPanel = new HT.GridPanel( {
			id:'CarMortgageManagementViewGrid',
			region : 'center',
			tbar : this.topbar,
...
//			}
	...
	storage :function(){
		...
		if (null == selected) {
		   ...
		   var carId = selected.get('id');
		   var  isstorage= selected.get('isStorage');
		   if(null==isstorage || ""==isstorage ||  isstorage==0){
		   		new CarStorageWindow({
			    	typedata:'1',
				    carId:carId
		    	}).show()
		   }else{
		   		Ext.ux.Toast.msg('操作提示','此车已进行入库操作');
		   }
		}
	},
	carout:function(){
	...
	seeMortgageInfo : function() {
				...
				if (null == selected) {
					...
					alert("3   "+selected.get('businessType'))
					var businessType = selected.get('businessType');
