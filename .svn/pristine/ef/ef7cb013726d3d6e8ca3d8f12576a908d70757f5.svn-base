...
CreditDocumentManagerZW=Ext.extend(Ext.Panel,{
	...
	initUIComponents:function(){
		...
		this.topBar =  new Ext.Toolbar({
			//如果要放开新增按钮151行注释的也要放开
			items:[{
				xtype : 'button',
				iconCls : 'btn-add',
				text : '新增模板类型',
				tooltip : '新增模板类型信息',
				hidden : isGranted('_create_cdm_'+businessType)?false:true,
				scope:this,
				handler : function() {
				...
				tooltip : '修改模板类型信息',
				iconCls : 'btn-edit',
				hidden : isGranted('_edit_cdm_'+businessType)?false:true,
				scope : this,
				handler : function(){
				...
				tooltip : '删除模板类型',
				iconCls : 'btn-del',
				hidden : this.isGranted('_remove_cdm_'+businessType)?false:true,
				scope : this,
				handler : function(){
					if(typeof(this.nodeObj) != "undefined"){
						if(this.nodeObj.attributes.parentid == 0){
		...
		var tbar=new Ext.Toolbar({
			items:[{
					...
					iconCls:'btn-add',
					scope:this,
					hidden : isGranted('_create_cddm_'+businessType)?false:true,
					handler:function(){
						if(typeof(this.nodeObj) != "undefined"){
							this.addTemplate(this.nodeObj,this.documentTreePanel,this.businessType);
						}else
				...
				},
				{
					text:'修改模板文档项',
					iconCls:'btn-edit',
					hidden : isGranted('_edit_cddm_'+businessType)?false:true,
					scope:this,
					handler:function(){
				...
				},
				{
					text:'删除模板文档项',
					iconCls:'btn-del',
					hidden : isGranted('_remove_cddm_'+businessType)?false:true,
					scope:this,
					handler:function(){
						var rows = this.centerPanel.getSelectionModel().getSelections();
						if(rows.length==0){
