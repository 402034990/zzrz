...
public class ProDefinitionAction extends BaseAction {
	...
	public String listByProTypeId() {
		...
		String type = getRequest().getParameter("typeKey");
		List<ProDefinition> list=null;
		if(proTypeId!=null&&!"".equals(proTypeId)){
			
			list = proDefinitionService.getNormalFlowByProTypeId(new Long(proTypeId),type);
		}else{
			list = proDefinitionService.getNormalFlowByProTypeId(null,type);
		}
...
//		filter.addParamValue(ProcessRun.RUN_STATUS_RUNNING);
	...
	public String isStart(){
		...
		String flowKey=this.getRequest().getParameter("flowKey");//流程key
		String flowId=this.getRequest().getParameter("flowId");//流程Id
		if(null!=flowKey && !"".equals(flowKey)){
			pdf=proDefinitionService.getDefByKey(flowKey);
		}else if(null!=flowId && !"".equals(flowId)){
			pdf=proDefinitionService.get(Long.valueOf(flowId));
		}
		if(null!=pdf){
			jsonString="{success:true,status:"+pdf.getStatus()+"}";
		}else{
			jsonString="{success:true,status:2}";//2代表流程没有记录
		}
		return SUCCESS;
	}
}
