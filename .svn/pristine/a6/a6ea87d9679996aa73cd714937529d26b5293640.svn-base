/**
 * @author
 * @class AddrateBpFundIntent
 * @extends Ext.Panel
 * @description [PlBidInfo]管理
 * @company 智维软件
 * @createtime:
 */
AddrateBpFundIntent = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		AddrateBpFundIntent.superclass.constructor.call(this, {
					id : 'AddrateBpFundIntent_'+this.userType+this.Type,
					title : this.title,
	...
	initUIComponents : function() {
		...
		this.gridPanel = new HT.GridPanel({
			...
			id : 'PlBidInfoGrid',
			plugins : [summary],
			url : __ctxPath + "/creditFlow/finance/getCouponsListSlFundIntent.do?fundType=commoninterest",
			fields : [{
						name : 'infoId',
						type : 'int'
	...
	distributeMoney :function(){
		...
		if (s <= 0) {
			...
			Ext.Msg.confirm('派息确认','此次派息系统将会检查该项目该期尚未派息的奖励进行派息',function(btn){
		    	...
		    	Ext.Ajax.request({
					url : __ctxPath
							+ "/creditFlow/finance/distributeMoneyBpFundIntent.do?planId="+planId+"&fundType=commoninterest&payintentPeriod="+payintentPeriod,
					method : 'post',
	...
	couponsExcel : function(){
					...
					window.open( __ctxPath +"/creditFlow/finance/excelCouponsBpfundDetailSlFundIntent.do?fundType=commoninterest&bidName="+bidName+"&bidProNumber="+bidProNumber+"&" +
