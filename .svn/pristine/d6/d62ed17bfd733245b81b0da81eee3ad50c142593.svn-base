 ...
 * @company 智维软件
 */
selectAccountlForm1 = function(funname){
		var isShow=false;
			if(RoleType=="control"){
			  isShow=true;
			}
	...
	var gridPanel = new HT.GridPanel({
					...
					fields : [{
							...
							'account', 'rawMoney', 'finalMoney', 'recordTimeString','sumbankname','orgName'],
					columns : [{
									...
									listeners:{
									     'rowdblclick' : function(grid,rowIndex,e) {
		                                                ...
		                                                grid.getSelectionModel().each(function(rec) {
				                                       	funname(rec.data.account,rec.data.bankName,rec.data.name,rec.data.bankAccountId,rec.data.finalMoney,rec.data.sumbankname);
			                      	                    });
