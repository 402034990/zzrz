	...
	initUI:function(){
		...
		//搜索用户列
	 	...
	 	this.gridPanel=new HT.GridPanel({
	 		...
	 		fields :[{name : 'userId',type : 'int'}, 'fullname', 'title', 'mobile','userNumber','shopId','jobUser','jobUserId','shopName'],
	 		columns:[
	 			{
					header : "用户名",
					dataIndex : 'fullname',
...
//				var jobUser = this.jobUser.split(',');
	...
	gridPanelRowClick:function(grid,rowIndex,e){
		...
		for(var i= 0 ; i<rows.length ; i++){
			...
			var jobUser = rows[i].data.jobUser;
			var jobUserId = rows[i].data.jobUserId;
			var shopId = rows[i].data.shopId;
			var shopName = rows[i].data.shopName;
			...
			if(!isExist){
				var newData = {userId : userId,fullname : fullname,jobUser:jobUser,jobUserId:jobUserId,shopId:shopId,shopName:shopName,title:title};
				var newRecord = new selStore.recordType(newData);
				this.selGridPanel.stopEditing();
				selStore.add(newRecord);
			}
	...
	confirm:function(){
		...
		if(this.single){//选择单个用户
			...
			for(var i = 0 ; i<selStore.getCount(); i++){
				...
				jobUser += selStore.getAt(i).data.jobUser;
				jobUserId += selStore.getAt(i).data.jobUserId;
				shopId += selStore.getAt(i).data.shopId;
				shopName += selStore.getAt(i).data.shopName;
				users.push(selStore.getAt(i).data);
			}
		}

		if (this.callback){
			this.callback.call(this.scope, userIds, fullnames,jobUser,jobUserId,shopId,shopName,users);
		}
		this.close();
	}
});