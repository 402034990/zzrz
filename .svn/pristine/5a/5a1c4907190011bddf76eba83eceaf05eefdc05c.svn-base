	...
	constructor : function(_cfg) {
		...
		switch (this.projectStatus) {
			...
			case 15 :
				...
				this.tabIconCls = "btn-tree-team60";
				this.isHiddenBd = false;
				this.isHiddenAn = false;
				break;
		}
			...
			this.topbar = new Ext.Toolbar({
		     ...
		     this.topbar = new Ext.Toolbar({
				...
				items : [{
					...
					text : '编辑项目',
					xtype : 'button',
					hidden : isGranted('_detailPro_p' + this.projectStatus) ? false : true,
					scope : this,
					handler : function() {
						detailPro(this.gridPanel, 'SmallLoanProjectInfoEdit_')
					}
					...
					xtype : 'button',
					scope : this,
					hidden : isGranted('_yjysm_p'+ this.projectStatus) ? false 	: true,
					handler : this.flowRecords
				},new Ext.Toolbar.Separator({
					hidden : !isGranted('_yjysm_p' + this.projectStatus) || !isGranted('_showFlowImg_p' + this.projectStatus)
				}), {
					iconCls : 'btn-example',
					text : '流程示意图',
					xtype : 'button',
		...
		this.gridPanel = new HT.GridPanel({
			...
			fields : [{
				...
				type : 'long'
					...
					'departMentName','departId','ownJointMoney','productId','runId','repaymentDate','productTypeName'],
