...
VFundDetail = Ext.extend(Ext.Panel, {
			...
			constructor : function(_cfg) {
				...
				VFundDetail.superclass.constructor.call(this, {
							...
							title : (this.businessType=='SmallLoan' || this.businessType=='Pawn')?'本息收支对账日志':'融资利息台账日志',
							region : 'center',
							layout : 'border',
							iconCls :'btn-team2',
...
//				                        arrStore.load();
								...
								items : [{
										...
										store : new Ext.data.SimpleStore({
											fields : ["name", "id"],
											data : [["贷款利息收取", "loanInterest"], 
													["贷款本金偿还", "principalRepayment"],
													["提前还款违约金", "interestPenalty"]
												  ]
										}),
										triggerAction : "all",
...
//									}]
				...
				this.topbar = new Ext.Toolbar({
					items : [{
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_cxdz_f_'+this.businessType)?false:true,
				...
				this.gridPanel = new HT.GridPanel({
	            	...
	            	isautoLoad:true,
	            	loadMask : true,
					url : __ctxPath + "/creditFlow/finance/listVFundDetail.do?businessType="+this.businessType,
					fields : [{
								name : 'fundDetailId',
								type : 'int'
							}, 'projectName','projectNumber', 'intentincomeMoney','fundTypeName', 'intentDate',
							'intentpayMoney', 'operTime', 'factDate','funType',
							'afterMoney', 'myAccount','flatMoney', 'isOverdue',
							'operTime', 'transactionType', 'qlideincomeMoney','qlidepayMoney','businessType','fundIntentId','fundQlideId','checkuser','iscancel','cancelremark','orgName'],
					columns : [{
								header : 'fundIntentId',
