...
slEnterPriseProcreditMaterialsPanel = Ext.extend(Ext.Panel, {
	        projId:null,
	        gridPanel:null,
			...
			constructor : function(_cfg) {
				...
				slEnterPriseProcreditMaterialsPanel.superclass.constructor.call(this, {
						    layout:'anchor',
		  					 anchor : '100%',
							 modal : true,
			...
			initUIComponents : function() {
				...
				var isHidden_materials=this.isHidden_materials;
...
//				        plugins : [checkColumn],
				        clicksToEdit :1,
				        tbar : isHidden_materials? null :this.topbar,
				        cm: new xg.ColumnModel([
				            ...
				            {header: "线下材料份数",fixed: true,width:93,dataIndex: 'datumNumsOfLine',renderer : fenRenderer,hidden:this.isp2pEdit,editor : isHidden?null:{xtype:'numberfield',readOnly : isHidden}},
				            {header: "是否收到",fixed: true,width:67,dataIndex: 'isReceive',renderer : receiveRender,hidden:this.editConfirmInfo?false:!this.showConfirmInfo,editor : isHidden?null:
				            ...
				            //是否已提交  add by gao
				            {header: "是否提交",fixed: true,width:67,dataIndex: 'isPigeonhole',renderer : receiveRender,hidden:this.editConfirmInfo?false:!this.showConfirmInfo,editor: this.editConfirmInfo?
				            new Ext.form.ComboBox({
												mode : 'local',
												editable : true,
												store : new Ext.data.SimpleStore({
				            ...
				            {									
									...
									sortable : true,
							        width : 82,
							        hidden : this.editConfirmInfo?false:!this.showConfirmInfo,
				            ...
				            {header: "上传或下载",fixed: true,width:86,dataIndex: 'uploadOrDown',hidden:this.isp2pEdit,renderer:function(){
				                if(isHidden){
                        ...
                        listeners : {
                		   'cellclick' : function(grid,rowIndex,columnIndex,e){
									...
									if("uploadOrDown"==fieldName){
										 var loadData=null;
										 var uploadFileCounts=100;
										 if(typeof(this.ownerCt.isHiddenArchive)!='undefined' && !this.ownerCt.isHiddenArchive && typeof(this.ownerCt.isProjectInfoEdit)!='undefined' && !this.ownerCt.isProjectInfoEdit){//项目保前归档节点并且不在项目信息编辑页面中
			...
			createRs : function() {
				new SlProcreditMaterialsEnterpriseForm({projectId : this.projId,operateObj:this.gridPanel,show:false,businessType:this.businessType,operationType:this.operationType,isDelete:false}).show();
			},
			updateRs : function(){
			    new SlProcreditMaterialsEnterpriseForm({projectId : this.projId,operateObj:this.gridPanel,show:true,businessType:this.businessType,operationType:this.operationType,isDelete:true}).show(); //改回原来的  gao
			},
			imageView :  function(){
