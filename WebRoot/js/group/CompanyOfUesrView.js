...
//						text:'添加代理商',
			...
			items:[{
				...
				border : false,
				treeType:'org',
				height : Ext.getBody().getViewSize().height-145,
				autoScroll : true,
				scope : this,
				url : __ctxPath+'/system/getBranchCompanyOrganization.do',
				onclick:this.posTreeClick,
	...
	addShop:function(){/*
		...
		var  posTreePanel=Ext.getCmp('OrganizationType');
		var  leftSelectNode=posTreePanel.selectedNode;
		var  orgType=leftSelectNode.attributes.orgType;
		if(null!=leftSelectNode){
		 
			if(orgType==3){
				Ext.ux.Toast.msg('操作','代理商下不允许再添加代理商!');
				return;
			}
		    parentName=this.getAllParentNodeName(leftSelectNode);
		    orgId=posTreePanel.selectedNode.id;
			...
			var pn="";
		    pn=parentName+orgText;
			new ShopForm({'parentName':pn,'orgSupId':orgId,'addCompany':true,'callback':reloadData}).show();
		}else{
	...
	addUser:function(){
		...
		if(leftSelectNode){
		   ...
		   /* if(orgType!=2){
		    	...
		    	var parNode=this.getParentNode(leftSelectNode,'0');//第二个参数代表是查代理商还是分公司id(0分公司,1代理商)
		    	var shopNode=this.getParentNode(leftSelectNode,'1');//代理商
		    	var grid = this.userGridPanel;
		    	var shopId="";
		    	if(shopNode){
		    		shopId=shopNode.attributes.id;
		    	}
		    	App.clickTopTab('UserFormPanel',{gridObj:grid,shopId:shopId,companyId:parNode.attributes.id});
		   // }
		}else{
