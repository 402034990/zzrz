//定量指标分类。。。copy
IndicatorManagement1 =Ext.extend(Ext.Panel,{
	constructor:function(config){
		Ext.applyIf(this,config);
		this.initUIComponents();
		IndicatorManagement1.superclass.constructor.call(this,{
		    id:'IndicatorManagement1',
			layout:'border',
			title:'定量指标体系管理',
	...
	initUIComponents:function(){
       ...
       this.jStore_indicator = new Ext.data.GroupingStore({
			proxy : new Ext.data.HttpProxy({url :  __ctxPath+'/creditFlow/creditmanagement/list1Indicator.do?type=dl'}),
			reader : this.reader,
				...
				handler : function() {
					...
					var fPanel_add = new Ext.FormPanel({
						...
						items : [ {
							...
							items : [ {
								items : [{
										...
										xtype :'hidden',
										name : 'indicator.ptype',
										value : ptype
								},{
										xtype :'hidden',
										name : 'indicator.operationType'
								},{
										xtype :'hidden',
										name : 'indicator.elementCode'
								},{
										xtype : 'textfield',
										fieldLabel : '要素名称',
										name : 'indicatorType',
									...
									onTriggerClick : function() {
												...
												var IndicatorUpdateNew=function(obj){
													op.getCmpByName('indicator.indicatorName').setValue("");
													op.getCmpByName('indicator.operationType').setValue("");
													op.getCmpByName('indicator.elementCode').setValue("");
													
													if(obj.depict!=0 && obj.depict!="")	
													op.getCmpByName('indicator.indicatorName').setValue(obj.depict);
													if(obj.operationTypeKey!=0 && obj.operationTypeKey!="")	
													op.getCmpByName('indicator.operationType').setValue(obj.operationTypeKey);
													if(obj.code!=0 && obj.code!="")	
													op.getCmpByName('indicator.elementCode').setValue(obj.code);
													
											}
												selectElementCode(IndicatorUpdateNew);
						...
						buttons : [ {
							...
							formBind : true,
							handler : function() {
								var vDates=getOptionGridDate1(fPanel_add.getCmpByName("gPanel_option"))
									...
									items : [ {
										...
										items : [ {
												items : [{
														...
														xtype :'hidden',
														name : 'indicator.ptype',
														value : obj.data.ptype
												},{
														xtype :'hidden',
														name : 'indicator.operationType',
														value : obj.data.operationType
												},{
														xtype :'hidden',
														name : 'indicator.elementCode',
														value : obj.data.elementCode
												},{
														xtype : 'textfield',
														fieldLabel : '要素名称',
														name : 'indicatorType',
									...
									buttons : [ {
										...
										handler : function() {
											fPanel_update.getForm().submit({
												...
												waitTitle : '连接',
												waitMsg : '消息发送中...',
												params:{optionStr:getOptionGridDate2(fPanel_update.getCmpByName("gPanel_option"))},
												success : function() {
			...
			this.button_seeIndicator = new Ext.Button({
				...
				handler : function() {
					...
					if (null == selected) {
						...
						var id = selected.get('indicatorId');
						var gPanel_option=new OptionList1({isHidden:true})
						gPanel_option.getCmpByName("gPanel_option").getStore().baseParams['indicatorId']=id
						gPanel_option.getCmpByName("gPanel_option").getStore().load()
		...
		this.dic_TreeLoader = new Ext.tree.TreeLoader({
				dataUrl :  __ctxPath+"/creditFlow/creditmanagement/storeListIndicatorStore.do?type=dl"
			})
			...
			items : [{
					...
					listeners : {
						'contextmenu' : function(node,e) {
							nodeSelected = node;
							nodeSelected.select();
							setCatalogMenuFun2(node,e,centerPanel,"dl");
						},
						'click' : function(node) {
