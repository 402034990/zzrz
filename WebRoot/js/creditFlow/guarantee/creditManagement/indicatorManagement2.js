//定量指标分类。。。copy
IndicatorManagement2 =Ext.extend(Ext.Panel,{
	constructor:function(config){
		Ext.applyIf(this,config);
		this.initUIComponents();
		IndicatorManagement2.superclass.constructor.call(this,{
		    id:'IndicatorManagement2',
			layout:'border',
			title:'定性指标体系管理',
	...
	initUIComponents:function(){
       ...
       this.jStore_indicator = new Ext.data.GroupingStore({
				...
				handler : function() {
					...
					var fPanel_add = new Ext.FormPanel({
						...
						buttons : [ {
							...
							formBind : true,
							handler : function() {
								var vDates=getOptionGridDate2(fPanel_add.getCmpByName("gPanel_option"))
		...
		this.dic_TreeLoader = new Ext.tree.TreeLoader({
				dataUrl :  __ctxPath+"/creditFlow/creditmanagement/storeListIndicatorStore.do?type=dx"
			})
			...
			items : [{
					...
					listeners : {
						'contextmenu' : function(node,e) {
							nodeSelected = node;
							nodeSelected.select();
							setCatalogMenuFun2(node,e,centerPanel,"dx");
						},
						'click' : function(node) {
