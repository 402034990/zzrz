...
var addCreditRatingEBOfCustomer = function (piKey, customerObj, jStore_creditRating, customerType) {
...
var countResult = function (sub, win, jStore_creditRating) {
	...
	var score = 0;
	var maxScore=0;
	var CompDom = Ext.getCmp("gPanel_enterprise").getEl().dom
	...
	for (var i =0; i <getCK.length; i++) {
		...
		if(whichObj.type == "radio" && whichObj.name.indexOf("jumpPath_")==-1 && whichObj.name.indexOf("customerType")==-1  && whichObj.name.indexOf("enterprise.isLicense")==-1 && whichObj.name.indexOf("person.ispublicservant")==-1 && whichObj.name.indexOf("person.isheadoffamily")==-1) {
