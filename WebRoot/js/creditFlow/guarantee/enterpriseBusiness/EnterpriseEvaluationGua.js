EnterpriseEvaluationGua = Ext.extend(Ext.Panel, {
            isHidden:false,
            customerObj:null,
            customerId:null,
	        customerType:null,
			constructor : function(_cfg) {
	             ...
	             Ext.Ajax.request({
	                   url: __ctxPath+'/creditFlow/common/getCustomerShareequity.do',
	                   method : 'POST',
	                   params : {
				...
				EnterpriseEvaluationGua.superclass.constructor.call(this, {
							border:false,
							items : [this.gPanel_creditRating]
						})
			},
			initUIComponents : function() {
			
                var jStore_creditRating = new Ext.data.JsonStore(getCreditRatingStoreCfg(null,this.customerId,this.customerType));
		       // var grid=this.gPanel_creditRating;
		        //alert(grid)
				...
				var button_addCreditRating = new Ext.Button({
					...
					scope : this,
					handler : function() {
						addCreditRatingEBOfCustomer(0, customerObj, jStore_creditRating, (typeof(customerObj.data.enterprisename)!='undefined')?'企业':'个人');
					}
				});
			...
			var cModel_enterprise = new Ext.grid.ColumnModel([
				...
				this.tbar=new Ext.Toolbar({
					items:this.isHidden?[button_see]:[
				...
				this.gPanel_creditRating = new Ext.grid.GridPanel({
