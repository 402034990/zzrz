 ...
 * @createtime:
 */
ContractManager = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ContractManager.superclass.constructor.call(this, {
	...
	initUIComponents : function() {
		...
		this.topbar = new Ext.Toolbar({
			items : [{
				...
				text : '查看项目详情',
				xtype : 'button',
				hidden : isGranted('_seeProjectInfo_cm')?false:true,
				scope:this,
		...
		this.gridPanel = new HT.GridPanel({
			...
			hiddenCm:true,
			tbar : this.topbar,
			url : __ctxPath + "/creditFlow/fileUploads/contractListFileForm.do?isGrantedShowAllProjects="+isGranted('_seeAllPro_p1')+"&businessType="+this.businessType,
			fields : [ {
				name : 'runId',
				type : 'long'
			}, 'projectId', 'projectNumber', 'projectName', 'projectMoney',
					'businessManagerValue', 'activityName', 'createtime', 'endDate', 'contractCount',
					'morContractCount','businessType','companyName','otherFileCount','bzContractCount','repaymentDate',
					'loanStartDate','appUserName','startDate','id'],
			columns : [ {
				header : 'id',
				dataIndex : 'runId',
				hidden : true
				...
				renderer : function(value,metaData, record,rowIndex, colIndex,store) {
					if(businessType=='SmallLoan'){
						return record.data.appUserName
					}else{
						return value
					}
				}
				...
				header : this.businessType=='SmallLoan'?'贷款开始时间':'开始时间',
				...
				header : this.businessType=='SmallLoan'?'贷款结束时间':'结束时间',
	...
	cellClick : function(grid,rowIndex,columnIndex,e){
		...
		if(9==columnIndex){
            ...
            if(businessType=="SmallLoan"){
            	piKey= grid.getStore().getAt(rowIndex).get('id');
            	var sContractView=new SlContractView({projId:piKey,businessType:businessType,isSignHidden:false,isqsEdit:true,htType:'loanContract',isHiddenAffrim:true,isgdEdit:true,isHiddenAddBtn:false,isHiddenDelBtn:false,isHiddenEdiBtn:false,sprojectId:grid.getStore().getAt(rowIndex).get('projectId')})
            	...
            	var flContractView=new SlContractView({projId:piKey,businessType:businessType,isSignHidden:false,isqsEdit:true,htType:'pawnContract',isHiddenAffrim:true,isgdEdit:true,isHiddenAddBtn:false,isHiddenDelBtn:false,isHiddenEdiBtn:false,isHiddenBZ:true,isHiddenDZY:true,isHiddenDW:false})
            	var win=new Ext.Window({
					height:200,
					width:800,
					autoScroll:true,
