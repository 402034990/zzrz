						...
						if (s <= 0) {
							...
							if(isOtherFlow==1){//isOtherFlow==1表示当前项目已经启动了展期流程
								...
								Ext.Ajax.request({
									...
									params : {flowKey:'repaymentAheadOfTimeFlow'},
									method : 'POST',
					...
					handler : function() {
						...
						if (s <= 0) {
							...
							if(isOtherFlow==1){//isOtherFlow==1表示当前项目已经启动了展期流程
								...
								Ext.Ajax.request({
									url : __ctxPath +"/flow/isStartProDefinition.do",
									params : {flowKey:'smallLoanPostponedFlow'},
									method : 'POST',
									success : function(response) {
										var obj=Ext.util.JSON.decode(response.responseText);
										if(obj.status==0){//禁用
					...
					handler : function() {
						...
						if (s <= 0) {
							...
							if(isOtherFlow==1){//isOtherFlow==1表示当前项目已经启动了展期流程
								...
								Ext.Ajax.request({
									url : __ctxPath +"/flow/isStartProDefinition.do",
									params : {flowKey:'alterAccrualFlow'},
									method : 'POST',
									success : function(response) {
										var obj=Ext.util.JSON.decode(response.responseText);
										if(obj.status==0){//禁用
					...
					handler : function() {
						...
						if (s <= 0) {
							...
							Ext.ux.Toast.msg('操作信息', '正在进行展期流程，不能启动新的提前还款流程！');
								...
								if(isOtherFlow==4){
									Ext.ux.Toast.msg('操作信息', '已启动结清流程不能重复启动');
							        return false;
								}
								Ext.Ajax.request({
									url : __ctxPath +"/flow/isStartProDefinition.do",
									...
									success : function(response) {
										...
										if(obj.status==0){//禁用
											...
											var gridPanel=this.gridPanel
			 								Ext.Msg.confirm('信息确认', '是否启动结清流程', function(btn) {
								       		 	if (btn == 'yes') {
													Ext.Ajax.request( {
														url : __ctxPath +"/project/StartRbjrSettlementFlowSlSmallloanProject.do",
														params : {projectId:record.data.projectId,businessType:record.data.businessType,
														          processType:"rbjrSettlementFlow",fundId:record.data.id},
														method : 'POST',
														success : function(response) {
