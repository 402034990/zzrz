...
SlFundintentUrgeAllForm = Ext.extend(Ext.Panel, {
	...
	initUIComponents : function() {
		...
		var requestUrl = __ctxPath
					+ '/creditFlow/finance/slFundIntentCountInfoSlFundIntent.do?businessType='+this.businessType
		Ext.Ajax.request({
			url : requestUrl,
			method : 'POST',
			scope : this,
		...
		var slProjectoverdueintent=infoObj.slProjectoverdueintent
		var urgeformpanel = new Ext.FormPanel({
...

			{
						...
						items : [{
							...
							boxLabel : '发送邮件【系统】',
							name : '1',
							id : "11all" +fundIntentId+istype,
							inputValue : true,
							checked : true,
							listeners : {
								scope : this,
								check : function(obj, checked) {
									var flag = Ext.getCmp("11all"
											+ fundIntentId+istype).getValue();
									if (flag == true) {
										this
												.getCmpByName('slFundintentUrge.urgeType')
												.setValue(1);
										this
												.getCmpByName('slFundintentUrge.urgeContext')
												.setValue("根据       字第　　 号借款合同，你单位(个人)于 "
														+ projectStartDatearray[0]
														+ "年"
														+ projectStartDatearray[1]
														+ "月"
														+ projectStartDatearray[2]
														+ "向我公司所借 贷款（大写）"
														+ payInMoney
														+ "，现尚余"
														+ slProjectoverdueintent
														+ "元将于"
														+ intentDatearray[0]
														+ "年"
														+ intentDatearray[1]
														+ "月"
														+ intentDatearray[2]
														+ "日到期，请抓紧筹措资金按期归还借款，如到期未还，我公司即依照借款合同有关规定予以处理。 ");
									}
								}
							}
						}]
						...
						items : [{
							...
							boxLabel : '发送短信【系统】',
							name : '1',
							id : "12all" + fundIntentId+istype,
							inputValue : false,
							listeners : {
								scope : this,
								check : function(obj, checked) {
									var flag = Ext.getCmp("12all" + fundIntentId+istype).getValue();
									if (flag == true) {
										var  upfujian = 	this.getCmpByName('upfujian');
										var  upfujianbutton = 	this.getCmpByName('upfujianbutton');
										this.getCmpByName('slFundintentUrge.urgeType').setValue(2);
										this.getCmpByName('slFundintentUrge.urgeContext') .setValue("现尚余" + slProjectoverdueintent + "元将于" + intentDatearray[0] + "年" + intentDatearray[1] + "月" + intentDatearray[2] + "日到期，请抓紧筹措资金按期归还借款，如到期未还，我公司即依照借款合同有关规定予以处理。 ");
										upfujian.remove(upfujianbutton, true);
										upfujian.doLayout();
									}else{
										var  upfujian = 	this.getCmpByName('upfujian');
										var  upfujianbutton = 	this.getCmpByName('upfujianbutton');
										if(upfujianbutton=='undefined' ||  upfujianbutton == null){
						...
						items : [{
							...
							boxLabel : '电话催收',
							name : '1',
							id : "13all" + fundIntentId+istype,
							inputValue : false,
							listeners : {
								scope : this,
								check : function(obj, checked) {
									var flag = Ext.getCmp("13all" + fundIntentId+istype).getValue();
									if (flag == true) {
						...
						items : [{
							...
							boxLabel : '纸质邮件',
							name : '1',
							id : "14all" +fundIntentId+istype,
							inputValue : true,
							listeners : {
								scope : this,
								check : function(obj, checked) {
									var flag = Ext.getCmp("14all"
											+ fundIntentId+istype).getValue();
									if (flag == true) {
										this
												.getCmpByName('slFundintentUrge.urgeType')
						...
						items : [{
							...
							value : "根据       字第　　 号借款合同，你单位(个人)于" + projectStartDatearray[0] + "年" + projectStartDatearray[1] + "月" + projectStartDatearray[2] + "向我公司所借 贷款（大写）" + payInMoney + "，现尚余" + slProjectoverdueintent + "元将于" + intentDatearray[0] + "年" + intentDatearray[1] + "月" + intentDatearray[2] + "日到期，请抓紧筹措资金按期归还借款，如到期未还，我公司即依照借款合同有关规定予以处理。",
							xtype : 'textarea',
							anchor : '98%'
						}]
					},{
