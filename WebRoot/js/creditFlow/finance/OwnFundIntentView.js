...
OwnFundIntentView = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
		...
		if (typeof(_cfg.preceptId) != "undefined") {
			this.preceptId = _cfg.preceptId;
		}
		...
		this.gridPanel = new HT.EditorGridPanel({
			...
			columns : [{
					...
					/*, {
						...
						align : 'right',
						summaryType : 'sum',
					//	hidden : this.isHiddenAccrual==false?false:true,
						width : 100,
						renderer : function(value, metaData, record, rowIndex,
								colIndex, store) {
							var flag1 = 0; // incomeMoney
	...
	createRs : function() {
		var payintentPeriod = (typeof(this.object) != 'undefined'
				? this.object.getCmpByName('slSmallloanProject.payintentPeriod').getValue()
	...
	autocreate : function() {
		...
		var businessType = this.businessType;
		var isActualDay="no";//默认按30天
		var operationType = null;
		var investorIds = null;
		var isPreposePayConsultingCheck = null;
		var isActualDay = this.object .getCmpByName("slSmallloanProject.isActualDay").getValue();
		var accrualtype = this.object .getCmpByName("slSmallloanProject.accrualtype").getValue();
		var payaccrualType = this.object .getCmpByName("slSmallloanProject.payaccrualType") .getValue();
		var isStartDatePay = this.object.getCmpByName("slSmallloanProject.isStartDatePay") .getValue();
		var startDate = this.object .getCmpByName("slSmallloanProject.startDate").getValue();
		var payintentPerioDate = this.object .getCmpByName("slSmallloanProject.payintentPerioDate").getValue();
		var payintentPeriod = this.object.getCmpByName("slSmallloanProject.payintentPeriod").getValue();
		var intentDate = this.object.getCmpByName("slSmallloanProject.intentDate").getValue();
		var dayOfEveryPeriod = this.object.getCmpByName("slSmallloanProject.dayOfEveryPeriod").getValue();
		var isPreposePayAccrual = this.object.getCmpByName("isPreposePayAccrualCheck").getValue();
		var isInterestByOneTime = this.object.getCmpByName('slSmallloanProject.isInterestByOneTime').getValue();
		var dateMode = this.object.getCmpByName('slSmallloanProject.dateMode').getValue();
		var dayAccrualRate = this.object.getCmpByName('slSmallloanProject.dayAccrualRate').getValue();
	    var balanceRate=this.object.getCmpByName("slSmallloanProject.balanceRate").getValue();
	    var accrual=this.object.getCmpByName("slSmallloanProject.accrual").getValue();
	    var yearAccrualRate=this.object.getCmpByName("slSmallloanProject.yearAccrualRate").getValue();
	    var projectMoney = this.object.getCmpByName("slSmallloanProject.projectMoneyPass").getValue();
		...
		if (calcutype == "SmallLoan") {
			...
			if (dayAccrualRate == "" || dayAccrualRate == null) {
				message = "贷款利率";
			}
		...
		var params1 = {
			projectId : projId,
			'ownBpFundProject.isActualDay' : isActualDay,
