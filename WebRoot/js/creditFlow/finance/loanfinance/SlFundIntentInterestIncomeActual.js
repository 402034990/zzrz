 ...
 * @createtime:
 */
SlFundIntentInterestIncomeActual = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		SlFundIntentInterestIncomeActual.superclass.constructor.call(this, {
			id : 'SlFundIntentInterestIncomeActual'+this.businessType,
			title : '实收款项',
			region : 'center',
			layout : 'border',
			iconCls : 'btn-team2',
	...
	initUIComponents : function() {
		...
		this.gridPanel = new HT.GridPanel({
			...
			rowActions : false,
			loadMask : true,
			id : 'SlFundIntentGrid4interestincome',
			isautoLoad:true,
			url : __ctxPath
					+ "/creditFlow/finance/listbyLedgerSlFundIntent.do?businessType="+this.businessType+this.getfundType()+"&typetab=actual",
			fields : [{
					name : 'fundIntentId',
					type : 'int'
		...
		if (s <= 0) {
			...
			if (accrualMoney != 0) {
				new SlPunishInterestView({
					...
					businessType : "Actual",
					type:v.type,
					FundType : "interest" // 本金
				}).show();
			} else {
				Ext.ux.Toast.msg('操作信息', '该款项未逾期！');
			}
		}
	},
