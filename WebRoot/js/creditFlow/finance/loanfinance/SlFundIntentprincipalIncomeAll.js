 ...
 * @createtime:
 */
SlFundIntentprincipalIncomeAll = Ext.extend(Ext.Panel, {
		// 构造函数
		constructor : function(_cfg) {
			...
			// 调用父类构造
			SlFundIntentprincipalIncomeAll.superclass.constructor.call(this, {
				id : 'SlFundIntentprincipalIncomeAll'+this.businessType+this.type,
				title : '全部款项',
				region : 'center',
				layout : 'border',
				iconCls :'btn-team2',
			...
			this.searchPanel = new HT.SearchPanel({
				...
				this.topbar = new Ext.Toolbar({
					items : [{
						...
						scope : this,
						type:0,
						hidden : isGranted('_fa_f_principalIncome_'+this.businessType)?false:true,
						...
						scope : this,
						type:1,
						hidden : isGranted('_YQ_f_principalIncome_'+ this.businessType) ? false : true,
						handler : this.faxiAccount
					}, new Ext.Toolbar.Separator({
						hidden : isGranted('_YQ_f_principalIncome_'+ this.businessType) ? false : true
					}) ,{
				...
				this.gridPanel = new HT.GridPanel({
					...
					fields : [{
						...
						'companyId','faxiAfterMoney','payintentPeriod','amountPayable','continueInterest','continueDay','graceDay','interestStarTime','interestEndTime','bidPlanId'],
					columns : [{
							header : 'fundIntentId',
							...
							renderer:function(v){	
								if(v==0){
						         	...
						         	return "<a><u>"+Ext.util.Format.number(v,',000,000,000.00')+"元"+"</u></a>"
						        }
							}
						}*/, {
							header : '罚息金额',
