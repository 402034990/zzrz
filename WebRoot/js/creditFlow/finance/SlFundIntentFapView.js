...
SlFundIntentFapView= Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		SlFundIntentFapView.superclass.constructor.call(this, {
			name : "SlFundIntentFapView",
			...
			items : [{
				...
				html : '<br/><B><font class="x-myZW-fieldset-title">【放款收息表】</font></B>',
	...
	initUIComponents : function() {
		...
		this.topbar = new Ext.Toolbar({
			items : [{
				html:'<font class="x-myZW-fieldset-title">【放款收息表】</font>'
			},{
				iconCls : 'btn-add',
				text : '增加',
				xtype : 'button',
	...
	/*	var field = Ext.data.Record.create([{
		...
		if(null!=this.inverstPersonId){
			url=__ctxPath + "/creditFlow/finance/listOfInverstPersonSlFundIntent.do?inverstPersonId="+this.inverstPersonId
		}
						...
						'select' : function(combox,record,index){
							...
							var personCom = new Ext.form.ComboBox({
								...
								onTriggerClick : function() {
									selectInvestEnterPrise(selectInvestEnterpriseObj,r);
								},
								mode : 'remote',
								editable : false,
								lazyInit : false,
							...
							grid_sharteequity.getColumnModel().setEditor(2,ComboBox);
						}
					}
				},
...
//				renderer : investPersonName
	...
	confirmRs : function(){
		Ext.Ajax.request({
			scope : this,
			url : __ctxPath + "/customer/listByProjectIdInvestPersonInfo.do?projectId="+this.projectId+"&businessType="+this.businessType,
//			async:false,
			waitMsg : '数据正在生成，请稍后...', //add by zcb 2013-09-18
			success : function(response, options){
				var responseJson = Ext.decode(response.responseText);
				if(responseJson.totalCounts>0){
					...
					if (this.businessType == "SmallLoan") {
						...
						payintentPerioDate = this.object
								...
								.getValue();
						accrual = this.object.getCmpByName("slSmallloanProject.accrual")
								.getValue();
					...
					var isPreposePayAccrualCheck = this.object
					...
					Ext.Msg.alert('友好提示','请先添加或保存投资人信息才能生成放款收息表')
				}
			}
		});
	},
