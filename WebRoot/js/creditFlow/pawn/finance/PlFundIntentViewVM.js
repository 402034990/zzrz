...
PlFundIntentViewVM  = Ext.extend(Ext.Panel, {
	name : "PlFundIntentViewVM",
	infoObj : null,
	isHiddenAddBtn : true,
	isHiddenDelBtn : true,
	isHiddenCanBtn : true,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		PlFundIntentViewVM.superclass.constructor.call(this, {
			name : "PlFundIntentViewVM_panel",
			...
			items : [{
				...
				html : '<br/><B><font class="x-myZW-fieldset-title">【放款收息表】</font></B><font class="x-myZW-fieldset-title">（</font>颜色预警：<font color=red>逾期款项</font>&nbsp;&nbsp<font style="line-height:20px">未结清项</font>&nbsp;&nbsp<font color=gray>已结清项</font><font class="x-myZW-fieldset-title" >）：</font>',
		...
		this.gridPanel = new HT.EditorGridPanel({
							...
							store : new Ext.data.SimpleStore({
								...
								baseParams : {
									payintentPeriod : typeof(this.object) != 'undefined'
											? this.object
													.getCmpByName('plPawnProject.payintentPeriod')
													.getValue()
											: 0,
									projectId : this.projectId,
									businessType : this.businessType
						...
						renderer : function(value, metaData, record, rowIndex,
							...
							if (record.data.isValid == 1) {
								if (null == record.data.slSuperviseRecordId
										&& record.data.fundType != 'pawnPrincipalLending') {
									...
									return '<font style="font-style:italic;text-decoration: line-through;color:gray">续当第'
											+ value + '期</font>'
								} else {
									if (null != value) {
								...
								if (record.data.notMoney == 0) {
									if (null == record.data.slSuperviseRecordId
										...
										return '<font style="color:gray">续当第'
												+ value + '期</font>'
									} else {
										if (null != value) {
			...
			listeners : {
				...
				afteredit : function(e) {
					if (e.record.data['fundType'] == 'pawnPrincipalLending'
	...
	createRs : function() {
		...
		var payintentPeriod = (typeof(this.object) != 'undefined'
				? this.object.getCmpByName('plPawnProject.payintentPeriod')
						.getValue()
		...
		if (vCount > 0) {
			...
			for (var i = 0; i < vCount; i++) {
				if (vRecords[i].data.fundType != null
						&& vRecords[i].data.fundType != "") {
					if ((null == vRecords[i].data.afterMoney
							|| vRecords[i].data.afterMoney == 0) && (vRecords[i].data.isValid==0 || vRecords[i].data.isValid==null)) {
						st = {
							"fundIntentId" : vRecords[i].data.fundIntentId==''?null:vRecords[i].data.fundIntentId,
							"fundType" : vRecords[i].data.fundType,
							"incomeMoney" : vRecords[i].data.incomeMoney,
							"payMoney" : vRecords[i].data.payMoney,
							"intentDate" : vRecords[i].data.intentDate,
	...
	autocreate : function() {
		...
		if (this.businessType == "Pawn") {
			payaccrualType = this.object
					.getCmpByName("plPawnProject.payaccrualType")
					.getValue();
			projectMoney = this.object
					.getCmpByName("plPawnProject.projectMoney").getValue();
			isStartDatePay = this.object
					.getCmpByName("plPawnProject.isStartDatePay")
					.getValue();
			startDate = this.object
					.getCmpByName("plPawnProject.startDate").getValue();
			payintentPerioDate = this.object
					.getCmpByName("plPawnProject.payintentPerioDate")
					.getValue();
			accrual = this.object.getCmpByName("plPawnProject.accrual")
					.getValue();
			...
			intentDate = this.object
					.getCmpByName("plPawnProject.intentDate").getValue();
			dayOfEveryPeriod = this.object
					.getCmpByName("plPawnProject.dayOfEveryPeriod")
					.getValue();
			isPreposePayAccrual = this.object
					.getCmpByName("isPreposePayAccrualCheck").getValue();
			isInterestByOneTime = this.object
					.getCmpByName('plPawnProject.isInterestByOneTime')
					.getValue();
			monthFeeRate = this.object
					.getCmpByName('plPawnProject.monthFeeRate').getValue();
		...
		var params1 = {
			projectId : projId,
			'plPawnProject.isPreposePayAccrual' : isPreposePayAccrual == false
					? 0
					: 1,
			...
			'plPawnProject.isStartDatePay' : isStartDatePay,
			'plPawnProject.payintentPerioDate' : payintentPerioDate,
			'plPawnProject.accrual' : accrual,
			'plPawnProject.financeServiceOfRate' : monthFeeRate,
			'plPawnProject.payintentPeriod' : payintentPeriod,
			'plPawnProject.dayOfEveryPeriod' : dayOfEveryPeriod,
			'plPawnProject.isInterestByOneTime' : isInterestByOneTime,
