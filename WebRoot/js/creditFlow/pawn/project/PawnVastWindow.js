 ...
 */
PawnVastWindow = Ext.extend(Ext.Window, {
	// 构造函数
	constructor : function(_cfg) {
		...
		PawnVastWindow.superclass.constructor.call(this, {
					layout : 'fit',
	...
	initUIComponents : function() {
		this.pawnVastForm=new PawnVastForm({projectId:this.projectId,businessType:this.businessType,isAllReadOnly:this.isAllReadOnly});
		this.plFundIntentViewVM=new PlFundIntentViewVM({
			object :this.pawnVastForm,
		...
		if (this.projectId != null && this.projectId != 'undefined' && this.businessType!=null && this.businessType!='undefined') {
			this.formPanel.loadData({
						url : __ctxPath
								+ '/creditFlow/pawn/project/getPawnInfoPawnVastMaragement.do?projectId='
								+ this.projectId+'&businessType='+this.businessType+"&vastId="+this.vastId,
						root : 'data',
						scope : this,
						preName : 'pawnVastMaragement',
						success : function(resp, options) {
							...
							if(typeof(result.data)!="undefined" && result.data!=null){
								this.getCmpByName('pawnVastMaragement.managerId').setRawValue(result.data.managerName);
								this.getCmpByName('pawnVastMaragement.managerId').nextSibling().setValue(result.data.managerId)
							}
						}
					});
		}
	...
	save : function() {
		...
		var vastGridPanel=this.vastGridPanel;
		var projectPanel=this.projectPanel;
		this.formPanel.getForm().submit({
		    clientValidation: false, 
			url : __ctxPath + '/creditFlow/pawn/project/savePawnVastMaragement.do',
			params : {
			...
			success : function(fp, action) {
				...
				if(null!=vastGridPanel && typeof(vastGridPanel)!="undefined"){
					vastGridPanel.getStore().reload()
				}
