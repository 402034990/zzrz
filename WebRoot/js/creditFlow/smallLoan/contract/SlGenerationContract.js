...
SlGenerationContract = Ext.extend(Ext.Window, {
		...
		SlGenerationContract.superclass.constructor.call(this, {
			...
			buttons : [{
				...
				handler : function(){
					...
					var temId = this.temptId?this.temptId:templateId;
	...
	initUIComponents : function() {
		...
		var contractRemark = {
			...
			listeners : {
				afterrender : function(combox) {
					Ext.Ajax.request({
						url : __ctxPath+ "/contract/getContractNumberProcreditContract.do",
						method : "post",
						params : {
						...
						success : function(response) {
							...
							var contractNumber = obj.contractNumber;
							if(contractNumber != null && contractNumber != ""){
								combox.setValue(contractNumber);
							}
						},
						failure : function() {
							Ext.ux.Toast.msg('操作信息', '获取信息失败！');
						}
					});
				}
			}
		...
		if(this.htType == "XEDQ" && this.businessType=='SmallLoan') {
			...
			htTypeTag = true;
			htTypeLabel = "C2P借款合同";
		}else if(this.htType == "shouxinhetong" && this.businessType=='BankCredit'){
			htTypeTag = true;
			htTypeLabel = "授信合同";
		}
		var contractTypeRadioGroup = new Ext.form.RadioGroup({
			...
			listeners : {
				...
				"change" : function(com, checked) {
					...
					if (inputValue == 1) {
						this.htType = null;
						if(this.oldHtType=='extenstionContract'){//展期合同
							this.htType='extenstionContract';
						}
		...
		if(this.businessType =='BankCredit'){
		}else{
	...
	makeContract :function(temId,businessType,piKey,taskId,contractType,categoryId,contractId,a,b,htType,thirdRalationId,isApply,clauseId){
		...
		if(businessType=='BankCredit'){
			var contractCategoryText = this.getCmpByName('contractCategoryTypeText').getValue(); //获取合同下拉框的值
			var contractCategoryTypeTextItems = this.getCmpByName('contractCategoryTypeText').getStore().data.items;//获取数据源集合长度
			var  itemsLength = contractCategoryTypeTextItems.length;
			for(var  i=0;i<itemsLength;i++){
				var itemid = contractCategoryTypeTextItems[i].data.itemId;
				var itemName = contractCategoryTypeTextItems[i].data.itemName;
				if(itemName==contractCategoryText){
					templateId  = itemid;
				}
			}
		}else{
			var contractCategoryText = this.getCmpByName('contractCategoryText').getValue(); //获取合同下拉框的值
		}
		var leaseObjectInfo="";
		var dwId="";
	...
	getmarkStr:function(inputValue){
		if (typeof(this.businessType) != "undefined") {
			...
			}else if(this.businessType == 'BankCredit'){
				this.HTLX = 'shouxinhetong'
				this.windowHeight = 291;
			}
