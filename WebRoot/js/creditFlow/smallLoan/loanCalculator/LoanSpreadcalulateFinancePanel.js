...
LoanSpreadcalulateFinancePanel = Ext.extend(Ext.Panel, {
	layout : "form",
	autoHeight : true,
	labelAlign : 'right',
	isAllReadOnly : false,
	...
	constructor : function(_cfg) {
		...
		LoanSpreadcalulateFinancePanel.superclass.constructor.call(this, {
			items : [{
				layout : "column",
				border : false,
				scope : this,
				...
				columnWidth : 1,
					...
					items : [{
						...
						items : [{
							...
							// value :true
							checked : (this.isInterestByOneTime == null || this.isInterestByOneTime== 0) ? null : true,
							listeners : {
								scope : this,
								'check' : function(box,value){
									if(value==true){
...
//									 this.getCmpByName("slSmallloanProject.dateMode").setValue(0);
									 ...
									 Ext.getCmp("jixifs1"+this.idDefinition).setValue(false);
									 Ext.getCmp("jixifs2"+this.idDefinition).setValue(false);
									 this.getCmpByName("bpProductParameter.isActualDay").setValue("");
//									 Ext.getCmp("jixifs4"+this.idDefinition).setValue(false);
									 //清空表格中的数据 s'w'j
									 var this1=Ext.getCmp("caluateIntentGrideditGrid");
									 this1.getStore().removeAll()
							}
						}]
					},{
						columnWidth : .1,
...
//								 var dateMode=this.getCmpByName("slSmallloanProject.dateMode").getValue();
  ...
  Ext.reg('calulateFinancePanel', LoanSpreadcalulateFinancePanel);
