...
//				text : '终止项目',
				...
				text : '删除项目',
				xtype : 'button',
				hidden : this.projectStatus==0?false:true && isGranted('ApproveProject_remove_' + this.projectStatus)? false: true,
				scope : this,
				handler : function() {
					removePro(this.gridPanel, 'SmallLoanProjectInfo_','SmallLoanProjectInfoEdit_')
				}
			}, new Ext.Toolbar.Separator({
				hidden : !isGranted('ApproveProject_remove_' + this.projectStatus)
			})*/,{
		...
		this.gridPanel = new HT.GridPanel({
			...
			fields : [{
				name : 'runId',
				type : 'int'
			}, 'projectId','appUserName','createDate','orgName', 'projectName',
					'projectNumber', 'projectMoney','oppositeType', 'oppositeTypeValue',
					'customerName', 'operationType','taskId', 'activityName',
					'oppositeID', 'businessType','endDate', 'payintentPeriod',
					'executor','productTypeName'],
			columns : [{
				header : 'runId',
				dataIndex : 'runId',
				hidden : true
