 ...
 * @createtime:
 */
SmallLoanedProjectManager = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		SmallLoanedProjectManager.superclass.constructor.call(this, {
			id : 'SmallLoanedProjectManager_' + this.managerType,
...
//				text : '对不起，您能查看的项目数为0。如有错误，请与系统管理员联系，看你的上下级关系是否设置正确!(查询授权规律为：你可以看你自己或你下属的项目)',
	...
	initUIComponents : function() {
		var jsArr = [
				...
				__ctxPath + '/js/creditFlow/smallLoan/project/SlProjectFinished.js' //贷款结项
		];
...
//						var superviseId = null;
//						Ext.Ajax.request({
//							url : __ctxPath
//									+ "/project/addSuperviseRecordSlSmallloanProject.do",
//							params : {
//								projectId : projectId
//							},
//							method : 'POST',
...
//								superviseId = obj.superviseId;
//								new AddSupervisionRecord({
//									projectId : projectId,
				...
				items : [/*{
					...
					handler : function() {
						...
						if (s <= 0) {
							...
							var gpObj = document
									.getElementById('SmallLoanExtensionView'
											+ record.data.projectId);
							if (gpObj == null) {

								gpObj = new SmallLoanExtensionView({
							...
							tabs.setActiveTab("SmallLoanExtensionView"
									+ record.data.projectId);
						}
					}
				},*/ {
						...
						if (s <= 0) {
							...
							/*Ext.Ajax.request({
								...
								if(isOtherFlow==1){//isOtherFlow==1表示当前项目已经启动了展期流程
									...
									var tabs = Ext.getCmp('centerTabPanel');
									var gpObj = document
											.getElementById('SmallLoanEarlyRepaymentView'
													+ record.data.projectId);
									if (gpObj == null) {
										gpObj = new SmallLoanEarlyRepaymentView({
									...
									tabs.setActiveTab("SmallLoanEarlyRepaymentView"+ record.data.projectId);
				...
				items : [{
					...
					handler : function() {
						...
						if (s <= 0) {
							...
							tabs.setActiveTab("SlProjectFinished_"+ record.data.projectId);
						}
					}
				}, new Ext.Toolbar.Separator({
...
/*								Ext.Ajax.request({
								...
								success : function(response, request) {
									...
									var tabs = Ext.getCmp('centerTabPanel');
									var gpObj = document.getElementById('SmallLoanAlterAccrualView'+ record.data.projectId);
									if (gpObj == null) {
										gpObj = new SmallLoanAlterAccrualView({
											surplusnotMoney : (record.data.projectMoney - ((record.data.payProjectMoney == null)
													? 0
													: record.data.payProjectMoney)),
											intentDate : record.data.expectedRepaymentDate,
									...
									tabs.setActiveTab("SmallLoanAlterAccrualView"+ record.data.projectId);
								}
							})*/
