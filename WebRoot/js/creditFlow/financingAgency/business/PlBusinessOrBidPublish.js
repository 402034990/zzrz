 ...
 * @company 智维软件
 */
PlBusinessOrBidPublish = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
		...
		PlBusinessOrBidPublish.superclass.constructor.call(this, {
					id : 'PlBusinessOrBidPublish_'+this.Q_proType_S_EQ+this.Q_state_N_EQ+this.buttonType,
					layout : 'border',
 ...
 var flag=this.Q_state_N_EQ==21 || this.Q_state_N_EQ==22 ||this.Q_state_N_EQ==23;
		...
		this.gridPanel = new HT.GridPanel({
			region : 'center',
			tbar : this.topbar,
			id : 'PlBusinessOrBidPublishGrid'+this.Q_proType_S_EQ+this.Q_state_N_EQ,
			url : url,
			...
			fields : [{
					...
					'bidStartTime', 'prepareSellTime','publishSingeTime', 'bidEndTime','bpBusinessOrPro.proId','bpBusinessOrPro.moneyPlanId','proType',
		...
		if (s <= 0) {
			...
			success : function(response, request) {
					...
					var  bidPlanInfoForm=new PlBidPlanInfoForm({
						...
						projectId:record.get('bpBusinessOrPro.proId'),
						fundProjectId:record.get('bpBusinessOrPro.moneyPlanId'),
				...
				params : {
					projectId:record.get('bpBusinessOrPro.moneyPlanId')
				}
			});
	...
	startRs : function() {
		...
		if("1"==record.data.isStart){
			Ext.ux.Toast.msg('操作信息', '已启动任务,不能再重新启动任务!');
			return;
		}
