...
ExperienceStandardPlanForm = Ext.extend(Ext.Window, {
		...
		this.formPanel = new Ext.FormPanel({
			...
			items : [{
				...
				items : [{
							 ...
							 * { columnWidth : .31, layout : 'form', labelWidth :
							 * leffwidth, items : [{ fieldLabel : '预售公告时间', name :
							 * 'plManageMoneyPlan.preSaleTime', xtype :
							 * 'datetimefield', fieldClass : 'field-align',
							 * format : 'Y-m-d H:i:s', readOnly :
							 * this.isAllReadOnly, anchor : '100%' }] },{
							 * columnWidth : .02, // 该列在整行中所占的百分比 layout :
							 * "form", // 从上往下的布局 labelWidth : 18, items : [{
							 * fieldLabel : " ", labelSeparator : '', anchor :
							 * "100%"
							 *  }] },
							 */{
							columnWidth : .28,
	...
	save : function() {
		 ...
		 * }else{
		 * 
		 * $postForm({ formPanel : this.formPanel, scope : this, url : __ctxPath +
		 * '/creditFlow/financingAgency/saveExperiencePlManageMoneyPlan.do?keystr=experience',
		 * callback : function(fp, action) { var obj =
		 * Ext.util.JSON.decode(action.response.responseText);
		 * fpl.getCmpByName("plManageMoneyPlan.mmplanId").setValue(obj.id);
		 * 
		 * var gridPanel = Ext.getCmp('ExperienceStandardPlanPublishGrid'); if
		 * (gridPanel != null) { gridPanel.getStore().reload(); }
		 *  } }); }
		 */
