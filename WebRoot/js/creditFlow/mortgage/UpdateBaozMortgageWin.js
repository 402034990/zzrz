UpdateBaozMortgageWin = Ext.extend(Ext.Window, {
	layout : 'anchor',
	anchor : '100%',
	constructor : function(_cfg) {
		...
		UpdateBaozMortgageWin.superclass.constructor.call(this, {
			        buttonAlign:'center',
			        title:'编辑保证担保',
			        iconCls : 'btn-add',
		...
		var businessType=this.businessType
			...
			if(Ext.getCmp('baozpersontype_id').getValue()==602){
				...
				if(businessType=='Financing'){
					...
					Ext.getCmp('baozr').setRawValue(obj.name) ;
						Ext.getCmp('mortgagename').setValue(obj.name);
					setCustomerName(obj.name,obj.zhusuo,obj.cardnumber,obj.cellphone,obj.ispublicservant,obj.cardtype);
				}
			}
			
		}
		if(this.type==602){
			this.otherPanel=new CompanyForm({id:this.id,type:this.type,businessType:this.businessType});
		}else if(this.type==603){
			this.otherPanel=new DutypersonForm({id:this.id,type:this.type,businessType:this.businessType});
		}
						...
						listeners : {
							...
							'select' : function(combo,record, index){
								...
								if(combox.getValue()==602){
									...
									this.formPanel.add(new CompanyForm({}))
									this.formPanel.doLayout()
								}else if(combox.getValue()==603){
									...
									this.formPanel.add(new DutypersonForm({}))
									this.formPanel.doLayout()
								}*/
							}
						}
		...
		if(null!=this.id && null!=this.type){
			this.formPanel.loadData({
				...
				success : function(resp, options) {
					...
					Ext.getCmp('baozr').setValue(obj.data.procreditMortgage.assureofname)
					Ext.getCmp('baozr').setRawValue(obj.data.assureofname)
