 ...
 * @createtime:
 */
MortgageManagement = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		MortgageManagement.superclass.constructor.call(this, {
			id:'MortgageManagement'+this.businessType,
			title : '押品手续办理',
			region : 'center',
			layout : 'border',
			iconCls :'btn-team2',
	...
	initUIComponents : function() {
		...
		this.searchPanel = new Ext.FormPanel( {
			...
			items : [{
				...
				border : false,
				labelWidth : 70,
				hidden:this.isCar==true?true:false,
				labelAlign : 'right',
				items : [ {
					xtype : 'dickeycombo',
					nodeKey : 'dblx',
		...
		this.topbar = new Ext.Toolbar( {
			items : [ {
				...
				text : '查看抵质押物详情',
				xtype : 'button',
				hidden : isGranted('_seeMortgageInfo_'+this.businessType)?false:true,
				scope : this,
				...
				text : '查看合同详情',
				xtype : 'button',
				hidden : isGranted('_seeThirdContract_'+this.businessType)?false:true,
				scope : this,
				...
				text : '查看项目详情',
				xtype : 'button',
				hidden : isGranted('_seeProjectInfo_'+this.businessType)?false:true,
				scope:this,
				handler : this.seeProjectInfo  
			}, {
				iconCls : 'btn-material',
				text : '办理抵质押物',
				xtype : 'button',  
				scope:this,
				hidden : isGranted('_banliMortgage_'+this.businessType)?false:true,
				handler : this.banliMortgage
			}, {
				iconCls : 'deleteIcon',
				text : '解除抵质押物 ',
				xtype : 'button',
				hidden : isGranted('_relieveMortgage_'+this.businessType)?false:true,
				scope:this,
				handler : this.relieveMortgage
			}, {
				iconCls : 'btn-material',
				text : '入库办理',
				xtype : 'button',
				scope:this,
				hidden : isGranted('_inStore_'+this.businessType)?false:true,
				flag:0,
				handler : this.InOutstore
			}, {
				iconCls : 'deleteIcons',
				text : '出库办理 ',
				xtype : 'button',
				hidden : isGranted('_outStore_'+this.businessType)?false:true,
				scope:this,
				flag:1,
				handler : this.InOutstore
			},{
				...
				text : 'GSP安装 ',
				xtype : 'button',
				hidden : isGranted('_GSPAdd_'+this.businessType)?false:true,
				scope:this,
				flag:1,
				...
				text : 'GSP安装查看 ',
				xtype : 'button',
				hidden : isGranted('_GSPSee_'+this.businessType)?false:true,
				scope:this,
				flag:1,
				handler : this.seeCarGps
			}/*,'->', {
				xtype : 'label',
       ...
       this.reader = new Ext.data.JsonReader({
			...
			totalProperty : "totalCounts"
						...
						name : 'id',
						type : 'int'
					}, 'mortgagename','mortgagepersontypeforvalue','assureofname','storehouseStatus', 
		...
		this.gridPanel = new HT.GridPanel( {
			...
			columns : [{
				...
				renderer : function(value,metaData, record,rowIndex, colIndex,store) {
		            if(value!=null && ""!=value){
		           	    value=Ext.util.Format.number(value,'0,000.00')+"万元";
		            }
					return 	value
				}
			}, {
	...
	//按条件搜索
	search : function() {
	/*	var businessType=Ext.getCmp("businessType").getValue();
	...
	InOutstore:function(v){
		...
		for (var i = 0; i < selRs.length; i++) {
			if(v.flag==0){//入库
				if(selRs[i].get('storehouseStatus')==1){
					Ext.ux.Toast.msg('操作信息', '选中的记录包含已入库,请重新选择!');
					return;
				}else if(selRs[i].get('storehouseStatus')==2){
					Ext.ux.Toast.msg('操作信息', '选中的记录包含已出库,请重新选择!');
					return;
				}
			}else if(v.flag==1){//出库
				if(selRs[i].get('storehouseStatus')==2){
					Ext.ux.Toast.msg('操作信息', '选中的记录包含已出库,请重新选择!');
					return;
				}else if(selRs[i].get('storehouseStatus')==null){
					Ext.ux.Toast.msg('操作信息', '选中的记录包含未入库,请重新选择!');
					return;
				}
			}
			ids+=selRs[i].get('id')+",";
	...
	seeMortgageInfo : function() {debugger
		var selRs = this.gridPanel.getSelectionModel().getSelections();
		if (selRs.length !=1) {
			Ext.ux.Toast.msg('操作信息', '请选择一条记录!');
			return;
		...
		var projid=selected.get('projid');
		var businessType = selected.get('businessType');
			if(typeId==1){
				new UpdateDzyMortgageWin({id:mortgageId,projectId:projid,businessType:businessType,
				gridPanel:null,type:typeId,projectMoney:null,isReadOnly:true}).show()
//				seeCarInfo(mortgageId,businessType);
				...
				seeQualifiedCarInfo(mortgageId,businessType);
			}else if(typeId==20){
				seeNoQualifiedCarInfo(mortgageId,businessType);
			}else{
