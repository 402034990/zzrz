 ...
 * @createtime:
 */
BaozMortgageView = Ext.extend(Ext.Panel, {
	// 构造函数
	isFlow:false,
	isAllReadOnly:false,
	constructor : function(_cfg) {
		...
		BaozMortgageView.superclass.constructor.call(this, {
			layout : 'anchor',
			anchor : "100",
			autoWidth:true,
			items : [ 
	...
	initUIComponents : function() {
		...
		this.topbar = new Ext.Toolbar( {
			items : [{
				...
				text : typeof(this.isBl)=="undefined"?(typeof(this.isAllReadOnly)=="undefined"?'落实担保保证':'查看落实担保措施详情'):(this.isBl?"落实担保保证":"查看落实担保措施详情"),
				xtype : 'button',
				scope : this,
				hidden : this.isblEdit,
				handler : this.luosMortgage
			}]
		});
...
//			id:'baozmortgage',
			...
			autoHeight:true,
			clicksToEdit : 1,
			plugins : [this.expanderFlow,checkColumn,contractCheckColumn,issignContractcheckColumn,recieveCheckColumn,transactCheckColumn],
			url : __ctxPath +'/credit/mortgage/getMortgageOfBZ.do?projectId='+this.projectId+'&businessType='+this.businessType+'&isReadOnly='+this.isReadOnly,
			fields : [{
						name : 'id',
						type : 'int'
	...
	createMortgage : function() {
		new AddBaozMortgageWin({projectId:this.projectId,businessType:this.businessType,gridPanel:this.gridPanel}).show()
	},
	...
	editMortgage : function(record) {
		...
		if (s <= 0) {
			...
			record=s[0]
			new UpdateBaozMortgageWin({id:record.data.id,projectId:this.projectId,businessType:this.businessType,gridPanel:this.gridPanel,type:record.data.personTypeId}).show()
		}	
	},
	...
	addThirdContract : function(businessType, piKey) {
		...
		if (null == selected) {
			...
			var title = selected.get('assureofnameEnterOrPerson');
	...
	operateThirdContract : function(businessType, piKey) {
		...
		if (null == selected) {
			...
			var title = '保证担保<<font color=red>'+selected.get('assureofnameEnterOrPerson')+'</font>>';
			var thirdRalationId = selected.get('id');
			var contractId = selected.get('contractId');
			var categoryId = selected.get('categoryId');
			var temptId = selected.get('temptId');
	...
	luosMortgage : function(){debugger
		var selected = this.gridPanel.getSelectionModel().getSelected();
		var grid=this.gridPanel;
		if (null == selected) {
			Ext.ux.Toast.msg('状态', '请选择一条记录!');
