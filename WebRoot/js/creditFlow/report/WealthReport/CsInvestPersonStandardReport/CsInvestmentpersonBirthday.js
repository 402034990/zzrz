			...
			listeners : {
				'afterrender' : function(v) {
					...
					Ext.getCmp('searchPanel'+this.reportKey).getForm().submit({
						waitMsg : '正在提交查询',
						url : __ctxPath + '/system/reportBeGoingBirthdayWealthReport.do?reportKey='+reportKey+"&reportKey2=BeGoingBirthday2",
						method : 'post',
				...
				items : [{
					...
					handler:function(){
						if (Ext.getCmp('searchPanel'+this.reportKey).getForm().isValid()) {
							Ext.getCmp('searchPanel'+this.reportKey).getForm().submit({
								waitMsg : '正在提交查询',
								url : __ctxPath + '/system/reportBeGoingBirthdayWealthReport.do?reportKey='+reportKey,
								method : 'post',
								success : function(form, action) {
									var object = Ext.util.JSON.decode(action.response.responseText)
									var temp = Ext.getCmp('reportTemp' + reportKey);
