 ...
 */
FinancialProductsDividendMonth = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		FinancialProductsDividendMonth.superclass.constructor.call(this, {
			id : 'FinancialProductsDividendMonth'+this.reportKey,
			title : this.title,
			region : 'center',
			layout : 'anchor',
			iconCls : 'menu-report',
			...
			listeners : {
				'afterrender' : function(v) {
					...
					Ext.getCmp('FinancialProductsDividend'+this.reportKey).getForm().submit({
						waitMsg : '正在提交查询',
						url : __ctxPath + '/system/reportPlmmMonthWealthReport.do?reportKey='+reportKey,
						method : 'post',
	...
	initUIComponents : function() {
		...
		searchPanel = new HT.SearchPanel({
			id : 'FinancialProductsDividend' + reportKey,
			layout : 'column',
			region : 'north',
			height : 20,
			anchor : '100%',
			...
			items : [{ 
				...
				items : [{
					...
					iconCls : 'btn-search',
					handler:function(){
						if (Ext.getCmp('FinancialProductsDividend'+this.reportKey).getForm().isValid()) {
							Ext.getCmp('FinancialProductsDividend'+this.reportKey).getForm().submit({
								waitMsg : '正在提交查询',
								url : __ctxPath + '/system/reportPlmmMonthWealthReport.do?reportKey='+reportKey,
								method : 'post',
				...
				items : [{
					...
					iconCls : 'reset',
					handler:function(){
						Ext.getCmp('FinancialProductsDividend'+this.reportKey).getForm().reset();
					}
				}]
			}]
		});
		
		var panel=Ext.getCmp('FinancialProductsDividend'+reportKey);
