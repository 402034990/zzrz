...
LoanTrialFormPanel = Ext.extend(Ext.Panel, {
	layout : "form",
	autoHeight : true,
	labelAlign : 'right',
	isAllReadOnly : false,
	...
	constructor : function(_cfg) {
		...
		if (typeof(_cfg.taskItems) != "undefined") {
			this.taskItems = _cfg.taskItems;
		}
		...
		var changeDisabled = function(selectType,dateType,objectPanel){
			...
			}else if(selectType=="chargeType"){
				objectPanel.getCmpByName(objectPanel.defineForm+'.chargeType').setValue(dateType);
				if(dateType=="1"){
					objectPanel.getCmpByName('chargeType1'+ objectPanel.idDefinition).setValue(true);
					objectPanel.getCmpByName('chargeType2'+ objectPanel.idDefinition).setValue(false);
				}else if(dateType=="2"){
					objectPanel.getCmpByName('chargeType1'+ objectPanel.idDefinition).setValue(false);
					objectPanel.getCmpByName('chargeType2'+ objectPanel.idDefinition).setValue(true);
				}
			}
			if(isAllReadOnly){
				objectPanel.getCmpByName('isStartDatePay1'+ objectPanel.idDefinition).setDisabled(true);
				...
				Ext.Ajax.request({
					...
					success : function(response, request) {
						...
						if(accrualType=="accrualRateType"){
							...
							var yearServiceChargeOfRate = objectPanel.getCmpByName(objectPanel.defineForm+'.yearServiceChargeOfRate').getValue();
							var dayServiceChargeOfRate = objectPanel.getCmpByName(objectPanel.defineForm+'.dayServiceChargeOfRate');
							var sumServiceChargeOfRate = objectPanel.getCmpByName(objectPanel.defineForm+'.sumServiceChargeOfRate');
							dayServiceChargeOfRate.setValue((yearServiceChargeOfRate / yearModel).toFixed(4));//日手续费率
							sumServiceChargeOfRate.setValue((yearServiceChargeOfRate/ yearModel * days).toFixed(4));//小计手续费率
		...
		LoanTrialFormPanel.superclass.constructor.call(this, {
			items : [{
				layout : "column",
				border : false,
				scope : this,
				...
				items : [{
						...
						items : [{
							...
							layout : 'column',
							items : [{
								columnWidth : firstColumnWidth+.14, // 该列在整行中所占的百分比
								layout : "form", // 从上往下的布局
								labelWidth : 80,
								...
								items : [{
									...
									listeners : {
										...
										change : function(nf) {
											...
											if (index <= 0) { // 如果第一次输入 没有进行格式化
												nf.setValue(Ext.util.Format.number(
														value, '0,000.00'))
												this
														.getCmpByName(this.defineForm+".projectMoney")
														.setValue(value);
											} else {
										...
										/*if(value>=800000){
										var checkom=this.getOriginalContainer().getCmpByName('taskSubmit');
										var taskItems=checkom.items;
											for(var i=0;i<taskItems.length;i++){
												if(taskItems.get(i).boxLabel.indexOf('总经理') != -1){
											...
											var taskItems=checkom.items;
												for(var i=0;i<taskItems.length;i++){
													if(taskItems.get(i).boxLabel.indexOf('风控总监') != -1){
														taskItems.get(i).setValue(true)
													}else{
														taskItems.get(i).setValue(false)
													}
											    ...
											    var taskItems=checkom.items;
												for(var i=0;i<taskItems.length;i++){
													if(taskItems.get(i).boxLabel.indexOf('风控经理') != -1){
														taskItems.get(i).setValue(true)
													}else{
														taskItems.get(i).setValue(false)
													}
													if(taskItems.get(i).boxLabel.indexOf('落实抵质押物及合同签署') != -1){
														taskItems.get(i).setValue(true)
													}else{
														taskItems.get(i).setValue(false)
													}
								...
								columnWidth :this.isProduct?.23: .16, // 该列在整行中所占的百分比
								layout : "form", // 从上往下的布局
											...
											change : function(nf) {
												...
												if (index <= 0) { // 如果第一次输入 没有进行格式化
													nf.setValue(Ext.util.Format.number(
															value, '0,000.00'))
													this
															.getCmpByName(this.defineForm+".fixedAccrualRate")
															.setValue(value);
												} else {
						...
						items : [{
									...
									items : [{
										...
										anchor : "100%",
										xtype : 'textfield',
										name : "fixedManagementConsultingOfRate1",
										readOnly : this.costEditor,
										decimalPrecision : 8,
										style : {
											imeMode : 'disabled'
											...
											change : function(nf) {
												...
												if (index <= 0) { // 如果第一次输入 没有进行格式化
													nf.setValue(Ext.util.Format.number(
															value, '0,000.00'))
													this
															.getCmpByName(this.defineForm+".fixedManagementConsultingOfRate")
															.setValue(value);
												} else {
						...
						items : [{
									...
									items : [{
										...
										anchor : "100%",
										xtype : 'textfield',
										name : "fixedFinanceServiceOfRate1",
										readOnly : this.costEditor,
										decimalPrecision : 8,
										style : {
											imeMode : 'disabled'
											...
											change : function(nf) {
												...
												if (index <= 0) { // 如果第一次输入 没有进行格式化
													nf.setValue(Ext.util.Format.number(
															value, '0,000.00'))
													this
															.getCmpByName(this.defineForm+".fixedFinanceServiceOfRate")
															.setValue(value);
												} else {
							...
							items : [{
								...
								items : [{
									xtype : 'radio',
									boxLabel : '前置',
									name : 'chargeType1'+ this.idDefinition,
									inputValue : false,
									anchor : "100%",
									disabled : this.radioType,
									listeners : {
										scope : this,
										check : function(obj, checked) {
											var flag = this.getCmpByName("chargeType1"+ this.idDefinition).getValue();
											if (flag == true) {
												changeDisabled("chargeType","1",this);
											}
										}
									}
								}]
								...
								items : [{
									...
									boxLabel : '后置',
									anchor : "100%",
									name : 'chargeType2'+ this.idDefinition,
									inputValue : false,
									disabled : this.radioType,
									listeners : {
										scope : this,
										check : function(obj, checked) {
											var flag = this.getCmpByName("chargeType2"+ this.idDefinition).getValue();
											if (flag == true) {
												changeDisabled("chargeType","2",this);
											}
										}
									}
								}]
								...
								xtype : "hidden",
								name : this.defineForm+".chargeType",
								value : "1"
							}, {
								columnWidth : .14,
								layout : 'form',
						...
						items : [{
									...
									items : [{
										...
										anchor : "100%",
										xtype : 'moneyfield',
										name : this.defineForm+".sumAllfixedMoney",
										readOnly : true,
										allowBlank : !this.isProduct,
										decimalPrecision : 8,
										style : {
...
Ext.reg('LoanTrialFormPanel', LoanTrialFormPanel);
