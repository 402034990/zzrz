...
ExtUD.Ext.CustomerType = Ext.extend(Ext.form.FieldSet, {
	...
	constructor : function(_cfg) {
		...
		var conf = [{
					...
					hidden : type == null ? false : type == "person" ? false : true,
					...
					inputValue : "2",
					style : "margin-left:20px;",
					hidden : type == null ? false : type == "person"? true : false,
		...
		ExtUD.Ext.CustomerType.superclass.constructor.call(this, {
			...
			items : [{
						xtype : "hidden",
						name : 'oppositeType',
						value : this.flag ? 'person_customer' : 'company_customer'
						...
						listeners : {
							"change" : function(com, checked) {
								...
								op.doLayout();
								if (inputValue == 1) {
									op .getCmpByName('operationType') .setValue('SmallLoan_SmallLoanBusiness');
									//op.getCmpByName("slSmallloanProject.productId") 	.setValue(null);
									//op.getCmpByName("slSmallloanProject.productName") .setValue(null);
									...
									op.insert(6, testFieldSet);
									op.doLayout();
									return false;

								} else if (inputValue == 2) {
									op.getCmpByName('operationType').setValue('SmallLoan_PersonalCreditLoanBusiness');
									//op.getCmpByName("slSmallloanProject.productId").setValue(null);
									//op.getCmpByName("slSmallloanProject.productName").setValue(null);
									var testFieldSet = new Ext.form.FieldSet({
										title : '个人客户信息',
 ...
 * combox.ownerCt.ownerCt.ownerCt.ownerCt.ownerCt; ressetProjuect(obj_n); },
 * afterrender : function(combox) { } } }] }, { columnWidth : .3, //
 * 该列在整行中所占的百分比 layout : "form", // 从上往下的布局 items : [{ fieldLabel : "流程类别",
 * xtype : "combo", anchor : "100%", id : "childxilaid_liucheng", mode :
 * 'local', store : new Ext.data.SimpleStore({ fields : ['displayText',
 * 'value'], data : [['CI', 3]] }), hiddenName : "flowType", allowBlank : false,
 * editable : false, displayField : 'itemName',// 显示字段值 valueField : 'itemId',
 * blankText : "流程类别不能为空，请正确填写!", emptyText : "请选择", triggerAction : 'all',
 * listeners : { afterrender : function(combox) { combox.clearInvalid(); },
 * change : function(combox) { var op = Ext.getCmp('createNewSLFunctionForm');
 * var comValue = combox.getValue(); if (comValue == "zmNormalFlow") { var
 * bankSet = new Ext.form.FieldSet({ title : '银行信息', collapsible : true,
 * labelAlign : 'right', name : 'bankinfo', autoHeight : true, items : [new
 * ExtUD.Ext.BankInfoPanel({})] }); var financeSet = new Ext.form.FieldSet({
 * title : '财务信息', collapsible : true, name : 'financeInfo', labelAlign :
 * 'right', items : [new ExtUD.Ext.ZmNormalProjectInfo( {})] });
 * bankSet.doLayout(true); op.insert(3, bankSet); op.insert(4, financeSet);
 * op.doLayout(true); } else { var items = op.items; var item1 = items.get(3);
 * var item2 = items.get(4); if (item1.name == "bankinfo" && item2.name ==
 * "financeInfo") { op.remove(item1, true); op.remove(item2, true);
 * op.doLayout(true); } } var obj_n =
 * combox.ownerCt.ownerCt.ownerCt.ownerCt.ownerCt; ressetProjuect(obj_n); } } }] }] }, {
 ...
 * columnWidth : .3, // 该列在整行中所占的百分比 layout : "form", // 从上往下的布局 items : [{
 * xtype : "combo", id : "dd_Degree", anchor : "100%", fieldLabel : "我方主体",
 * store : new Ext.data.SimpleStore({ fields : ['displayText', 'value'], data :
 * [['地区', 1]] }), emptyText : "请选择", allowBlank : false, displayField :
 ...
 * "hidden", value : "" }] }] }] }); }, initComponents : function() { } })
 */
...
ExtUD.Ext.PeerMainInfoPanel = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {debugger
		...
		if (typeof(_cfg.carNohidde) != "undefined") {
			this.carNohidde = _cfg.carNohidde;
		}
		...
		var legalpersonid=this.legalpersonid
						...
						items : [{
							...
							items : [{
								...
								onTriggerClick : function(e) {
									...
									var hangyeType=this.getOriginalContainer().getCmpByName('enterprise.hangyeType');
									var rootHangYeType=this.getOriginalContainer().getCmpByName('enterprise.rootHangYeType');
									var com=this;
									var selectBankLinkMan = function(array){
										hangyeType.setValue(array[0].id) ;
										com.setValue(array[0].text);
										rootHangYeType.setValue(array[array.length-1].id) 
									};
							...
							layout : "form", // 从上往下的布局
							labelWidth : leftlabel,
							hidden:this.carNohidde,
							id : 'carNo11',
							items : [{
										xtype : "textfield",
										name : "enterprise.carNo",
										fieldLabel : "车牌号",
										readOnly : this.isAllReadOnly,
										allowBlank : true,
										anchor : "100%"
							...
							items : [{
										xtype : "textfield",
										name : "enterprise.temporaryCarNo",
										fieldLabel : "临时车牌号",
										readOnly : this.isAllReadOnly,
//										blankText : "营业执照号码不能为空，请正确填写!",
										allowBlank : true,
							...
							items : [{
										xtype : 'textfield',
										// emptyText : '请输入税务登记号码',  睿本改为社会唯一编码
										blankText : '社会唯一编码',
										readOnly : this.isAllReadOnly,
										allowBlank : true,
//										regex : /^[0-9]*$/,
...
//										readOnly : this.isAllReadOnly,
					...
					items : [{
						...
						items : [{
							...
							items : [{
								...
								items : [{
									xtype:'hidden',
									name:'slSmallloanProject.bankId'
								},{
									fieldLabel : '开户名称',
									xtype : "combo",
									anchor : "90%",
									...
									hiddenName : 'enterpriseBank.name',
									mode : 'local',
									editable : false,
									...
									onTriggerClick : function() {
										...
										var bankInfo = function(obj) {
											op.getCmpByName('slSmallloanProject.bankId').setValue("");
											op.getCmpByName('enterpriseBank.id').setValue("");
											op.getCmpByName('enterpriseBank.name').setValue("");
											op.getCmpByName('enterpriseBank.accountnum').setValue("");//账号
											op.getCmpByName('enterpriseBank.bankid').setValue("");//银行名称
											op.getCmpByName('enterpriseBank.bankOutletsName').setValue("");//网点名称
											op.getCmpByName('enterpriseBank.areaId').setValue("");//开户地区Id
											op.getCmpByName('enterpriseBank.areaName').setValue("");//开户地区名称
											
											if(obj.id){
												op.getCmpByName('enterpriseBank.id').setValue(obj.id);
												op.getCmpByName('slSmallloanProject.bankId').setValue(obj.id);
											}
											if(obj.name){
												op.getCmpByName('enterpriseBank.name').setValue(obj.name);
											}
											if(obj.accountnum){
												op.getCmpByName('enterpriseBank.accountnum').setValue(obj.accountnum);
											}
											if(obj.bankid){
												op.getCmpByName('enterpriseBank.bankid').setValue(obj.bankid);
											}
											if(obj.bankOutletsName){
												op.getCmpByName('enterpriseBank.bankOutletsName').setValue(obj.bankOutletsName);
											}
											if(obj.areaId){
										...
										bankInfoWin(legalpersonid,false,0,0,bankInfo);
									}
								}]
							}, {
								columnWidth : 0.5, // 该列在整行中所占的百分比
								...
								items : [{
									...
									onTriggerClick : function() {
										...
										selectDictionary('area',selectBankLinkMan,bankAreaRootControl);
									}
								}]
							}]
						}]
					 ...
					 * , { columnWidth : 1, // 该列占用的宽度，标识为50％ border : false,
					 * name : "gudong_store", layout : "form", style :
					 * 'margin-top:10px', bodyStyle : 'padding-left:
					 * 0px;text-align:left;', hidden : this.isHideGudongInfo,
					 * scope : this, items : [new EnterpriseShareequity({ type :
					 * this.isLoadShareequity, bussinessType :
					 * this.bussinessType, projectId : this.projectId, isHidden :
					 * this.isGudongDiseditable && this.isAllReadOnly })]
					 *  }
					 */]
...
ExtUD.Ext.PeerPersonMainInfoPanel = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
		...
		if (_cfg.isHiddenCustomerDetailBtn!= "undefined") {
					...
					items : [{
						...
						items : [{
									...
									items : [{
										...
										anchor : "100%",
										onTriggerClick : function() {debugger
											...
											var selectCusNew = function(obj) {
												...
												op.getCmpByName('person.name')
														.setValue("");
												// op.getCmpByName('person.personNumber')
												// .setValue("");
												op.getCmpByName('person.sex')
														.setValue("");
												op
												...
												op
														...
														.setValue("");
												op
														.getCmpByName('person.archivesBelonging')
														.setValue("");
												...
												if (obj.archivesBelonging != 0
														&& obj.archivesBelonging != "")
													op
															.getCmpByName('person.archivesBelonging')
															.setValue(obj.archivesBelonging);
											}
...
//										allowBlank : false,
									...
									hidden : true,
									labelAlign : 'right',
								/*	hidden : (this.isHiddenArchives == false
											? false
											: true),*/
									items : [{
										xtype : "dickeycombo",
								...
								onTriggerClick : function() {
									...
									var EnterpriseNameStockUpdateNew = function(
										...
										if (null != obj.orgName
												&& "" != obj.orgName)
											op
													.getCmpByName('slSmallloanProject.departMentName')
													.setValue(obj.orgName);
										if (null != obj.orgId
												&& "" != obj.orgId)
											op
													.getCmpByName('slSmallloanProject.departId')
													.setValue(obj.orgId);
									}
								...
								listeners : {
									...
									loadData : function(combo) {
										...
										function test() {
											...
											combo
													.setValue(managementConsultingMineId)
										}
...
var bankCreditDatePanduan = function(begintime,endtime,payAccrualType, dayOfEveryPeriod, payintentPeriod,
		startDate, intentDatePanel, objectPanel) {
	Ext.Ajax.request({
		...
		success : function(response, request) {
			...
			if (startDate != '' && intentDate != '') {
				...
				var days = ((new Date(intentDate.substring(0, 4), intentDate
								.substring(5, intentDate.lastIndexOf('-'))
								- 1, intentDate.substring(intentDate
										.lastIndexOf('-')
										+ 1, intentDate.length))).getTime() - (new Date(
						startDate.substring(0, 4), startDate.substring(5,
								startDate.lastIndexOf('-'))
								- 1, startDate.substring(startDate
										.lastIndexOf('-')
										+ 1, startDate.length))).getTime())
						/ 1000 / 60 / 60 / 24;
						
				if(begintime == ""  || endtime =="" ) {
				}else{
					var  isbankcredtiTongyongN = Ext.getCmp('isbankcredtiTongyongN').getValue();//获取 ‘否’的值
					//授信开始时间
					var bankCreditbeginTime 	= new  Date(begintime);	
					//授信结束时间
					var bankCreditEndTime 	= new  Date(endtime);	
					
					var endtime111 = new Date(intentDate);//贷款期限--结束时间
					var startDate11 = new Date(startDate);//贷款期限 -- 开始时间
					if(bankCreditbeginTime<startDate11){
					}else{
						Ext.ux.Toast.msg('操作信息', '贷款开始时间不能小于授信开始时间!');
						return ;
					}
					if(endtime111<bankCreditEndTime){
					}else{
						Ext.ux.Toast.msg('操作信息', '贷款结束时间不能大于授信结束时间');
						return ;
					}
				}
				...
				var dmanaRate=objectPanel.getCmpByName('slSmallloanProject.dayManagementConsultingOfRate');
				if(dmanaRate){
					if (dmanaRate.getValue()) {
						objectPanel.getCmpByName('slSmallloanProject.sumManagementConsultingOfRate').setValue(dmanaRate.getValue() * days)
					}
				}
				var dfinaRate=objectPanel.getCmpByName('slSmallloanProject.dayFinanceServiceOfRate');
				if(dfinaRate){
					if (dfinaRate.getValue()) {
						objectPanel.getCmpByName('slSmallloanProject.sumFinanceServiceOfRate').setValue(dfinaRate.getValue() * days)
					}
				}
			}
		},
...
var setIntentDate = function(payAccrualType, dayOfEveryPeriod, payintentPeriod,
		startDate, intentDatePanel, objectPanel) {
	...
	Ext.Ajax.request({
		...
		success : function(response, request) {
			...
			if (startDate != '' && intentDate != '') {
				intentDate = Ext.util.Format.date(intentDate, 'Y-m-d')
				startDate = Ext.util.Format.date(startDate, 'Y-m-d')
				var days = ((new Date(intentDate.substring(0, 4), intentDate.substring(5, intentDate.lastIndexOf('-'))- 1, intentDate.substring(intentDate.lastIndexOf('-')+ 1, intentDate.length))).getTime() - (new Date(
						startDate.substring(0, 4), startDate.substring(5,startDate.lastIndexOf('-'))- 1, startDate.substring(startDate.lastIndexOf('-')+ 1, startDate.length))).getTime())
						/ 1000 / 60 / 60 / 24;
					 if(objectPanel){
					 	var dayRateField=objectPanel.getCmpByName('slSmallloanProject.dayAccrualRate');
						if(dayRateField){
							var dayAccrualRate = dayRateField.getValue();
							if (dayAccrualRate != '') {
								var sumRate=objectPanel.getCmpByName('slSmallloanProject.sumAccrualRate');
								if(sumRate){
									sumRate.setValue((dayAccrualRate * days).toFixed(7))
								}
							}
						}
					 }
...
ExtUD.Ext.newProjectInfoFinancePanel = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
		...
		if (_cfg.isHkReadOnly != "undefined") {
			this.isHkReadOnly = _cfg.isHkReadOnly;
		}
		if (_cfg.isAllReadOnly) {
			this.isAllReadOnly = _cfg.isAllReadOnly;
			this.isDiligenceReadOnly = true;
		}
		if (_cfg.isSomeReadOnly) {
			this.isSomeReadOnly = _cfg.isSomeReadOnly;
			this.isSomeReadOnly = true;
		}
		if (_cfg.isDiligenceReadOnly) {
			this.isDiligenceReadOnly = _cfg.isDiligenceReadOnly;
		}
					...
					items : [{
						...
						items : [{
							...
							items : [{
										...
										xtype : 'hidden',
										name : 'slSmallloanProject.isActualDay'
									},{
										xtype : 'hidden',
										name : 'slSmallloanProject.projectMoneyPass'
									}, {
										xtype : "textfield",
										fieldLabel :this.isAfterLoan==true?"贷款金额": "申请金额",//贷前流程和贷后的显示不一样
										fieldClass : 'field-align',
										name : "projectMoney1",
										readOnly : this.isSomeReadOnly == true
												? false
												: this.isAllReadOnly,
										allowNegative : false, // 允许负数
										style : {
										...
										/*		if(this.getOriginalContainer().getCmpByName('bankCredit.shengyuMoney')){
												    var shouxinyue =  this.getOriginalContainer().getCmpByName('bankCredit.shengyuMoney').getValue();
												    var baozhengjinyue = this.getOriginalContainer().getCmpByName('cashDeposit.cashshengyu').getValue();
												    var baozhengjinbili = this.getOriginalContainer().getCmpByName('cashDeposit.cashrate').getValue();
													var  isbankcredtiTongyongN = this.getOriginalContainer().getCmpByName('isbankcredtiTongyong').getValue();
													//判断授信金额
													if(isbankcredtiTongyongN==true){
														if(baozhengjinyue==null  || baozhengjinyue==""){
													    	Ext.ux.Toast .msg('警告', '保证金余额不能为空！');
													    	nf.setValue("");
														   	return;
												   		}
												   		if(shouxinyue*1<=nv*1){
													    	Ext.ux.Toast .msg('警告', '贷款金额不能大于授信金额！');
													    	nf.setValue("");
														   	return;
												    	}
													}
													//判断保证金
													var iscashDepositTongyongN = this.getOriginalContainer().getCmpByName('iscashDepositTongyong').getValue();
												    if(iscashDepositTongyongN==true){
												  	   	if(baozhengjinyue==null  || baozhengjinyue==""){
												    	   Ext.ux.Toast .msg('警告', '保证金比例不能为空！');
												    	   nf.setValue("");
													   	   return;
												       	}
												 	  	if(nv*1*baozhengjinbili*1/100 >= baozhengjinyue*1){
													    	Ext.ux.Toast .msg('警告', '贷款金额不能大于保证金剩余金额*保证金比例！');
													    	nf.setValue("");
														   	return;
												      	}
												 	}
												    
												 	var smoney=this.getOriginalContainer().getCmpByName('slSmallloanProject.money');
													if (smoney) {
														if (nv > parseFloat(smoney.getValue())) {
															Ext.ux.Toast .msg('警告', '贷款金额大于申请额度!');
															nf.setValue(ov);
															return;
														}
													}
											...
											listeners : {
												...
												check : function(obj, checked) {
													...
													if (flag == true) {
														...
														// 不支持一次性
														this
																.getCmpByName('isInterestByOneTimeCheck')
																.setValue(false);
														this
											...
											listeners : {
												...
												check : function(obj, checked) {
													...
													if (flag == true) {
														...
														Ext .getCmp("jixifs6" + this.idDefinition)
																.setValue(false);
														Ext
																.getCmp("meiqihkrq1"
																		+ this.idDefinition)
							...
							items : [{
								...
								items : [{
									...
									/*		var   beginTime = "";
											var   endtime  = "";
											var  beginCom = this.ownerCt.ownerCt.getCmpByName('bankCredit.beginTime');
											var  endCom = this.ownerCt.ownerCt.getCmpByName('bankCredit.endTime');
											if(beginCom==null || beginCom == ""){
											}else{
												beginTime = beginCom.getValue();
											}
											if(endCom == null  ||  endCom ==""){
											}else{
												endtime = endCom.getValue();
											}*/
										...
										//	bankCreditDatePanduan(beginTime,endtime,payAccrualType, dayOfEveryPeriod, payintentPeriod, startDate, intentDatePanel, this)
										}
									}
								}]
							}, {
										...
										items : [{
											...
											listeners : {
												...
												'blur' : function(nf) {
													...
													var overdueRateLoan = this.getCmpByName('slSmallloanProject.overdueRateLoan')
//													var sumAccrualRatenf = this.getCmpByName('slSmallloanProject.sumAccrualRate')
													...
													accrualnf.setValue((nf.getValue()/12).toFixed(5));
													if (null != dateModel && dateModel == 'dateModel_360') {
														dayAccrualRatenf.setValue((nf.getValue()/360).toFixed(5));
														overdueRateLoan.setValue((nf.getValue()/360).toFixed(5));
														if (startDate != '' && intentDate != '') {
															intentDate = Ext.util.Format.date(intentDate,'Y-m-d')
															startDate = Ext.util.Format.date(startDate,'Y-m-d')
															var days = ((new Date(
																	intentDate.substring(0,4),
																	intentDate.substring(5,intentDate.lastIndexOf('-'))- 1,
																	intentDate.substring(intentDate.lastIndexOf('-')+ 1,intentDate.length)))
																	.getTime() - (new Date(
																	startDate.substring(0,4),
																	startDate.substring(5,startDate.lastIndexOf('-'))- 1,
																	startDate.substring(startDate.lastIndexOf('-')+ 1,startDate.length)))
																	.getTime())
																	/ 1000
																	/ 60
																	/ 60 / 24
//															sumAccrualRatenf.setValue(nf.getValue()/ 360* days)
														...
														dayAccrualRatenf.setValue((nf.getValue()/365).toFixed(5))
														overdueRateLoan.setValue((nf.getValue()/365).toFixed(5));
														if (startDate != '' && intentDate != '') {
															intentDate = Ext.util.Format.date(intentDate,'Y-m-d')
															startDate = Ext.util.Format.date(startDate,'Y-m-d')
															var days = ((new Date(
...
//															sumAccrualRatenf.setValue(nf.getValue()/ 365* days)
							...
							items : [{
								...
								items : [{
									...
									listeners : {
										...
										check : function(obj, checked) {
											if (checked) {
												this.getCmpByName('slSmallloanProject.overdueReceiveWay').setValue("0");
												Ext.getCmp("yqsqfs2"+ this.idDefinition).setValue(false);
											}
										}
									}
								}, {
									xtype : "hidden",
									name : "slSmallloanProject.overdueReceiveWay",
									value : "0"
								}]
							}, {
								columnWidth : 0.135,
								...
								items : [{
									...
									listeners : {
										...
										check : function(obj, checked) {
											if (checked) {
												this.getCmpByName('slSmallloanProject.overdueReceiveWay').setValue("1");
												Ext.getCmp("yqsqfs1"+ this.idDefinition).setValue(false);
											}
										}
									}
								}]
							...
							items : [{
								...
								items : [{
									...
									listeners : {
										...
										check : function(obj, checked) {
											if (checked) {
												this.getCmpByName('slSmallloanProject.penaltyReceiveWay').setValue("0");
												Ext.getCmp("fxsqfs2"+ this.idDefinition).setValue(false);
											}
										}
									}
								}, {
									xtype : "hidden",
									name : "slSmallloanProject.penaltyReceiveWay",
									value : "0"
								}]
							}, {
								columnWidth : 0.135,
								...
								items : [{
									...
									listeners : {
										...
										check : function(obj, checked) {
											if (checked) {
												this.getCmpByName('slSmallloanProject.penaltyReceiveWay').setValue("1");
												Ext.getCmp("fxsqfs1"+ this.idDefinition).setValue(false);
											}
										}
									}
								}]
								...
								items : [{
									hideLabel : true,
									xtype : 'numberfield',
									name : "slSmallloanProject.penaltyReceiveRate",
									readOnly:true,
									fieldClass : 'field-align',
									decimalPrecision : 7,
									anchor : '100%',
							...
							items : [{
								...
								items : [{
									...
									listeners : {
										...
										check : function(obj, checked) {
											if (checked) {
												this.getCmpByName('slSmallloanProject.prepayMoney').setValue("0");
												Ext.getCmp("tqhkje2"+ this.idDefinition).setValue(false);
											}
										}
									}
								}, {
									xtype : "hidden",
									name : "slSmallloanProject.prepayMoney",
									value : "0"
								}]
							}, {
								columnWidth : 0.135,
								...
								items : [{
									...
									listeners : {
										...
										check : function(obj, checked) {
											if (checked) {
												this.getCmpByName('slSmallloanProject.prepayMoney').setValue("1");
												Ext.getCmp("tqhkje1"+ this.idDefinition).setValue(false);
											}
										}
									}
								}]
								...
								items : [{
									hideLabel : true,
									xtype : 'numberfield',
									name : "slSmallloanProject.prepayMoneyRate",
									readOnly:true,
									fieldClass : 'field-align',
									decimalPrecision : 7,
									anchor : '100%',
							...
							items : [{
								...
								items : [{
									xtype : 'numberfield',
									hideLabel:true,
									name : 'slSmallloanProject.prepayRateTypeA',
									value:0,
									anchor : "100%",
									readOnly:true
								}]
								...
								items : [{
									xtype : 'numberfield',
									hideLabel:true,
									name : 'slSmallloanProject.prepayRateTypeB',
									value:0,
									anchor : "100%",
									readOnly:true
								}]
							...
							items : [{
								...
								items : [{
									...
									listeners : {
										...
										check : function(obj, checked) {
											if (checked) {
												this.getCmpByName('slSmallloanProject.depositsReleaseWay').setValue("0");
												Ext.getCmp("release2"+ this.idDefinition).setValue(false);
											}
										}
									}
								}, {
									xtype : "hidden",
									name : "slSmallloanProject.depositsReleaseWay",
									value : "0"
								}]
							}, {
								columnWidth : 0.12,
								...
								items : [{
									...
									listeners : {
										...
										check : function(obj, checked) {
											if (checked) {
												this.getCmpByName('slSmallloanProject.depositsReleaseWay').setValue("1");
												Ext.getCmp("release1"+ this.idDefinition).setValue(false);
											}
										}
									}
								}]
							...
							items : [{
								...
								items : [{
									...
									listeners : {
										...
										check : function(obj, checked) {
											if (checked) {
												this.getCmpByName('slSmallloanProject.creditfreedType').setValue("0");
												Ext.getCmp("credit2"+ this.idDefinition).setValue(false);
											}
										}
									}
								}, {
									xtype : "hidden",
									name : "slSmallloanProject.creditfreedType",
									value : "0"
								}]
							}, {
								columnWidth : 0.12,
								...
								items : [{
									...
									listeners : {
										...
										check : function(obj, checked) {
											if (checked) {
												this.getCmpByName('slSmallloanProject.creditfreedType').setValue("1");
												Ext.getCmp("credit1"+ this.idDefinition).setValue(false);
											}
										}
									}
								}]
							...
							labelWidth : 85,
							labelAlign : 'right',
							hidden : this.isHiddenRemark == false
									? false
									: true,
							items : [{
										xtype : 'textarea',
 ...
 * @extends Ext.Panel
 */
ExtUD.Ext.bankCreditPanel = Ext.extend(Ext.Panel, {
	layout : "form",
	autoHeight : true,
	labelAlign : 'right',
	isCPLX : false,
									...
									items : [{
										...
										listeners : {
											...
											afterrender : function(combox) {
												 ...
												 * //var projectName =
												 * obj.data.projectName; //var
												 * createTime =
												 * obj.data.createTime; //var
												 * dueTime = obj.data.dueTime;
												 * //var creator =
												 * obj.data.creator;
												 * if(obj.success==true){
											 ...
											 * if(obj.success==true){
											 * this.getCmpByName('projectInfo').setTitle('项目基本信息【任务分配时间：'+createTime+'&nbsp;&nbsp;任务完成时限：'+dueTime+'&nbsp;&nbsp;当前处理人：'+creator+'】');
											 * }else{ Ext.ux.Toast.msg('操作信息',
											 * '查询的项目信息不存在!'); } }, failure :
											 * function(response,request) {
											 * Ext.ux.Toast.msg('操作信息',
											 * '查询的项目信息不存在!'); }, params : {
											 * //taskId : taskId, //businessType :
											 * '$!businessType', id :'$!id' }
											 * 
											 * });
											 *  }
...
ExtUD.Ext.CreditLoanProjectInfoPanel = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
		...
		Ext.applyIf(this, _cfg);
		var leftlabel = 85;
		ExtUD.Ext.CreditLoanProjectInfoPanel.superclass.constructor.call(this,
				{
					items : [{
						layout : "column",
						border : false,
						...
						items : [{
									 ...
									 * labelWidth : 80,
									 * 
									 * hidden : this.product == null ? false :
									 * !this.product, labelAlign : 'right',
									 * items : [{ xtype : "combo", triggerClass :
									 * 'x-form-search-trigger', hiddenName :
									 * "slSmallloanProject.teamManagerName",
									 * editable : false, fieldLabel : "项目主管",
									 * labelWidth : 85, blankText :
									 * "项目主管不能为空，请正确填写!", allowBlank : true,
									 * readOnly : this.readOnly, anchor :
									 * "100%", onTriggerClick : function(cc) {
									 * var obj = this; var appuerIdObj =
									 * obj.nextSibling(); var userIds =
									 * appuerIdObj .getValue(); if ("" ==
									 * obj.getValue()) { userIds = ""; } new
									 * UserDialog({ userIds : userIds, userName :
									 * obj.getValue(), single : false, title :
									 * "选择项目主管", callback : function(uId, uname) {
									 * obj.setValue(uname);
									 * obj.setRawValue(uname);
									 * appuerIdObj.setValue(uId); } }).show(); } }, {
									 * xtype : "hidden", disable : true, name :
									 * 'slSmallloanProject.teamManagerId', value : "" }] },
									 */{
									columnWidth : .33,
									layout : 'form',
									labelWidth : 80,
									...
									layout : "form", // 从上往下的布局
									labelWidth : 80,
									hidden:this.producthidd,
									items : [{
										fieldLabel : "业务办理人",
										xtype : "textfield",
										readOnly : true,
										name : "slSmallloanProject.yewu",
//										blankText : "临时车牌号称不能为空，请正确填写!",
										anchor : "100%"// 控制文本框的长度
									}]
								}, /*{
									columnWidth : .33, // 该列在整行中所占的百分比
...
//										readOnly : this.readOnly,
									...
									labelWidth : 80,
									labelAlign : 'right',
									hidden : (this.isArchivesHidden == false
											? false
											: true),
									items : [{
										xtype : "dickeycombo",
									...
									items : [{
										...
										xtype : "textfield",
										readOnly : true,
										name : "slSmallloanProject.carNo",
										blankText : "车牌号称不能为空，请正确填写!",
										anchor : "100%"// 控制文本框的长度
									}]
								},{
									columnWidth : .33, // 该列在整行中所占的百分比
									...
									items : [{
										fieldLabel : "客户人群",
										xtype : "combo",
										hiddenName : 'slSmallloanProject.customerGroups',
										mode : 'local',
										displayField : 'typeValue',
										valueField : 'typeId',
										readOnly : this.seeLook,
										emptyText : "请选择",
										editable :false,
										anchor : "100%",
										allowBlank:this.editorss,
										triggerAction : 'all',
										store : new Ext.data.SimpleStore({
											//data : [['法人',0],['股东',1],['个体',2],['授薪人士',3],['自由职业',4]],
											data : [['自雇人士',1],['无收入来源',2],['工薪人士',3],['自由职业',4]],
											fields:['typeValue','typeId']
										})
									}]
								},{
									...
									items : [{
										fieldLabel : "渠道名称",
										xtype : "dickeycombo",
										hiddenName : 'slSmallloanProject.channelName',
										displayField : 'itemName',
										readOnly : this.seeLook,
										itemName : '渠道名称',
										nodeKey : 'smallloan_channelName',	
										emptyText : "请选择",
										editable : false,
										anchor : "100%",
										//allowBlank : false,
									...
									items : [{
										...
										xtype : "textfield",
										readOnly : true,
										name : "slSmallloanProject.temporaryCarNo",
										blankText : "临时车牌号称不能为空，请正确填写!",
										anchor : "100%"// 控制文本框的长度
									}]
								},{
									...
									items : [{
										fieldLabel : "贷款方式",
										xtype : "combo",
										hiddenName : 'slSmallloanProject.loanWay',
										mode : 'local',
										displayField : 'typeValue',
										valueField : 'typeId',
										readOnly : this.seeLook,
									...
									items : [{
										...
										readOnly : this.seeLook,
										xtype : "textfield",
										name : "slSmallloanProject.customerSource",
										blankText : "临时车牌号称不能为空，请正确填写!",
										anchor : "100%"// 控制文本框的长度
									}]
							},{
							...
							items :[{
								xtype : 'radiogroup',
								fieldLabel : '是否转单',
								disabled:this.seeLook,
								name : 'zhundan',
//								name:'slSmallloanProject.IsSingle',
								items : [{
									boxLabel : '是',
									name : 'jiedan',
									inputValue : 1
								...
								listeners : {
									...
									change  : function(rg,r){
										var linkmanKnown=this.getCmpByName('slSmallloanProject.isSingle')
								...
								listeners : {
									...
									loadData : function(com){
										 var linkmanKnown=this.getCmpByName('slSmallloanProject.isSingle').getValue()
										 var zhundan=this.getCmpByName('zhundan')
										 if(linkmanKnown==1){
										 	zhundan.items[0].checked=true
										 	zhundan.items[1].checked=false
										 }else{
										 	zhundan.items[0].checked=false
										 	zhundan.items[1].checked=true
										 }
									}
								}
							}]
							...
							items :[{
								...
								listeners : {
									scope : this,
									change  : function(rg,r){
										var linkmanKnown=this.getCmpByName('slSmallloanProject.customerType')
										    linkmanKnown.setValue(r.inputValue)
									}
								}
							},{
								xtype : 'hidden',
								name : 'slSmallloanProject.customerType',
								listeners : {
									scope : this,
									loadData : function(com){
										 var linkmanKnown=this.getCmpByName('slSmallloanProject.customerType').getValue()
										 var xinzeng=this.getCmpByName('xinzeng')
										 if(linkmanKnown==1){
										 	xinzeng.items[0].checked=true
										 	xinzeng.items[1].checked=false
										 }else{
										 	xinzeng.items[0].checked=false
										 	xinzeng.items[1].checked=true
										 }
									}
								}
							}]
							...
							items :[{
								...
								listeners : {
									scope : this,
									change  : function(rg,r){
										var linkmanKnown=this.getCmpByName('slSmallloanProject.isShanghai')
										linkmanKnown.setValue(r.inputValue)
									}
								}
							},{
								xtype : 'hidden',
								name : 'slSmallloanProject.isShanghai',
								listeners : {
									scope : this,
									loadData : function(com){
										 var linkmanKnown=this.getCmpByName('slSmallloanProject.isShanghai').getValue()
										 var shanghai=this.getCmpByName('shanghai')
										 if(linkmanKnown==1){
										 	shanghai.items[0].checked=true
										 	shanghai.items[1].checked=false
										 }else{
										 	shanghai.items[0].checked=false
										 	shanghai.items[1].checked=true
										 }
									}
								}
							}]
...
// 速贷的客户信息
ExtUD.Ext.BankCreditFlowPanel = Ext.extend(Ext.Panel, {
	layout : "form",
	autoHeight : true,
	labelAlign : 'right',
	isAllReadOnly : true,
...
ExtUD.Ext.CustomerInfoFastPanel = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
		...
		if (typeof(_cfg.isViews) != "undefined") {
			this.isViews = _cfg.isViews;
		}
		if (typeof(_cfg.isAllReadOnly) != "undefined") {
			this.isAllReadOnly = _cfg.isAllReadOnly;
		}
		...
		var randomId = rand(100000);
		var panelId = "see_person" + randomId;
		Ext.applyIf(this, _cfg);
		ExtUD.Ext.CustomerInfoFastPanel.superclass.constructor.call(this, {
			items : [{
				layout : "column",
...
//										hiddenName:'person.sex',
										...
										listeners : {
											...
											select : function(combo, record,index) {
												...
												this.getOriginalContainer().getCmpByName('person.sexId').setValue(combo.value);
											}
										}
								},{
									xtype : 'datefield',
							...
							// 属性为hiddenName 页面控件拿不到值
								...
								editable : false,
								readOnly : this.isRead,
//								value : person == null ? null : person.belongedName,
								scope : this,
								onTriggerClick : function() {
									var obj = this;
										...
										listeners : {
											...
											'blur' : function(tf) {
												...
												var personId = this
														...
														.getValue();
												var penal = this
														.getCmpByName("person.birthday");
												var penalAge = this
														.getCmpByName("person.age");
												Ext.Ajax.request({
													url : __ctxPath
															+ '/creditFlow/customer/person/verificationPerson.do',
													method : 'POST',
														...
														var formatBrithday = brithday
																...
																+ brithday
																		.substr(
																				...
																				2);
														penal
																.setValue(formatBrithday)// 出生日期
														var nowDate = new Date();
														var nowYear = Ext.util.Format
																.date(nowDate,
																		'Y-m-d')
																.substr(0, 4);
														var brith = brithday
											...
											afterrender : function(v) {// 渲染时，如果cardNumber不为null，自动计算出生日期和生日
												if (this.getCmpByName('person.cardtype').getValue() == 309) {
													...
													if (null != cardNum && '' != cardNum && !v.readOnly) {
...
//										allowBlank:false,
										...
										listeners : {
											...
											afterrender : function(opr_obj) {
                                                 var v=this.ownerCt.ownerCt.getCmpByName('homelandProvince').getValue();
													var arrStore = new Ext.data.SimpleStore({
														url : __ctxPath
																+ '/creditFlow/multiLevelDic/listByParentIdAreaDic.do',
														fields : ['itemId', 'itemName'],
						...
						items : [{
							...
							items : [{
								...
								listeners : {
									scope : this,
										change : function(nf) {
										var jobPay=this.getCmpByName('person.jobPay').getValue();
										if(jobPay!=null && jobPay!=''){
											this.getCmpByName('person.payScale').setValue((jobPay/nf.getValue()*100).toFixed(2))
										}
										}
...
//								xtype : 'textfield',
						...
						items : [{
							...
							items : [{
								...
								listeners : {
									scope : this,
										change : function(nf) {
										var jobincome=this.getCmpByName('person.jobincome').getValue();
										if(jobincome!=null && jobincome!=''){
											this.getCmpByName('person.payScale').setValue((nf.getValue()/jobincome*100).toFixed(2))
										}
										}
						...
						labelWidth : 90,
						   items : [{
									...
									value : personData == null? null: personData.employway,
									emptyText : '请选择居住状况',
									width : 80,
									editable : false,
									readOnly : this.isRead,
								...
								xtype : "textfield",
								fieldLabel : "社保缴存基数",
								name:'person.socialMoney',
								 value : personData == null? null: personData.socialMoney,
								readOnly : this.isRead,
								editable : false
							},{
								xtype : "textfield",
								fieldLabel : "公积金缴存基数",
								name:'person.publicMoney',
								value : personData == null? null: personData.publicMoney,
								readOnly : this.isRead,
								editable : false
							}]
					}, {
						...
						items : [{
								...
								listeners : {
									scope : this,
									change  : function(rg,r){
										var linkmanKnown=this.getCmpByName('person.havehouse')
										linkmanKnown.setValue(r.inputValue)
									}
								}
							},{
								...
								listeners : {
									scope : this,
									loadData : function(com){
										 var linkmanKnown=this.getCmpByName('person.havehouse').getValue()
								...
								allowBlank : this.companyHidden,
								readOnly : this.isRead,
								 value : personData == null? null: personData.rentHouseStartDate,
								name : 'person.rentHouseStartDate'
									} 
							 ]
					}, {
						columnWidth : .34,
						...
						items : [{
								...
								listeners : {
									scope : this,
									change  : function(rg,r){
										var linkmanKnown=this.getCmpByName('person.havehouseloan')
										linkmanKnown.setValue(r.inputValue)
									}
								}
							},{
								...
								listeners : {
									scope : this,
									loadData : function(com){
										 var linkmanKnown=this.getCmpByName('person.havehouseloan').getValue()
										 var linkmanKnownObj=this.getCmpByName('linkmanKnownObj1')
										 if(linkmanKnown==1){
										 	linkmanKnownObj.items[0].checked=true
										 	linkmanKnownObj.items[1].checked=false
										 }else{
				...
				items : [{
							...
							columnWidth : .5,
							style : 'margin-left:20px',
							hidden : ((typeof(this.loanId) != 'undefined'
									&& this.loanId != null && this.loanId != '')
									? false
									: true),
							items : [{
								xtype : 'button',
								...
								handler : function() {
									Ext.Ajax.request({
										url : __ctxPath
												+ '/p2p/getInfoBpFinanceApplyUser.do?loanId='
												+ this.loanId,
										method : 'post',
										async : false,
										success : function(resp, opts) {
											var respText = resp.responseText;
											alarm_fields = Ext.util.JSON
													.decode(respText);
	...
	constructor : function(_cfg) {
		...
		ExtUD.Ext.TypeSelectInfoPanel.superclass.constructor.call(this, {
			items : [{
				 ...
				 * if(record.data.itemValue=='1'){ type = 'SmallLoan'; } } var
				 * store = new Ext.data.ArrayStore({ //autoLoad:true,
				 ...
				 * cb.view.setStore(store); } } // } } }] },{ layout:'form',
				 * columnWidth:.4, labelWidth : 120, items:[{ xtype:'combo',
				 * hiddenName :'slSmallloanProject.availableId', name:'iId',
				 * anchor:'100%', displayField :'itemName', valueField
				 * :'itemValue', triggerAction :'all', mode : 'remote',
				 * labelWidth : 120, store:new Ext.data.ArrayStore({ url:'',
				 * fields:['itemValue','itemName'],
				...
				items : [{
					...
					items : [{
						...
						items : [{
									...
									listeners : {
										...
										loadData : function(comp) {
											...
											var bankIdInProj = this.getOriginalContainer().getCmpByName('slSmallloanProject.bankId')
											if (val) {
						...
						items : [{
							...
							onTriggerClick : function() {
								var op = this.getOriginalContainer();
								var oppositeType=this.ownerCt.ownerCt.ownerCt.ownerCt.oppositeType;
								var bankInfo = function(obj) {
									op.getCmpByName('slSmallloanProject.bankId').setValue("");
									op.getCmpByName('enterpriseBank.id').setValue("");
									op.getCmpByName('enterpriseBank.name').setValue("");
								...
								if(oppositeType=='person_customer'){
								  ...
								  bankInfoWin(legalpersonid,false,0,1,bankInfo);
								}
							}
						}]
					}, {
...
//							allowBlank : false,
							...
							onTriggerClick : function() {
								...
								selectDictionary('area',selectBankLinkMan,false);
							}
						}]
					}]
				}]
...
// 标准流程项目启动页面 项目基本信息
ExtUD.Ext.bankCreditInfo = Ext.extend(Ext.Panel, {
	layout : "form",
	autoHeight : true,
	labelAlign : 'right',
	isAllReadOnly : true,
	...
	xiangmujingli : false,
	shouxinbianhaoHidden : true,
	shouxinleixingeditable : true,
	wofangzhutiHiddenReadOnly:false,
	isProductReadOnly:false,
	isWAD : true,
	userType : null,
	wfztIsAllowBlank:false,
	isDeptReadOnly:true,
	constructor : function(_cfg) {
		if (_cfg == null) {
			_cfg = {};
		}
		if (typeof(_cfg.isDeptReadOnly) != "undefined") {
			if(_cfg.isDeptReadOnly=='true'){
				this.isDeptReadOnly = _cfg.isDeptReadOnly;
			}else{
				this.isDeptReadOnly = !isGranted('_isUpdateShop_2');//如果有权限进行编辑，readOnly的属性值为false，否则为true
			}
		} 
		if (typeof(_cfg.shouxinleixingeditable) != "undefined") {
			this.shouxinleixingeditable = _cfg.shouxinleixingeditable;
		}
		if (typeof(_cfg.xiangmujingli) != "undefined") {
			this.xiangmujingli = _cfg.xiangmujingli;
		}
		if (typeof(_cfg.wofangzhutiHiddenReadOnly) != "undefined") {
			this.wofangzhutiHiddenReadOnly = _cfg.wofangzhutiHiddenReadOnly;
		}
		if (typeof(_cfg.isProductReadOnly) != "undefined") {
			this.isProductReadOnly = _cfg.isProductReadOnly;
		}
		// 授信可视标记
		if (typeof(_cfg.shouxinbianhaoHidden) != "undefined") {
			this.shouxinbianhaoHidden = _cfg.shouxinbianhaoHidden;
		}
		// 授信可视标记
		if (typeof(_cfg.wofangzhutiHidden) != "undefined") {
			this.wofangzhutiHidden = _cfg.wofangzhutiHidden;
		}
		if (typeof(_cfg.isAllReadOnly) != "undefined") {
			this.isAllReadOnly = _cfg.isAllReadOnly;
		}
		...
		Ext.applyIf(this, _cfg);
		var url = __ctxPath
				+ "/system/listBpProductParameter.do?Q_operationType_S_EQ="
				+ this.operationType;
		if (null != this.userType) {
			...
			var url = __ctxPath + "/p2p/listP2pLoanProduct.do?Q_operationType="
					+ operationType;
		}
		ExtUD.Ext.bankCreditInfo.superclass.constructor.call(this, {
			items : [{
				layout : "column",
				border : false,
				scope : this,
				...
				items : [{
					...
					items : [{
						...
						allowBlank : false,
						triggerClass : 'x-form-search-trigger',
						hiddenName : "bankCredit.departMentName",
						value : "null" == shopName ? '' : shopName,
						onTriggerClick : function() {
							var op = this.ownerCt.ownerCt.ownerCt.ownerCt;
							var EnterpriseNameStockUpdateNew = function(obj) { 
								if (null != obj.orgName && "" != obj.orgName)
									op.getCmpByName('bankCredit.departMentName').setValue(obj.orgName);
								if (null != obj.orgId && "" != obj.orgId)
									op.getCmpByName('bankCredit.departId').setValue(obj.orgId);
							}
//							selectShop(EnterpriseNameStockUpdateNew);
							
							departmentSelectWin(EnterpriseNameStockUpdateNew);
						}
					/*	listeners : {
							scope : this,
							// 设置默认用户
							'afterRender' : function(combo) {
								...
								Ext.Ajax.request({
									url : __ctxPath
											+ '/system/findParentByOrgIdOrganization.do',
									method : 'POST',
									params : {
									...
									success : function(response, request) {
										...
										obj.getCmpByName('bankCredit.departMentName').setValue(result.data.orgName);
										obj.getCmpByName('bankCredit.departId').setValue(result.data.orgId);
									}
								});
...
//							departmentSelectWin(EnterpriseNameStockUpdateNew);
					...
					layout : "form", // 从上往下的布局
					labelWidth : 85,
					hidden : this.shouxinbianhaoHidden,
					items : [{
						xtype : "textfield",
						anchor : '100%',
						name : "bankCredit.creditName",
						fieldLabel : "授信名称",
						readOnly : true,
						blankText : "授信名称不能为空，请正确填写!"
					}]
				},{
					columnWidth : .5, // 该列在整行中所占的百分比
					layout : "form", // 从上往下的布局
					...
					items : [{
						xtype : "combo",
						triggerClass : 'x-form-search-trigger',
						hiddenName : "bankCredit.appUserName",
						editable : false,
						fieldLabel : "客户经理",
						...
						listeners : {
							...
							// 设置默认用户
							'afterRender' : function(combo) {
								this.getCmpByName('bankCredit.appUserName')
										.setValue(currentUserFullName);
								this.getCmpByName('bankCredit.appUserId')
						...
						xtype : "hidden",
						name : "bankCredit.appUserId"
					}]
				}, {
					columnWidth : .25, // 该列在整行中所占的百分比
					layout : "form", // 从上往下的布局
					hidden : this.wofangzhutiHidden,
					labelWidth : 85,
					items : [{
						xtype : "dicIndepCombo",
						labelWidth : 85,
						...
						emptyText : "请选择",
						nodeKey : 'ourmainType',
						allowBlank : this.wfztIsAllowBlank,
						anchor : "100%",
						readOnly : this.wofangzhutiHiddenReadOnly,
						editable : true,
						hiddenName : "bankCredit.mineTypeName",
					 	name:"bankCredit.mineTypeName",
						scope : this,
						listeners : {
							change : function(combox, record, index) {
								var comboxValue = combox.getValue();
					 			//获取数据值
								var obj =this.ownerCt.getCmpByName("bankCredit.mineTypeId");
								var conmmineTypeName1 = this.ownerCt.ownerCt.getCmpByName('bankCredit.mineTypeName');
								conmmineTypeName1.setValue("");
								obj.setValue("");
								var  data  = combox.store.data.items;
								for(var i=0 ;i<data.length;i++){
									var  dicKey = data[i].data.dicKey;
									var itemValue = data[i].data.itemValue;
									if(comboxValue==dicKey){
										obj.setValue(dicKey);
									}
								}
								var url = '';
...
//								debugger
//								var combo = Ext.getCmp('dd_Degree');
								var combo = this.ownerCt.ownerCt.getCmpByName('bankCredit.mineName');
								combo.clearValue();
								if (comboxValue == "company_ourmain") {// 企业
									url = __ctxPath
											+ '/creditFlow/ourmain/listSlCompanyMain.do';
							...
							afterrender : function(combox) {
								//设置企业默认类型
								var conmmineTypeName = this.ownerCt.ownerCt.getCmpByName('bankCredit.mineTypeName');
								var conmmineTypeNameValue = conmmineTypeName.getValue();
								if(conmmineTypeNameValue=="" || conmmineTypeNameValue==null || conmmineTypeNameValue=='请选择'){
										conmmineTypeName.setValue("company_ourmain");
										var store = null;
										var 	combo  = 	this.ownerCt.ownerCt.getCmpByName('bankCredit.mineName')
//										var combo = Ext.getCmp('dd_Degree');
										combo.clearValue();
...
//								id : "dd_Degree",
								...
								readOnly : this.wofangzhutiHiddenReadOnly,
								editable : true,
								hiddenName : 'bankCredit.mineName',
								typeAhead : true,
								mode : 'local',
								editable : false,
								selectOnFocus : true,
								store : new Ext.data.JsonStore({}),
								triggerAction : 'all',
								blankText : "我方主体名称不能为空，请正确填写!",
								listeners : {
									change : function(combox, record, index) {
										var comboxValue = combox.getValue();
							 			//获取数据值
										var obj =this.ownerCt.getCmpByName("bankCredit.mineType");
										var  data  = combox.store.data.items;
										for(var i=0 ;i<data.length;i++){
											var  itemName = data[i].data.itemName;
											var itemValue = data[i].data.itemValue;
											if(comboxValue==itemValue){
												obj.setValue(itemValue);
											}
										}
									},
									afterrender : function(combox) {
										var bankCreditmineName = this.ownerCt.ownerCt.getCmpByName('bankCredit.mineName');//我方主题名称
										var bankCreditmineType = this.ownerCt.ownerCt.getCmpByName('bankCredit.mineType')
										var bankCreditmineNameValue = bankCreditmineName.getValue();
										if(bankCreditmineNameValue==""||bankCreditmineNameValue==null  ||bankCreditmineNameValue=="请选择" ){
												combox.setValue(31);
									 			bankCreditmineType.setValue(31);
										}else{
											var st = combox.getStore();
											st.on("load", function() {
														var v = combox.getValue();
					...
					items : [{
						...
						xtype : "combo",
						mode : 'local',
						editable : this.shouxinleixingeditable,
						allowBlank : !this.hiddenchanpin,
						triggerAction : 'all',
						hiddenName : "bankCredit.ceditType",
						displayField : 'productName',
						valueField : 'id',
						fieldLabel : "授信类型",
						editable : false,
						...
						listeners : {
							...
							select : function(combox, record) {
								var st = combox.getStore();
								var obj = this.getCmpByName("bankCredit.ceditTypeId");
								var obj1 = this.getCmpByName("bankCredit.ceditType")
								obj.setValue(record.data.id);
							}
						}
					}, {
						xtype : "hidden",
						name : "bankCredit.ceditTypeId"
					}]
				}]
			}]
		});
		...
		this.loadData({
			url : __ctxPath
					+ "/system/getByUserIdToStoreNameAndStoreNameIdOrganization.do",
			method : "POST",
			root : 'result',
			success : function(response, options) {
				var respText = response.responseText;
				...
				if (result.result != null && result.result != "undefined") {
					this.getCmpByName("slSmallloanProject.departId")
							.setValue(result.result.orgId)
					this.getCmpByName("slSmallloanProject.departMentName")
							.setValue(result.result.orgName)
				}
			}
		});
	}
	...
	constructor : function(_cfg) {
		...
		ExtUD.Ext.newProjectBaseInfo.superclass.constructor.call(this, {
			items : [{
				...
				items : [{
					columnWidth : 0.33, // 该列在整行中所占的百分比
					layout : "form", // 从上往下的布局
					labelWidth : 70,
					items : [{
						fieldLabel : '业务受理地',
//						readOnly : isGranted('_isUpdateShop_1') ? false : true ,
						readOnly:true,
						anchor : '100%',
						editable : false,
						xtype : 'combo',
						...
						triggerClass : 'x-form-search-trigger',
						hiddenName : "slSmallloanProject.departMentName",
//						value : "null" == shopName ? '' : shopName,  页面默认选择门店传值
						/*onTriggerClick : function(){
							var op = this.ownerCt.ownerCt.ownerCt.ownerCt;
							var EnterpriseNameStockUpdateNew = function(obj) {
								if (null != obj.orgName && "" != obj.orgName)
							...
							'afterRender' : function(combo) {
								...
								Ext.Ajax.request({
									...
									success : function(response, request) {
										var respText = response.responseText;
										var result = Ext.util.JSON.decode(respText);
										obj.getCmpByName('slSmallloanProject.departMentName').setValue(result.data.orgName);
										obj.getCmpByName('slSmallloanProject.departId').setValue(result.data.orgId);
									}
								});
					 ...
					 * { columnWidth : .3, // 该列在整行中所占的百分比 layout : "form", //
					 * 从上往下的布局 labelWidth : 70, items : [{ xtype : "combo",
					 * triggerClass : 'x-form-search-trigger', hiddenName :
					 * "slSmallloanProject.teamManagerName", editable : false,
					 * fieldLabel : "项目主管", blankText : "项目主管不能为空，请正确填写!",
					 * allowBlank : false, //readOnly : this.isAllReadOnly,
					 * anchor : "100%", onTriggerClick : function(cc) { var obj =
					 * this; var appuerIdObj = obj.nextSibling(); var userIds =
					 * appuerIdObj.getValue(); if (null == obj.getValue() || "" ==
					 * obj.getValue()) { userIds = ""; } new UserDialog({
					 * userIds : userIds, userName : obj.getValue(), single :
					 * false, type : 'branch', title : "选择项目主管", callback :
					 * function(uId, uname) { obj.setValue(uname);
					 * //obj.setRawValue(uname); appuerIdObj.setValue(uId); }
					 * }).show();
					 *  } }, { xtype : "hidden", name :
					 * "slSmallloanProject.teamManagerId" }] },
					 *//*{
					columnWidth : .33, // 该列在整行中所占的百分比
					layout : "form", // 从上往下的布局
					labelWidth : 70,
					items : [{
						...
						listeners : {
							...
							'afterRender' : function(combo) {
								this
										.getCmpByName('slSmallloanProject.appUserName')
										.setValue(currentUserFullName);
								this
										.getCmpByName('slSmallloanProject.appUserId')
										.setValue(currentUserId);
							}
						},
						onTriggerClick : function(cc) {
					...
					items : [{
						xtype : "combo",
						triggerClass : 'x-form-search-trigger',
						hiddenName : "slSmallloanProject.yewu",
						editable : false,
						fieldLabel : "业务办理人",
						blankText : "业务办理人不能为空，请正确填写!",
						allowBlank : false,
						// readOnly : this.isAllReadOnly,
						anchor : "100%",
					/*	listeners : {
							...
							'afterRender' : function(combo) {
								this
										.getCmpByName('slSmallloanProject.yewu')
										.setValue(currentUserFullName);
								this
										.getCmpByName('slSmallloanProject.yewuId')
										.setValue(currentUserId);
							}
						},*/
						onTriggerClick : function(cc) {
							...
							var appuerIdObj = obj.nextSibling();
							var userIds = appuerIdObj.getValue(); 
							var teamManagerName = this.ownerCt.ownerCt.getCmpByName('slSmallloanProject.teamManagerName');
							var teamManagerId = this.ownerCt.ownerCt.getCmpByName('slSmallloanProject.teamManagerId');
							var departMentName = this.ownerCt.ownerCt.getCmpByName('slSmallloanProject.departMentName');
							var departId = this.ownerCt.ownerCt.getCmpByName('slSmallloanProject.departId');
							if (null == obj.getValue() || "" == obj.getValue()) {
								userIds = "";
							}
							new UserDialog({
										...
										type : 'branch',
										title : "选择项目经理",
										callback : function(uId, uname,jobUser,jobUserId,shopId,shopName) {debugger
											obj.setValue(uname);
											// obj.setRawValue(uname);
											appuerIdObj.setValue(uId);
											teamManagerName.setValue(jobUser);
											teamManagerId.setValue(jobUserId);
											departMentName.setValue(shopName);
											departId.setValue(shopId);
										}
									}).show();
						}
					}, {
						xtype : "hidden",
						name : "slSmallloanProject.yewuId"
					}]
				},{
					columnWidth : .33, // 该列在整行中所占的百分比
					layout : "form", // 从上往下的布局
...
//						allowBlank : false,
					...
					items : [{
								...
								listeners : {
									...
									afterrender : function(combox) {
										...
										var st = combox.getStore();
										st.on("load", function() {
											if(combox.getValue()=='' || combox.getValue()==null || combox.getValue()=='请选择'){												
												combox.setValue(st.getAt(0).data.dicKey);
												if (combox.getValue() == "company_ourmain"){// 企业
					...
					items : [{
							...
							/*	store : autoHeight
								...
								store:new Ext.data.SimpleStore({
											...
											// proxy : new Ext.data.HttpProxy({
											url : true
													? __ctxPath+ '/system/getControlNameOrganization.do'
													: __ctxPath+ '/creditFlow/ourmain/listSlCompanyMain.do',
											// }),
											baseParams : {
												isGroup : true
											},
											...
											if(combox.getValue()=='' || combox.getValue()==null || combox.getValue()=='请选择'){
												Ext.Ajax.request({
													...
													success : function(response, request) {
														...
														combox.setValue(result.corName);
														combox.clearInvalid();
													},
													params : {isGroup:true}
												});
					...
					items : [{
						...
						allowBlank : this.hiddenchanpin,
						triggerAction : 'all',
						hiddenName : "slSmallloanProject.productName",
						displayField : 'productName',
						valueField : 'productName',
						fieldLabel : "产品类型",
						editable : false,
						...
						store : new Ext.data.JsonStore({
							url : __ctxPath
									+ "/system/list2BpProductParameter.do",
							totalProperty : 'totalCounts',
							autoLoad : true,
							root : 'result',
							fields : [{
								...
								// baseParams:{Q_taskId_L_EQ:161}
							}),
						/*
						 * onTriggerClick : function(combox) { var op =
						 * Ext.getCmp('createNewSLFunctionForm'); var
						 * obj=op.getCmpByName("oppositeType");
						 * if(obj!=null&&obj.value=='company_customer'){
						 * this.getStore().baseParams.Q_taskId_L_EQ=158; }else
						 * if(obj!=null&&obj.value=='person_customer'){
						 * this.getStore().baseParams.Q_taskId_L_EQ=161; }
						 * this.getStore().load(); },
						 */
						listeners : {
							scope : this,
							select : function(combox, record) {
								...
								obj.setValue(record.data.id);
								if(this.getOriginalContainer().getCmpByName('huicongFlowKey')){
									this.getOriginalContainer().getCmpByName('huicongFlowKey').setValue(record.data.taskId);
								}
							}
						}
			...
			items : [{
				...
				items : [{
					...
					listeners : {
						scope : this,
						select : function(combox) {
							this.getCmpByName('ownBpFundProject.receiverName')
									.setValue(null);
							this.getCmpByName('ownBpFundProject.reciverId')
									.setValue(null);
							this
									.getCmpByName('ownBpFundProject.receiverP2PAccountNumber')
									.setValue(null);
						},
						afterrender : function(combox) {
							var st = combox.getStore();
							...
							var obj = this
									.getCmpByName('ownBpFundProject.reciverType');
							obj.setValue("person")
						}
					}
				}]
				...
				items : [{
					...
					onTriggerClick : function() {
						...
						if (obtype == "") {
							...
							var EnterpriseNameStockUpdateNew = function(obj) {
								op
										.getCmpByName('ownBpFundProject.receiverName')
										.setValue(null);
								op.getCmpByName('ownBpFundProject.reciverId')
										.setValue(null);
								op
										.getCmpByName('ownBpFundProject.receiverP2PAccountNumber')
										.setValue(null);
								...
								if (null != obj.id && "" != obj.id) {
									op
											.getCmpByName('ownBpFundProject.reciverId')
											.setValue(obj.id);
								}
								if (null != obj.p2pAccount
										&& "" != obj.p2pAccount) {
									op
											.getCmpByName('ownBpFundProject.receiverP2PAccountNumber')
											.setValue(obj.p2pAccount);
								}

							}
							if (obtype == "enterprise") {
								selectCsCooperationEnterprise(
										EnterpriseNameStockUpdateNew, 'lenders');
							} else {
								selectCsCooperationPerson(
										EnterpriseNameStockUpdateNew, 'lenders');
							}
						...
						items : [{
							...
							listeners : {
								...
								change : function(nf) {
									...
									var intentDatePanel = this
											.getCmpByName('plBidPlan.endIntentDate');
									if (startDate != '') {
										if (this.proType == 'P_Dir'
												|| this.proType == 'B_Dir') {
											var payAccrualType = this.objectInfo
													.getCmpByName('platFormBpFundProject.payaccrualType')
													.getValue();
											var dayOfEveryPeriod = this.objectInfo
													.getCmpByName('platFormBpFundProject.dayOfEveryPeriod')
													.getValue();
											var payintentPeriod = this.objectInfo
											 ...
											 * else{ var
											 * payAccrualType=this.objectInfo.getCmpByName('ownBpFundProject.payaccrualType').getValue();
											 * var
											 * dayOfEveryPeriod=this.objectInfo.getCmpByName('ownBpFundProject.dayOfEveryPeriod').getValue();
											 * var
											 * payintentPeriod=this.objectInfo.getCmpByName('ownBpFundProject.payintentPeriod').getValue();
											 * setIntentDate(payAccrualType,dayOfEveryPeriod,payintentPeriod,startDate,intentDatePanel,this) }
											 */
									}
...
// 授信信息
ExtUD.Ext.bankcredtiPanel = Ext.extend(Ext.Panel, {
			layout : "form",
			isreadOnly : false,
			bankCreditReadOnly:false,
			constructor : function(_cfg) {
				if (_cfg == null) {
					_cfg = {};
				}
				if (typeof(_cfg.bankCreditReadOnly) != "undefined") {
					this.bankCreditReadOnly = _cfg.bankCreditReadOnly;
				}
				Ext.applyIf(this, _cfg);
				ExtUD.Ext.bankcredtiPanel.superclass.constructor.call(this, {
							layout : 'column',
							items : [{
								columnWidth : .25, // 该列在整行中所占的百分比
								layout : "form", // 从上往下的布局
								...
								items : [{
									...
									allowBlank :false,
									triggerAction : 'all',
									hiddenName : "bankCredit.bankCreditWay",
									displayField : 'productName',
									valueField : 'id',
									fieldLabel : "授信方式",
									anchor : "100%",
									readOnly : this.bankCreditReadOnly,
									store : new Ext.data.SimpleStore({
												fields :["id", "productName"],
												data :  [["1", "一次性授信"], ["2", "循环授信"]]
									}),
									listeners : {
										...
										select : function(combox, record) {
											var st = combox.getStore();
											var obj = this.getCmpByName("bankCredit.bankCreditWayName");
											var obj1 = this.getCmpByName("bankCredit.bankCreditWay")
											obj.setValue(record.data.productName);
										}
									}
								},{
									xtype : "hidden",
									name : "bankCredit.bankCreditWayName"
								}]
							},{
										columnWidth : .19,
										layout : 'form',
...
//通用流程中的授信页面
ExtUD.Ext.bankcredtiTongyong = Ext.extend(Ext.Panel, {
		layout : "form",
		isreadOnly : false,
		disableIshidden:false,
		bianhaoReadOnly:false,//设置授信编号的可读性
		constructor : function(_cfg) {
			if (_cfg == null) {
				_cfg = {};
			}
			if (typeof(_cfg.shouxinreadOnly) != "undefined") {
				this.shouxinreadOnly = _cfg.shouxinreadOnly;
			}
			
			if (typeof(_cfg.bianhaoReadOnly) != "undefined") {
				this.bianhaoReadOnly = _cfg.bianhaoReadOnly;
			}
			if (typeof(_cfg.disableIshidden) != "undefined") {
				this.disableIshidden = _cfg.disableIshidden;
			}
			...
			function common(op,rg,flag){
				if(flag){
					op.getCmpByName('bankCredit.shengyuMoney').allowBlank =false;
					op.getCmpByName('bankCredit.beginTime').allowBlank =false;
					op.getCmpByName('bankCredit.endTime').allowBlank =false;
					op.getCmpByName('bankCredit.bankCreditNum').allowBlank =false;
					op.getCmpByName('bankCredit.bankCreditNum').readOnly = false;
					op.getCmpByName('SXBH2').setVisible(true);
					op.getCmpByName('SXBH1').setVisible(false);
					if(rg.items && rg.items.items){
						rg.items.items[0].setValue(true);
						rg.items.items[1].setValue(false);
					}
				}else{
					if(op.getCmpByName('tempAssureType')){
						op.getCmpByName('slSmallloanProject.assuretypeid').setValue(op.getCmpByName('tempAssureType').getValue());
					}
					op.getCmpByName('bankCredit.id').setValue("");
					op.getCmpByName('bankCredit.shengyuMoney').setValue("");
					op.getCmpByName('slSmallloanProject.bankCreditId').setValue("");
					op.getCmpByName('bankCredit.shengyuMoney').allowBlank =true;
					op.getCmpByName('bankCredit.beginTime').setValue("");
					op.getCmpByName('bankCredit.beginTime').allowBlank = true;
					op.getCmpByName('bankCredit.endTime').setValue("");
					op.getCmpByName('bankCredit.endTime').allowBlank = true;
					op.getCmpByName('bankCredit.bankCreditNum').setValue("");
					op.getCmpByName('bankCredit.bankCreditNum').allowBlank = true;
					op.getCmpByName('SXBH2').setVisible(false);
					op.getCmpByName('SXBH1').setVisible(true);
					if(rg.items && rg.items.items){
			...
			ExtUD.Ext.bankcredtiTongyong.superclass.constructor.call(this, {
				layout : 'column',
				items : [{
					xtype : "hidden",
					name : "slSmallloanProject.bankCreditId"
				},{
					xtype : "hidden",
					name : "bankCredit.id"
				},{
					xtype : "hidden",
					name : "bankCredit.isYes"
				},{
					columnWidth : 1,
					layout : 'column',
					items : [{
						...
						items :[{
							...
							listeners : {
								...
								'change'  : function(rg,r){
									var op = this.getOriginalContainer();
									var flag=op.getCmpByName('slSmallloanProject.bankCreditId').getValue();
									if(r){
										flag=(r.inputValue==1?true:false);
									}
									common(op,rg,flag);
								}
							}
						...
						items : [{
							xtype:'label',
							labelSeparator : '',
							html:'<a href="#" onclick="seeCredit(\''+personId+'\',\''+oppositeType+'\',\''+taskId+'\')" style="color:#1678E2;padding-left:15px;">查看授信信息</a>',
							anchor : '20%'
						}]
					}]
			},{
				...
				items : [{
					...
					labelWidth : 115,
				    layout : 'form',
				    bodyStyle:'padding-left:58px;padding-top:4px;',
				    items :[{
						html : '&nbsp;&nbsp;授信编号:',	
					 	name : 'SXBH1',
					    xtype:'label',
					...
					items : [{
						...
						allowBlank : true,
						hideLabel:true,
						readOnly : this.bianhaoReadOnly,
						hiddenName : 'bankCredit.bankCreditNum',
						mode : 'local',
						editable : false,
						store : new Ext.data.JsonStore({}),
						triggerClass : 'x-form-search-trigger',
						onTriggerClick : function() {
							var op = this.getOriginalContainer();
							var  flag = op.getCmpByName('isbankcredtiTongyong').items.items[0].checked;
							if(!flag){
								Ext.ux.Toast .msg('提示', '请选择’是‘确认使用授信金额');
								return;	
							}
							var obtype = op.getCmpByName("bankCredit.shengyuMoney").getValue();
							var EnterpriseNameStockUpdateNew = function(obj) {
								op .getCmpByName('bankCredit.id') .setValue("");
								op .getCmpByName('bankCredit.shengyuMoney') .setValue("");
								op .getCmpByName('bankCredit.beginTime') .setValue("");
								...
								//修改担保方式-----------开始-----------
								op.getCmpByName('slSmallloanProject.assuretypeid') .setValue("");
								if(obj.ceditTypeId==1){//最高额授信
									if(op.getCmpByName('tempAssureType')){
										op.getCmpByName('slSmallloanProject.assuretypeid').setValue(op.getCmpByName('tempAssureType').getValue());
									}
								}else if(obj.ceditTypeId==2){//最高额抵押授信
									op.getCmpByName('slSmallloanProject.assuretypeid').setValue(815);
								}else if(obj.ceditTypeId==3){//最高额质押授信
									op.getCmpByName('slSmallloanProject.assuretypeid').setValue(816);
								}else if(obj.ceditTypeId==4){//最高额保证授信
									op.getCmpByName('slSmallloanProject.assuretypeid').setValue(818);
								}
								//修改担保方式-----------结束-----------
								
								if (null != obj.id && "" != obj.id){
									op.getCmpByName('bankCredit.id').setValue(obj.id);
									op.getCmpByName('slSmallloanProject.bankCreditId').setValue(obj.id);
								}
								if (null != obj.bankCreditNum && "" != obj.bankCreditNum) {
									op .getCmpByName('bankCredit.bankCreditNum') .setValue(obj.bankCreditNum);
								}else{
									op .getCmpByName('bankCredit.bankCreditNum') .setValue("");
								}
								if (null != obj.shengyuMoney && "" != obj.shengyuMoney) {
									op 	.getCmpByName('bankCredit.shengyuMoney') .setValue(obj.shengyuMoney);
								}else{
									op 	.getCmpByName('bankCredit.shengyuMoney') .setValue("");	
								}
								if (null != obj.beginTime && "" != obj.beginTime) {
									op 	.getCmpByName('bankCredit.beginTime') .setValue(obj.beginTime);
								}else{
									op 	.getCmpByName('bankCredit.beginTime') .setValue("");	
								}
								if (null != obj.endTime && "" != obj.endTime) {
									op 	.getCmpByName('bankCredit.endTime') .setValue(obj.endTime);
								}else{
									op 	.getCmpByName('bankCredit.endTime') .setValue("");	
								}
								Ext.getCmp('bankCreditId').setValue(obj.id);
							}
							BankCreditSelect(EnterpriseNameStockUpdateNew, 2,personId,oppositeType );
						},
						listeners : {
							scope : this,
							afterrender : function(combox) {
							  	var isbankcredtiTongyong =  this.getOriginalContainer().getCmpByName('isbankcredtiTongyong');
							  	var smalllbankCrdit = this.getOriginalContainer().getCmpByName('slSmallloanProject.bankCreditId').getValue();
							  	if(smalllbankCrdit !=null  && smalllbankCrdit!=""){
							  		isbankcredtiTongyong.setValue(true)
							  	}
							}
						}
					}]
...
function seeCredit(personId,oppositeType,taskId){
	if(Ext.getCmp('bankCreditId').getValue()){
		new SeeBankCreditDetailWin({
			oppositeID:personId,
			oppositeType:oppositeType,
			taskId:taskId,
			id:Ext.getCmp('bankCreditId').getValue()
		}).show();
	}else{
		Ext.ux.Toast.msg('操作信息', '请选择授信编号再查看!');
	}
}

//通用流程中保证人
ExtUD.Ext.bondPanel = Ext.extend(Ext.Panel, {
		layout : "form",
		isreadOnly : false,
		disableIshidden:false,
		 baozhengrenTextReadOnly:false,//保证人输入框的可读性
		...
		constructor : function(_cfg) {
			...
			if (typeof(_cfg.baozhengrenTextReadOnly) != "undefined") {
				this.baozhengrenTextReadOnly = _cfg.baozhengrenTextReadOnly;
			}
			...
			//单选按钮可编辑性
			if (typeof(_cfg.radioButtonDisable) != "undefined") {
				this.radioButtonDisable = _cfg.radioButtonDisable;
			}
			
			Ext.applyIf(this, _cfg);
			ExtUD.Ext.bondPanel.superclass.constructor.call(this, {
				layout : 'column',
				items : [{
					xtype : "hidden",
					name : "cashDeposit.id"
					...
					xtype : "hidden",
					name : "cashDeposit.isYes"
				},{
					xtype : "hidden",
					name : "slSmallloanProject.baozhengjinId"
				},{
					columnWidth : 1,
					layout : 'column',
					items : [{
						...
						items :[{
							...
							listeners : {
								...
								change  : function(rg,r){
									var op = this.getOriginalContainer();
									var flag=op.getCmpByName('slSmallloanProject.baozhengjinId').getValue();
									if(r){
										flag=(r.inputValue==1?true:false);
									}
									if(flag){
										op.getCmpByName('cashDeposit.custormerName').allowBlank =false;
										op.getCmpByName('cashDeposit.cashName').allowBlank =false;
										op.getCmpByName('cashDeposit.cashshengyu').allowBlank =false;
										op.getCmpByName('cashDeposit.cashrate').allowBlank =false;
										op.getCmpByName('JNBZJ2').setVisible(true);
										op.getCmpByName('JNBZJ1').setVisible(false);
										if(rg.items.items){
											rg.items.items[0].setValue(true);
										}
									}else{
										op.getCmpByName('cashDeposit.id').setValue("") ;
										op.getCmpByName('slSmallloanProject.baozhengjinId').setValue("") ;
										op.getCmpByName('cashDeposit.custormerName').setValue("");
										op.getCmpByName('cashDeposit.custormerName').allowBlank = true;
										op.getCmpByName('cashDeposit.cashName').setValue("");
										op.getCmpByName('cashDeposit.cashName').allowBlank = true;
										op.getCmpByName('cashDeposit.cashshengyu').setValue("");
										op.getCmpByName('cashDeposit.cashshengyu').allowBlank = true;
										op.getCmpByName('cashDeposit.cashrate').setValue("");
										op.getCmpByName('cashDeposit.cashrate').allowBlank = true;
										op.getCmpByName('JNBZJ2').setVisible(false);
										op.getCmpByName('JNBZJ1').setVisible(true);
										if(rg.items.items){
											rg.items.items[1].setValue(true);
										}
									}
					...
					items : [{
							...
							labelWidth : 115,
						    layout : 'form',
						    bodyStyle:'padding-left:47px;padding-top:4px;',
						    items :[{
								html : '&nbsp;&nbsp;保证金账户:',	
							 	name : 'JNBZJ1',
							    xtype:'label',
								...
								html : '<font color="red">*</font>保证金账户:',	
							 	name : 'JNBZJ2',
							    xtype:'label',
							    anchor : '100%'
							}]
							...
							items : [{
								...
								allowBlank : true,
								hideLabel:true,
								readOnly:this.baozhengrenTextReadOnly,
								hiddenName : 'cashDeposit.cashName',
								typeAhead : true,
								mode : 'local',
								editable : false,
								store : new Ext.data.JsonStore({}),
								triggerClass : 'x-form-search-trigger',
								blankText : "保证金账户不能为空，请正确填写!",
								onTriggerClick : function() {
									var op = this.getOriginalContainer();
									var  flag = op.getCmpByName('iscashDepositTongyong').items.items[0].checked;
									if(!flag){
										Ext.ux.Toast .msg('提示', '请选择’是‘确认使用保证金');
										return;	
									}
									var obtype = op.getCmpByName("bankCredit.shengyuMoney").getValue();
									var EnterpriseNameStockUpdateNew = function(obj) {
										...
										if (null != obj.id && "" != obj.id){
											op.getCmpByName('cashDeposit.id').setValue(obj.id);
											op.getCmpByName('slSmallloanProject.baozhengjinId').setValue(obj.id);
										}
										if (null != obj.custormerName && "" != obj.custormerName) {
											op.getCmpByName('cashDeposit.custormerName').setValue(obj.custormerName);
										}else{
											op.getCmpByName('cashDeposit.custormerName').setValue("");
										}
										if (null != obj.cashName && "" != obj.cashName) {
											op.getCmpByName('cashDeposit.cashName').setValue(obj.cashName);
										}else{
											op.getCmpByName('cashDeposit.cashName').setValue("");	
										}
										if (null != obj.cashshengyu && "" != obj.cashshengyu) {
											op.getCmpByName('cashDeposit.cashshengyu').setValue(obj.cashshengyu);
										}else{
											op.getCmpByName('cashDeposit.cashshengyu').setValue("");	
										}
										if (null != obj.cashrate && "" != obj.cashrate) {
											op.getCmpByName('cashDeposit.cashrate').setValue(obj.cashrate);
										}else{
											op.getCmpByName('cashDeposit.cashrate') .setValue("");	
										}
									}
									CashDepositSelect(EnterpriseNameStockUpdateNew, "" );
								},
								listeners : {
									scope : this,
									afterrender : function(combox) {
									  	var isbankcredtiTongyong =  this.getOriginalContainer().getCmpByName('iscashDepositTongyong');
									  	var smalllbankCrdit = this.getOriginalContainer().getCmpByName('slSmallloanProject.baozhengjinId').getValue();
									  	if(smalllbankCrdit !=null  && smalllbankCrdit!=""){
									  		isbankcredtiTongyong.setValue(true)
									 	}
									}
