//CreateNewProjectFroms.js
CreateNewProjectFroms = Ext.extend(Ext.Window, {
	formPanel : null,
	title:"",
	operationType:null,
	userId:null,
	...
	constructor : function(_cfg) {
		...
		if(this.operationType=="SmallLoan_SmallLoanBusiness"){
			...
			}else if(this.history=='efastapp'){
				this.title="企业快速审批流程",
				this.hiddenchanpin=false
			}
			...
			}else if(this.history=='fastapp'){
				this.title="个人快速审批流程",
				this.hiddenchanpin=false
			}else if(this.history=='pcnl'){
				this.title="个人贷款标准流程",
				this.hiddenchanpin=false
			}
		}else{
		...
		this.initComponents();
		CreateNewProjectFroms.superclass.constructor.call(this, {
			id : 'CreateNewProjectFroms'+this.operationType,
	...
	initComponents : function() {
		...
		if (this.operationType=="SmallLoan_PersonalCreditLoanBusiness") {
			...
			pertitle="个人客户基本信息";
				 ...
				 isHiddenCustomerDetailBtn:true,
				 formPanelId:'CreateNewProjectFroms'+this.operationType,
				 isEditEnterprise : false
			});
		}
		
		this.formPanel = new Ext.form.FormPanel({
			id : "createNewSLFunctionForms"+this.operationType,
			frame:true,
			border : false,
			monitorValid : true,
			autoScroll : true,
			...
			items:[{
				xtype : 'hidden',
				name : 'preHandler',
				value : 'creditProjectService.startCreditP2PProjectFlows'
			},{
				xtype : 'hidden',
				name:"operationType",
				...
				items : [ 
					new ExtUD.Ext.newProjectBaseInfo({
						hiddenchanpin:this.hiddenchanpin,
