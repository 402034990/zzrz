...
ProcessNextForm = Ext.extend(Ext.Panel, {
	...
	constructor : function(config) {
		...
		ProcessNextForm.superclass.constructor.call(this, {
					...
					items : [ this.formPanel,this.reSetNextPanel,this.bigprogressRunGridPanel, {
								xtype : 'fieldset',
								layout : 'form',
								autoHeight : true,
								collapsed : false,
	...
	getTaskRadioCheck : function(object) {
		...
		for (var i = 0; i < object.data.length; i++) {
			...
			if ('decision' == destType || 'fork' == destType) {
						...
						if (outers.length > 0) {
							...
							for (var i = outers.length-1; i>=0; i--) {
								...
								if (!outers[i][6]
									...
									if(outers[i][6]){
										boxValue = "同意,任务提交至["+ outers[i][1].replace(/[0-9]/g, '')+ "]";
									}else{
								...
								if (!outers[i][5]) {
									radioItems.push({
										...
										listeners:{
											scope:this,
											change:function(radio,check){
												var money=this.getOriginalContainer().getCmpByName('projectMoney2');
												var examineFieldSet=this.getOriginalContainer().getCmpByName('approvalFieldSet');
												if(radio.boxLabel.indexOf('信贷员补充放款资料')==-1){
													if(examineFieldSet){
														examineFieldSet.setVisible(false);
													}
													if(money){
														money.allowBlank=true;
													}
												}else{
													if(examineFieldSet){
														examineFieldSet.setVisible(true);
													}
													if(money){
														money.allowBlank=false;
													}
												}
												if(radio.boxLabel.indexOf('落实担保措施及合同签署')==-1){
													if(examineFieldSet){
														examineFieldSet.setVisible(false);
													}
													if(money){
				...
				this.initNextPanel(object.data[0].destination); //@@
				radioItems.push({
					boxLabel : "同意，任务提交至"+object.data[i].destination+"["+outers[0][4]+"]",
					name : 'jumpPath_display_' + this.taskId,
					// inputValue : object.data[i].name,
					inputValue : object.data[i].destination,
					destType : object.data[i].destType,
