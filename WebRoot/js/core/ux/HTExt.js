 ...
 * @return {}
 */
DicKeyCombo2 = Ext.extend(Ext.form.ComboBox, {
    ...
    constructor : function(config) {
        ...
        DicKeyCombo.superclass.constructor.call(this, {
            ...
            store : new Ext.data.ArrayStore({
                ...
                url : __ctxPath + '/system/loadppDictionary.do',
                fields : ['brand_id','brandId','brand_name','brandName','initial','initialValue','update_time','updateeTime']
            }),
...
Ext.reg('dickeycombo2', DicKeyCombo2);

DicKeyCombo3 = Ext.extend(Ext.form.ComboBox, {
	
    constructor : function(config) {
         Ext.apply(this, config);
        var nodeKey = this.nodeKey;
        var sign=1;
        var carStyle=this.carStyle;
        var ppid =this.ppid;
        var isDisplayItemName = this.isDisplayItemName;
        DicKeyCombo.superclass.constructor.call(this, {
        	triggerAction : 'all',
            store : new Ext.data.ArrayStore({
                ...
                url : __ctxPath + '/system/loadCarStyleDictionary.do',
                fields : ['series_id','seriesId','series_name','seriesName','series_group_name','seriesGroupName']
            }),
...
Ext.reg('dickeycombo3', DicKeyCombo3);

DicKeyCombo4 = Ext.extend(Ext.form.ComboBox, {
    ...
    constructor : function(config) {
        ...
        DicKeyCombo.superclass.constructor.call(this, {
            ...
            store : new Ext.data.ArrayStore({
                ...
                url : __ctxPath + '/system/loadCityDictionary.do',
                fields : ['city_id','cityId','city_name','cityName','prov_id','provId','prov_name','provName']
            }),
...
Ext.reg('dickeycombo4', DicKeyCombo4);

DicKeyCombo5 = Ext.extend(Ext.form.ComboBox, {
	
    constructor : function(config) {
         Ext.apply(this, config);
        var nodeKey = this.nodeKey;
        var sign=1;
        var carStyleId=this.carStyleId;
        var isDisplayItemName = this.isDisplayItemName;
        DicKeyCombo.superclass.constructor.call(this, {
        	triggerAction : 'all',
            store : new Ext.data.ArrayStore({
                ...
                url : __ctxPath + '/system/loadCarModelDictionary.do',
                fields : ['model_id','modelId','model_name','modelName','model_price','modelPrice','model_year','modelYear','min_reg_year','minRegYear','max_reg_year','maxRegYear','liter','literValue','gear_type','gearType','discharge_standard','dischargeStandard']
            }),
...
Ext.reg('dickeycombo5', DicKeyCombo5);
...
HT.initGridConfig = function(conf) {
    ...
    if (conf.columns) {
        ...
        if(conf.hiddenCm){
               conf.columns.unshift(new Ext.grid.RowNumberer({width :30}));
        }
        else {
            if(conf.hiddenCmTwo){
        ...
        conf.columns = [conf.sm, new Ext.grid.RowNumberer({width :100})];
    }
...
ZW.MoneyField = Ext.extend(Ext.form.TextField, {
	...
	onRender : function(ct, position) { //渲染的时候才执行
		...
		this.hiddenField = this.el.insertSibling(hide, 'before', true);
		Ext.ComponentMgr.register(Ext.get(this.hiddenField));
		this.hiddenField.value=this.hiddenFieldvalue;
	},
	change : function(nf) {
		var value = nf.getValue();
		nf.setValue(Ext.util.Format.number(value, '0,000.00'));
...
Ext.override(Ext.form.TextField, {
	 ...
	 onRender : function(ct, position){  
		  ...
		  if (this.unitText != ''){
			  	ct.getDocument().createTextNode(this.unitText);
			  	var unitPanel=ct.getDocument().createElement("font");
				var redStar=ct.getDocument().createTextNode(" "+this.unitText);
				unitPanel.appendChild(redStar);
				ct.dom.appendChild(unitPanel);
		  }  
	 }  
...
//先用Ext.apply方法添加自定义的onlynum验证函数（也可以取其他的名字） 
Ext.apply(Ext.form.VTypes,{ 
	onlynum:function(val,field){//val指这里的文本框值，field指这个文本框组件，大家要明白这个意思 
		if(isNaN(val)){
			field.setValue(null);
			return false;
		}
