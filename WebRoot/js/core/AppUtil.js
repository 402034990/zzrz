...
//验证保证金
function validCashDeposit(id,projectMoney){
	if(id){
		var cashshengyu=0;
	    var flag=false;
	    var crate=0;
		Ext.Ajax.request({
	        url :  __ctxPath + "/entityhbm/getCashDep.do",
	    	method : 'POST',
	        scope:this,
	        async :  false,
	    	success : function(response, request) {
	    		obj = Ext.util.JSON.decode(response.responseText);
	    		if(null!=obj.data.cashshengyu && ""!=obj.data.cashshengyu){
	         		cashshengyu  = obj.data.cashshengyu;
	        			if(cashshengyu-projectMoney>=0){
	        				flag=true;
	            		}
	    		}
			},
		...
		Ext.ux.Toast.msg('操作信息', '请选择保证金账户!');
		return false;
	}
}
//验证授信
function validBankCredit(id,projectMoney,startDate,intentDate){
	if(id){
		var shengyuMoney=0;
	     var flag=false;
	     var message="";
		 Ext.Ajax.request({
         	url :  __ctxPath + "/agentAproval/getBankCredit.do",
        	method : 'POST',
            scope:this,
            async :  false,
        	success : function(response, request) {
	        	obj = Ext.util.JSON.decode(response.responseText);
	        	if(null!=obj.data.shengyuMoney && ""!=obj.data.shengyuMoney){
		            var  shengyuMoney  = obj.data.shengyuMoney;
		            var beginTime=obj.data.beginTime;
		            var endTime=obj.data.endTime;
	           		var sdate=dateString(startDate);
	           		var edate=dateString(intentDate);
	            	if(beginTime<=sdate && endTime>=edate ){
	               		if(shengyuMoney-projectMoney>=0){
	            			flag=true;
		                }else{
		                   message="该授信剩余金额不足";
		                }
	            	}else{
	              		message="放款的起始日期不在该授信的有效范围内";
	           		}
       			}
			},
			failure : function(response,request) {
...
function dateString(date){
  var sdate="";
   if(null!=date && ""!=date){
      var  sydate=date.getFullYear();
      var  smdate=date.getMonth()+1;
	  var  sddate=date.getDate();
	  if(smdate<10){
	    smdate="0"+smdate;
	  }
	  if(sddate<10){
