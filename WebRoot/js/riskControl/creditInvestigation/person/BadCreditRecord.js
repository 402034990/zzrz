/**
 * @author:
 * @class BadCreditRecord
 * @extends Ext.Panel
 * @description [BadCreditRecord]管理
 * @company 北京互融时代软件有限公司
 * @createtime:
 */
BadCreditRecord = Ext.extend(Ext.Panel, {
	...
	/*		if(typeof(_cfg.customerId)!="undefined"){
		...
		// 调用父类构造
		BadCreditRecord.superclass.constructor.call(this, {
			id : 'BadCreditRecord'+this.customerType,
			title : '不良征信登记',
			region : 'center',
			layout : 'border',
	...
	initUIComponents : function() {
		...
		this.topbar = new Ext.Toolbar({
			items : [ {
				...
				xtype : 'button',
				scope : this,
				hidden:!isGranted('_add_BadCredit_'+this.customerType),
				handler : function() {
					new BadCreditForm({listPanel :this.gridPanel,type:'add',customerType:this.customerType}).show();
				}
			}, new Ext.Toolbar.Separator({
						hidden : isGranted('_add_BadCredit_'+this.customerType) ? false : true
					}), {
				iconCls : 'btn-edit',
				text : '编辑',
				xtype : 'button',
				scope : this,
				hidden:!isGranted('_edit_BadCredit_'+this.customerType),
				handler : this.editSelR
			}, new Ext.Toolbar.Separator({
						hidden : isGranted('_edit_BadCredit_'+this.customerType) ? false : true
					}), {
				iconCls : 'btn-del',
				text : '删除',
				xtype : 'button',
				scope : this,
				hidden:!isGranted('_del_BadCredit_'+this.customerType),
				handler : this.removeSelRs
			}, new Ext.Toolbar.Separator({
						hidden : isGranted('_del_BadCredit_'+this.customerType) ? false : true
					}),{
											iconCls : 'btn-xls',
											text : '导出Excel',
											xtype : 'button',
											scope : this,
											hidden:!isGranted('_out_BadCredit_'+this.customerType),
		...
		this.gridPanel = new HT.GridPanel( {
			...
			showPaging:true,
			autoScroll:true,
			url : __ctxPath + "/creditFlow/riskControl/creditInvestigation/listBpBadCredit.do?customerType="+this.customerType,
			fields : [ {
				name : 'creditId',
				type : 'long'
			}, 'businessType','badTypeValue','customerName',
			'customerType','customerId','cardnumber','loneMoney',
			'loneCompany','overdueDays','projectName','badType',
			'createDate','projectStatus','creator','reportType',
			'remarks'
			],
			columns : [ {
				header : 'creditId',
...
/*		var businessType=this.searchPanel.getCmpByName('businessType');
	...
	editSelR : function(id) {
		...
		if (selectRecords.length == 0) {
			...
			if (selectRecords[0].data.reportType==0) {
			Ext.ux.Toast.msg("信息", "内部不良记录不允许编辑！");
			return;
		}
		var creditId=selectRecords[0].data.creditId;
		new BadCreditForm({listPanel :this.gridPanel,type:'edit',customerType:this.customerType,creditId:creditId}).show();
	...
	removeSelRs : function(id) {
		...
		for (var i = 0; i < selectRecords.length; i++) {
			ids.push(selectRecords[i].data.creditId);
		}
			Ext.Ajax.request({
						url : __ctxPath
								+ '/creditFlow/riskControl/creditInvestigation/multiDelBpBadCredit.do',
						params : {
							ids : ids
						},
						method : 'post',
	...
	outToExcel:function(){
		...
		window.open(__ctxPath + '/creditFlow/riskControl/creditInvestigation/outExcelBpBadCredit.do?customerType='+this.customerType+'&businessType='+businessType+"&customerId="+customerId+"&startDate="+sdate+"&endDate="+edate,'_blank');
	}
