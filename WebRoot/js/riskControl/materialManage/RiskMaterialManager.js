RiskMaterialManager = Ext.extend(Ext.Panel, {
	...
	initUIComponents : function() {
		...
		var cm = new Ext.grid.ColumnModel([
				            ...
				            {header: "上传材料份数",fixed: true,width:93,dataIndex: 'datumNums',renderer :iteamCountRender},
				            {header: "线下材料份数",fixed: true,width:93,dataIndex: 'datumNumsOfLine',renderer : fenRenderer},
				            {header: "是否收到",fixed: true,width:67,dataIndex: 'isReceive',renderer : receiveRender},
				            {header: "是否归档",fixed: true,width:67,dataIndex: 'isArchiveConfirm',renderer : receiveRender},
				            //是否已提交  add by gao
				            {header: "是否提交",fixed: true,width:67,dataIndex: 'isPigeonhole',renderer : receiveRender},
				            //提交时间  add by gao
				            {header : '提交时间',format : 'Y-m-d',dataIndex : 'confirmTime',sortable : true,width : 82,renderer : ZW.ux.dateRenderer(this.confirmDatefield)},
