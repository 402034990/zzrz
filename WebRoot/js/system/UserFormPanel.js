...
UserFormPanel=Ext.extend(Ext.FormPanel,{
	constructor:function(conf){
		...
		this.shopId=this.shopId?this.shopId:'';
		...
		if(this.userId!=''){
			this.requiredPanel.loadData({
				...
				success:function(resp,options){debugger
					...
					//var reg=/[a-zA-Z_]+([0-9]{4})$/;//后四位为数字
					//var ss=reg.exec(alarm.data.userNumber);
					var sss=alarm.data.userNumber.split('_');
					userNumber.setValue(sss[1]); 
				}
			});
...
//		}
	...
	save:function(){
        ...
        var pre="hry_";//如果是集团版  该变量可以根据分公司的不同而改变
        var userNumber=this.getCmpByName('appUser.userNumber').getValue();
        if(userNumber<10){
			userNumber="000"+userNumber;
		}else if(userNumber>10 && userNumber<100){
			userNumber="00"+userNumber;
		}else if(userNumber>100 &&userNumber<1000){
			userNumber="0"+userNumber;
		}
		userNumber=pre+userNumber
		var passWord="";
		if(this.getCmpByName('password')!=null){
		   passWord=this.getCmpByName('password').getValue();
		}
		if(passWord!=""){
			if(passWord.length<8){
				...
				return false;
			}else if(/^[a-zA-Z0-9\u4e00-\u9fa5]+$/.test(passWord)){
