/**
 * @author
 * @class CouponsAssignIntent
 * @extends Ext.Panel
 * @description [PlBidInfo]管理
 * @company 智维软件
 * @createtime:
 */
CouponsAssignIntent = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		CouponsAssignIntent.superclass.constructor.call(this, {
					id : 'CouponsAssignIntent_'+this.userType+this.Type,
					title : this.title,
	...
	initUIComponents : function() {
		...
		this.gridPanel = new HT.GridPanel({
			tbar : this.topbar,
			region : 'center',
			id : 'CouponsAssignIntentGrid',
			plugins : [summary],
			url : __ctxPath + "/creditFlow/financingAgency/getCouponsListPlMmOrderAssignInterest.do?fundType=notCommoninterest",
	...
	seeInfo : function(){
		...
		if (s <= 0) {
			...
			Ext.Msg.confirm('派息确认','此次派息系统将会检查该项目该期尚未派息的奖励进行派息',function(btn){
		    	...
		    	Ext.Ajax.request({
						url : __ctxPath
							+ "/creditFlow/financingAgency/distributeMoneyPlMmOrderAssignInterest.do?planId="+planId+"&fundType=coupons&periods="+periods,
					method : 'post',
	...
	couponsExcel : function(){
					...
					window.open( __ctxPath +"/creditFlow/financingAgency/getcouponsExcelPlMmOrderAssignInterest.do?fundType=notCommoninterest&bidName="+bidName+"&bidProNumber="+bidProNumber+"&" +
