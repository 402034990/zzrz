 ...
 * @createtime:
 */
BpFinanceApplyUserNotAcceptView = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				BpFinanceApplyUserNotAcceptView.superclass.constructor.call(this, {
							id : 'BpFinanceApplyUserNotAcceptView',
							title : '已提交',
							region : 'center',
							layout : 'border',
			...
			initUIComponents : function() {
				...
				this.gridPanel=new HT.GridPanel({
					...
					url : __ctxPath + "/p2p/personListBpFinanceApplyUser.do?state=1",
					fields : [{
									name : 'loanId',
									type : 'int'
								}
			...
			updateSelRs : function(obj,e) {
				...
				if (rows.length == 0) {
					...
					Ext.Msg.confirm('信息确认', '是否'+btest1, function(btn) {
       					 if (btn == 'yes') {
							Ext.Ajax.request({
