/**
 * @author
 * @class CouponsBpFundIntent
 * @extends Ext.Panel
 * @description [PlBidInfo]管理
 * @company 智维软件
 * @createtime:
 */
CouponsBpFundIntent = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		CouponsBpFundIntent.superclass.constructor.call(this, {
					id : 'CouponsBpFundIntent_'+this.userType+this.Type,
					title : this.title,
	...
	initUIComponents : function() {
		...
		this.gridPanel = new HT.GridPanel({
			...
			id : 'couponsGrid',
			plugins : [summary],
			url : __ctxPath + "/creditFlow/finance/getCouponsListSlFundIntent.do?fundType=notCommoninterest",
			fields : [{
						name : 'infoId',
						type : 'int'
					}, 'planId', 'bidPlanName', 'bidPlanProjectNum', 'rebateType', 'rebateWay','payintentPeriod','incomeMoney','fundType','intentDate','factDate','afterMoney'],
			columns : [{
						header : 'infoId',
						dataIndex : 'infoId',
						hidden : true
	...
	seeInfo : function(){
		...
		if (s <= 0) {
			...
			var record = s[0];
			new CouponsBpfundIntentDetail({
			  fundType:'notCommoninterest',
		...
		if (s <= 0) {
			...
			Ext.Msg.confirm('派息确认','此次派息系统将会检查该项目该期尚未派息的奖励进行派息',function(btn){
		    	...
		    	Ext.Ajax.request({
					url : __ctxPath
							+ "/creditFlow/finance/distributeMoneyBpFundIntent.do?planId="+planId+"&fundType=coupons&payintentPeriod="+payintentPeriod,
					method : 'post',
	...
	couponsExcel : function(){
					...
					window.open( __ctxPath +"/creditFlow/finance/excelCouponsBpfundDetailSlFundIntent.do?fundType=notCommoninterest&bidName="+bidName+"&bidProNumber="+bidProNumber+"&" +
