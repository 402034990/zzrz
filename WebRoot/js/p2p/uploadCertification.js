 ...
 * @createtime:
 */
uploadCertification = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				uploadCertification.superclass.constructor.call(this, {
			...
			initUIComponents : function() {
				...
				this.gridPanel=new HT.GridPanel({
					...
					url : __ctxPath + "/p2p/upLoadListWebFinanceApplyUploads.do?state=3",
					fields : [{
									name : 'id',
									type : 'int'
								}
			...
			updateRs : function(obj,e) {
				...
				if(rows>1){
					...
					var id=rows[0].data.id;
					Ext.Msg.confirm('信息确认', '是否'+btest, function(btn) {
       					 if (btn == 'yes') {
							Ext.Ajax.request({
						...
						buttons : [{
								...
								handler : function(){
									...
									Ext.Ajax.request({
										url : __ctxPath + '/p2p/updateStateWebFinanceApplyUploads.do',
										method:'post',
										params:{'id':rows[0].data.id,'state':'2','webFinanceApplyUploads.rejectReason':rejectReason},
										//async:false,
										success: function(resp,opts) {
											Ext.ux.Toast.msg('操作信息', '操作成功!');
